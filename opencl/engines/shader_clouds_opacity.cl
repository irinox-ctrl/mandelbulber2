...
#ifdef CLOUDS
float RemapCloudDensity(__constant sClInConstants *consts, float rawNoise)
{
	float threshold = 1.0f - consts->params.cloudsDensity * 2.0f;
	float softness = max(consts->params.cloudsSoftness, 0.05f);
	float t = clamp((rawNoise - threshold) / softness, 0.0f, 1.0f);
	float opacity = pow(t, consts->params.cloudsDensityGamma);
	if (consts->params.cloudsSharpEdges)
		opacity = 0.5f * (1.0f + tanh((opacity - 0.5f) * consts->params.cloudsSharpness));
	return opacity * consts->params.cloudsOpacity;
}
	...
	float3 point, float distance, float detailSize, float *distanceOut)
{
	...
	if (h > 0.0f)
	{
		rawNoise = NormalizedOctavePerlinNoise3D_0_1(point2.x / consts->params.cloudsPeriod,
		...
		distToCloud = fabs(1.0f - rawNoise - consts->params.cloudsDensity) * 0.2f
									* consts->params.cloudsPeriod * consts->params.cloudsDEMultiplier;

		opacityOut = RemapCloudDensity(consts, rawNoise) * h;
	}

	float baseDistance = max(max(distToGeometry, distToCloud),
	...
	if (consts->params.cloudsAdaptiveDetail && opacityOut > 0.01f && rawNoise > 0.05f
			&& rawNoise < 0.95f)
	{
		float eps = consts->params.cloudsPeriod * 0.05f;
		float nXp = NormalizedOctavePerlinNoise3D_0_1((point2.x + eps) / consts->params.cloudsPeriod,
			point2.y / consts->params.cloudsPeriod, point2.z / consts->params.cloudsPeriod,
			consts->params.cloudsSpeed * consts->params.frameNo, consts->params.cloudsIterations,
			perlinNoiseSeeds);
		float grad = fabs(nXp - rawNoise) / eps;
		baseDistance *= clamp(1.0f / (1.0f + grad * 4.0f), 0.35f, 1.0f);
	}
