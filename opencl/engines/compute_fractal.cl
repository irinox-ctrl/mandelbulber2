		...
		if (mutationActive)
		{
			...
			if (mut->clipEnabled && i >= mut->clipIterStart && i < mut->clipIterStop && mut->clipType != 0)
			{
				...
				switch (mut->clipType)
				{
					...
					case 31: { float r2d=native_sqrt(cz.x*cz.x+cz.y*cz.y); float theta=atan2(cz.y,cz.x); int n=mut->clipNPoints; float starR=cr*(1.0f+mut->clipAmplitude*native_cos((float)n*theta)); clipDist=r2d-starR; break; } // Star polygon clip
					...
					case 33: { float r2d2=native_sqrt(cz.x*cz.x+cz.y*cz.y); float theta2=atan2(cz.y,cz.x); float m=(float)mut->clipNPoints; float n1=ca,n2=cb,n3=cc; float t=m*theta2/4.0f; float r_sf=pow(pow(fabs(native_cos(t)/sx),n2)+pow(fabs(native_sin(t)/sy),n3),-1.0f/n1); clipDist=r2d2-r_sf*cr; break; } // Superformula clip
					case 34: { float r2d3=native_sqrt(cz.x*cz.x+cz.y*cz.y); float theta3=atan2(cz.y,cz.x); int ng=mut->clipNPoints; float gearR=cr*(1.0f+0.1f*tanh(native_sin((float)ng*theta3))); clipDist=r2d3-gearR; break; } // Gear clip
					case 35: { float r2d4=native_sqrt(cz.x*cz.x+cz.y*cz.y); float theta4=atan2(cz.y,cz.x); int ns=mut->clipNPoints; float spiralR=ca*native_exp(-cb*theta4)*(1.0f+cc*native_cos((float)ns*theta4)); clipDist=r2d4-spiralR; break; } // Spiral polygon clip
					case 36: { float3 seeds0=(float3)(ca,0.0f,0.0f); float3 seeds1=(float3)(-ca,cb,0.0f); float3 seeds2=(float3)(0.0f,-ca,cc); float3 seeds3=(float3)(cb,cc,-ca); float minVD=1e20f; float vd0=length(cz-seeds0); if(vd0<minVD)minVD=vd0; float vd1=length(cz-seeds1); if(vd1<minVD)minVD=vd1; float vd2=length(cz-seeds2); if(vd2<minVD)minVD=vd2; float vd3=length(cz-seeds3); if(vd3<minVD)minVD=vd3; clipDist=minVD-cr; break; } // Voronoi clip
					...
					case 43: { float time=(float)i*mut->clipFrequency; float tR=cr*(1.0f+mut->clipAmplitude*native_sin(time)); clipDist=length(cz)-tR; break; } // Temporal pulsing clip
					...
					case 49: { float nx=tanh(ca*cz.x+cb*cz.y+cc*cz.z); float ny=tanh(cb*cz.x-ca*cz.y+cc*cz.z); float nz=tanh(cc*cz.x+ca*cz.y-cb*cz.z); clipDist=native_sqrt(nx*nx+ny*ny+nz*nz)-cr; break; } // Neural tanh clip
					...
					case 56: { float t56=atan2(cz.y,cz.x); float kx=native_cos(2.0f*t56)*(cR+cr*native_cos(3.0f*t56)); float ky=native_sin(2.0f*t56)*(cR+cr*native_cos(3.0f*t56)); float kz=cr*native_sin(3.0f*t56); clipDist=native_sqrt((cz.x-kx)*(cz.x-kx)+(cz.y-ky)*(cz.y-ky)+(cz.z-kz)*(cz.z-kz))-ca; break; } // Torus knot
					case 57: { clipDist=cz.x*cz.x*cz.z+cz.y*cz.y*cz.z-ca*cz.x*cz.y-cr; break; } // Cross-cap surface
					case 58: { float phi58=0.5f*(1.0f+native_sqrt(5.0f)); float p2=phi58*phi58; float r2=cz.x*cz.x+cz.y*cz.y+cz.z*cz.z; clipDist=4.0f*(p2*cz.x*cz.x-cz.y*cz.y)*(p2*cz.y*cz.y-cz.z*cz.z)*(p2*cz.z*cz.z-cz.x*cz.x)-(1.0f+2.0f*phi58)*(r2-ca*ca)*(r2-ca*ca); break; } // Barth sextic
					...
					case 62: { float r62=native_sqrt(cz.x*cz.x+cz.y*cz.y); float t62=atan2(cz.y,cz.x); float spiralR=ca*native_exp(cb*t62); clipDist=fabs(r62-spiralR)+fabs(cz.z)-cr; break; } // Logarithmic spiral shell
					case 63: { float t63=atan2(cz.y,cz.x); float r63=native_sqrt(cz.x*cz.x+cz.y*cz.y); float h1=r63-cR-cr*native_cos(ca*cz.z); float h2=r63-cR+cr*native_cos(ca*cz.z+M_PI_F); clipDist=fmin(native_sqrt(h1*h1+pow(cr*native_sin(ca*cz.z),2.0f)),native_sqrt(h2*h2+pow(cr*native_sin(ca*cz.z+M_PI_F),2.0f)))-cb; break; } // Double helix
					case 64: { float dx64=fabs(cz.x)-sx; float dy64=fabs(cz.y)-sy; float dz64=fabs(cz.z)-sz; float chamfer=ca; clipDist=fmax(fmax(dx64,fmax(dy64,dz64)),(dx64+dy64+dz64)*chamfer); break; } // Chamfered cube
					case 65: { float dx65=fmax(fabs(cz.x)-sx,0.0f); float dy65=fmax(fabs(cz.y)-sy,0.0f); float dz65=fmax(fabs(cz.z)-sz,0.0f); clipDist=native_sqrt(dx65*dx65+dy65*dy65+dz65*dz65)-cr; break; } // Rounded box
					...
					case 69: { float d69a=fmax(fabs(cz.x),fmax(fabs(cz.y),fabs(cz.z)))-sx; float d69b=(fabs(cz.x)+fabs(cz.y)+fabs(cz.z))*0.577f-cr; clipDist=fmax(d69a,d69b); break; } // Truncated octahedron
					case 70: { float q70x=fabs(cz.x); float q70y=fabs(cz.y); clipDist=fmax(q70x*0.866f+q70y*0.5f,q70y)-cr; clipDist=fmax(clipDist,fabs(cz.z)-sz); break; } // Hex prism
					case 71: { clipDist=fmax(fabs(cz.z)-sz,fmax(fabs(cz.x)*0.866f+cz.y*0.5f,-cz.y)-cr); break; } // Triangular prism
					case 72: { float rxy72a=native_sqrt(cz.x*cz.x+cz.y*cz.y); float d72a=native_sqrt((rxy72a-cR)*(rxy72a-cR)+cz.z*cz.z)-cr; float ryz72=native_sqrt(cz.y*cz.y+cz.z*cz.z); float d72b=native_sqrt((ryz72-cR)*(ryz72-cR)+cz.x*cz.x)-cr; clipDist=fmin(d72a,d72b); break; } // Link
					case 73: { float r73=native_sqrt(cz.x*cz.x+cz.y*cz.y)-cr; float h73=fabs(cz.z)-sz; clipDist=fmin(fmax(r73,h73),0.0f)+native_sqrt(fmax(r73,0.0f)*fmax(r73,0.0f)+fmax(h73,0.0f)*fmax(h73,0.0f)); break; } // Capped cylinder
					case 74: { float capH=fmax(0.0f,fmin(sz,cz.z)); clipDist=native_sqrt(cz.x*cz.x+cz.y*cz.y+(cz.z-capH)*(cz.z-capH))-cr; break; } // Capsule
					case 75: { float t75=atan2(cz.y,cz.x); float r75=native_sqrt(cz.x*cz.x+cz.y*cz.y); float knotR=cR+cr*native_cos(3.0f*t75); clipDist=native_sqrt((r75-knotR)*(r75-knotR)+cz.z*cz.z)-ca; break; } // Trefoil knot
					case 76: { float t76=atan2(cz.y,cz.x); float r76=native_sqrt(cz.x*cz.x+cz.y*cz.y)-cR; float localZ=cz.z*native_cos(t76*0.5f)-r76*native_sin(t76*0.5f); clipDist=fabs(localZ)-ca; break; } // Mobius strip approx
					case 77: { float sc77x=native_sin(cz.x*ca)*native_sin(cz.x*ca); float sc77y=native_sin(cz.y*ca)*native_sin(cz.y*ca); float sc77z=native_sin(cz.z*ca)*native_sin(cz.z*ca); clipDist=sc77x+sc77y+sc77z-cr; break; } // Sine cage
					...
					case 80: { float r80=native_sqrt(cz.x*cz.x+cz.y*cz.y)-cR; float ang80=ca*atan2(cz.y,cz.x)/(2.0f*M_PI_F); clipDist=r80*r80+(cz.z-ang80)*(cz.z-ang80)-cr*cr; break; } // Corkscrew
					case 81: { float u81=atan2(cz.y,cz.x); float v81=atan2(cz.z,native_sqrt(cz.x*cz.x+cz.y*cz.y)-cR); float kb=ca*(1.0f-0.5f*native_cos(u81))*native_sin(v81)-cb*native_cos(v81); clipDist=fabs(kb)-cr; break; } // Klein bottle section
					case 82: { float r82=native_sqrt(cz.x*cz.x+cz.y*cz.y); float wave82=cr+ca*native_sin(cb*cz.z); clipDist=r82-wave82; break; } // Wavy cylinder
					case 83: { float twist83=ca*cz.z; float rx83=cz.x*native_cos(twist83)-cz.y*native_sin(twist83); float ry83=cz.x*native_sin(twist83)+cz.y*native_cos(twist83); clipDist=fmax(fabs(rx83)-sx,fmax(fabs(ry83)-sy,fabs(cz.z)-sz)); break; } // Twisted box
					case 84: { float bend84=ca*cz.x; float bz84=cz.z+bend84*bend84; float rxy84=native_sqrt(cz.x*cz.x+cz.y*cz.y)-cR; clipDist=native_sqrt(rxy84*rxy84+bz84*bz84)-cr; break; } // Bent torus
					case 85: { clipDist=cz.z-ca*native_sin(cb*cz.x)-cc*native_cos(cb*cz.y)-cr; break; } // Corrugated plane
					case 86: { float r86=length(cz); if(r86<1e-15f)r86=1e-15f; float theta86=acos(cz.z/r86); float phi86=atan2(cz.y,cz.x); float m86=(float)mut->clipNPoints; float st86=pow(pow(fabs(native_cos(m86*theta86/4.0f)),cb)+pow(fabs(native_sin(m86*theta86/4.0f)),cc),-1.0f/ca); float sp86=pow(pow(fabs(native_cos(m86*phi86/4.0f)),cb)+pow(fabs(native_sin(m86*phi86/4.0f)),cc),-1.0f/ca); clipDist=r86-cr*st86*sp86; break; } // Supershape 3D
					case 87: { float r87=length(cz); float cosTheta87=cz.z/(r87+1e-15f); clipDist=r87-cr*(1.0f+ca*cosTheta87); break; } // Egg shape
					...
					case 89: { float fx89=fabs(fmod(cz.x+ca,2.0f*ca))-ca; float fy89=fabs(fmod(cz.y+ca,2.0f*ca))-ca; float fz89=fabs(fmod(cz.z+ca,2.0f*ca))-ca; clipDist=native_sqrt(fx89*fx89+fy89*fy89+fz89*fz89)-cr; break; } // Folded sphere
					...
					case 94: { float u94=cz.x,v94=cz.y; float ex94=u94-u94*u94*u94/3.0f+u94*v94*v94; float ey94=v94-v94*v94*v94/3.0f+v94*u94*u94; float ez94=u94*u94-v94*v94; clipDist=native_sqrt((cz.x-ex94)*(cz.x-ex94)+(cz.y-ey94)*(cz.y-ey94)+(cz.z-ez94)*(cz.z-ez94))-cr; break; } // Enneper surface
					...
					case 99: { float d99s=length(cz)-cr; float d99b=fmax(fabs(cz.x)-sx,fmax(fabs(cz.y)-sy,fabs(cz.z)-sz)); float k99=mut->clipSmoothK; float h99=fmax(0.0f,fmin(1.0f,0.5f-0.5f*(d99s-d99b)/fmax(k99,1e-10f))); clipDist=d99s*(1.0f-h99)+d99b*h99+k99*h99*(1.0f-h99); break; } // Smooth intersection
...
case 1: { z.x=fmax(fmin(z.x,ta),-ta); z.y=fmax(fmin(z.y,ta),-ta); z.z=fmax(fmin(z.z,ta),-ta); aux.DE*=sf; break; }
						...
						case 14: { float rxy=native_sqrt(z.x*z.x+z.y*z.y); float lim=ta+tb*fabs(z.z); if(rxy>lim){float s=lim/rxy; z.x*=s; z.y*=s; aux.DE*=s;} break; }
						...
						case 16: { float rxy=native_sqrt(z.x*z.x+z.y*z.y)-ta; float dt=native_sqrt(rxy*rxy+z.z*z.z); if(dt>tb){float s=tb/dt; rxy*=s; z.z*=s; float newR=rxy+ta; if(native_sqrt(z.x*z.x+z.y*z.y)>1e-10){float scale=newR/native_sqrt(z.x*z.x+z.y*z.y); z.x*=scale; z.y*=scale;} aux.DE*=s;} break; }
						...
						case 29: { float theta29=atan2(z.y,z.x); float r29=native_sqrt(z.x*z.x+z.y*z.y); if(r29>ta){r29=ta; z.x=r29*native_cos(theta29); z.y=r29*native_sin(theta29); aux.DE*=ta/fmax(native_sqrt(z.x*z.x+z.y*z.y),1e-10);} break; }
						...
						case 34: { float theta34=atan2(z.y,z.x); float r34=native_sqrt(z.x*z.x+z.y*z.y); float spiralLim=ta+tb*theta34/(2.0f*M_PI_F); if(r34>spiralLim){z.x*=spiralLim/r34; z.y*=spiralLim/r34;} break; }
						...
						case 49: { float capH49=fmax(0.0f,fmin(ta,z.z)); float rd49=native_sqrt(z.x*z.x+z.y*z.y+(z.z-capH49)*(z.z-capH49)); if(rd49>tb){float s=tb/rd49; z.x*=s; z.y*=s; z.z=capH49+(z.z-capH49)*s; aux.DE*=s;} break; }
						case 50: { float p50=fmax(tc,0.5f); float d50=pow(pow(fabs(z.x/fmax(ta,0.01f)),p50)+pow(fabs(z.y/fmax(ta,0.01f)),p50)+pow(fabs(z.z/fmax(tb,0.01f)),p50),1.0f/p50); if(d50>1.0f){z/=d50; aux.DE/=d50;} break; }
						default: break;
					}
...
case 1: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; float r2jb=z.x*z.x+z.y*z.y+z.z*z.z; float minR2jb=tb*tb; float fixedR2jb=1.0f; if(r2jb<minR2jb){z*=fixedR2jb/minR2jb; aux.DE*=fixedR2jb/minR2jb;} else if(r2jb<fixedR2jb){z*=fixedR2jb/r2jb; aux.DE*=fixedR2jb/r2jb;} z=z*tc+(float4)(td,td,td,0.0f); aux.DE=aux.DE*fabs(tc)+1.0f; break; }
						...
						case 16: { float tw16=ta*M_PI_F/180.0f*z.z; float c16=native_cos(tw16),s16=native_sin(tw16); float nx16=z.x*c16-z.y*s16; z.y=z.x*s16+z.y*c16; z.x=nx16; z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0f; break; }
						...
						case 23: { float r2so=z.x*z.x+z.y*z.y+z.z*z.z; float minR2so=ta*ta; float fixedR2so=tb*tb; if(r2so<minR2so){float m=fixedR2so/minR2so; z*=m; aux.DE*=m;} else if(r2so<fixedR2so){float m=fixedR2so/r2so; z*=m; aux.DE*=m;} break; }
						...
						case 26: { float r2rb=z.x*z.x+z.y*z.y+z.z*z.z; float theta26=atan2(z.y,z.x); float phi26=atan2(native_sqrt(z.x*z.x+z.y*z.y),z.z); float rr26=native_sqrt(r2rb); rr26=fabs(rr26+ta)-fabs(rr26-ta)-rr26; z.x=rr26*native_sin(phi26)*native_cos(theta26); z.y=rr26*native_sin(phi26)*native_sin(theta26); z.z=rr26*native_cos(phi26); z*=tc; aux.DE=aux.DE*fabs(tc)+1.0f; break; }
						...
						case 36: { float q36x=z.x; float q36y=z.y*0.866f+z.x*0.5f; if(q36y>ta){q36y=2.0f*ta-q36y;} if(q36x>ta){q36x=2.0f*ta-q36x;} z.x=q36x; z.y=(q36y-z.x*0.5f)/0.866f; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0f; break; }
						case 37: { float d37x=fabs(z.x)-ta; float d37y=fabs(z.y)-ta; float d37z=fabs(z.z)-ta; if(d37x>0&&d37y>0){z.x*=ta/(ta+d37x); z.y*=ta/(ta+d37y);} if(d37y>0&&d37z>0){z.y*=ta/(ta+d37y); z.z*=ta/(ta+d37z);} if(d37x>0&&d37z>0){z.x*=ta/(ta+d37x); z.z*=ta/(ta+d37z);} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0f; break; }
						...
						case 45: { float r45=native_sqrt(z.x*z.x+z.y*z.y); float th45=atan2(z.y,z.x); th45=fabs(th45+ta)-fabs(th45-ta)-th45; z.x=r45*native_cos(th45); z.y=r45*native_sin(th45); z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0f; break; }
				...
				if (mut->mdEnabled && i >= mut->mdIterStart && i < mut->mdIterStop && mut->mdType != 0)
				{
					...
					switch (mut->mdType)
					{
						...
						case 11: { float rxy=native_sqrt(z.x*z.x+z.y*z.y); float minR=ta; if(rxy<minR&&rxy>1e-21f){float s=minR/rxy; z.x*=s;z.y*=s;aux.DE*=s;} z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0f; break; }
						...
						case 16: { float fold=ta; z.x=fabs(z.x+fold)-fabs(z.x-fold)-z.x; z.y=fabs(z.y+fold)-fabs(z.y-fold)-z.y; z.z=fabs(z.z+fold)-fabs(z.z-fold)-z.z; float r2=z.x*z.x+z.y*z.y+z.z*z.z; float k=fmax(tb/fmax(r2,1e-21f),1.0f); z*=k; aux.DE*=k; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0f; z.x+=td; break; }
						...
						case 32: { float a32=ta*M_PI_F/180.0f; float c32=native_cos(a32),s32=native_sin(a32); float ny=z.y*c32-z.z*s32; z.z=z.y*s32+z.z*c32; z.y=ny; z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; float r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tc*tc){z*=td/(tc*tc);aux.DE*=td/(tc*tc);} else if(r2<td){z*=td/r2;aux.DE*=td/r2;} z*=sf; aux.DE=aux.DE*fabs(sf)+1.0f; break; }
