	...
	__constant sClInConstants *consts, int initRandomSeed)
{
	...
	for (int eye = 0; eye < 2; eye++)
	{
		...
		float3 defaultLightVector = (float3){cos(lightAlpha - 0.5f * M_PI_F) * cos(lightBeta),
			sin(lightAlpha - 0.5f * M_PI_F) * cos(lightBeta), sin(lightBeta)};
		defaultLightVector = Matrix33MulFloat3(rot, defaultLightVector);
...
// view vector
	...
	float aspectRatio = width / height;
		...
		if (found)
		{
			...
			float3 lightVector;
			if (renderData.numberOfLights > 0)
				lightVector = fast_normalize(renderData.lights[0].position - point);
			else
				lightVector = defaultLightVector;
			...
			float ao = 1.0f;
#ifdef FAST_AO
			float aoStep = distThresh * 5.0f;
			float d1 = CalculateDistance(consts, point + normal * aoStep, &calcParam, &renderData).distance;
			float d2 =
				CalculateDistance(consts, point + normal * aoStep * 2.5f, &calcParam, &renderData).distance;
			ao = clamp((d1 / aoStep + d2 / (aoStep * 2.5f)) * 0.5f, 0.0f, 1.0f);
#endif
			colour = colour * (shade + specular) * (0.3f + 0.7f * ao);
			alpha = 1.0f;
			...
			if (fogDensity > 0.0f)
			{
				...
				colour = mix(consts->params.fogColor, colour, fogFactor);
			}
#endif
		}
		else
	...
	pixel.image.s1 = colour.s1 + glow;
	pixel.image.s2 = colour.s2 + glow;
	pixel.zBuffer = scan;
	pixel.color = 128;
	pixel.opacity = 0;
	pixel.alpha = alpha * 65535;
