	...
	sClCalcParams *calcParam, float intensity)
{
	...
	for (float i = start; i < distance; i += step)
	{
		...
		{
			float opacity =
				IterOpacity(step, outF.iters, consts->params.iterFogEffectiveN,
		...
		if (consts->params.distanceFogShadows)
		{
			...
			float depthAlongRay = dot(point2 - consts->params.camera, input->viewVector);
			...
			float opacity = DistanceFogOpacity(step, dist, consts->params.volFogDistanceFromSurface,
				...
				depthAlongRay, point2, consts->params.camera, &distanceShifted);
		...
		if (consts->params.fogEnabled && consts->params.fogCastShadows)
		{
