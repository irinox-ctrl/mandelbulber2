	...
	sShaderInputDataCl *input, sClCalcParams *calcParam)
{
	...
	int vectorsToUse = renderData->AOVectorsCount;
#ifdef ADAPTIVE_AO_SAMPLES
	{
		formulaOut probeOut = CalculateDistance(consts, input->point + input->normal * start_dist,
			calcParam, renderData);
		float curvature = fabs(probeOut.distance - input->lastDist) / start_dist;
		if (curvature < 0.05f)
			vectorsToUse = max(2, vectorsToUse / 4);
		else if (curvature > 0.3f)
			vectorsToUse = min(vectorsToUse * 2, renderData->AOVectorsCount);
	}
#endif

#ifndef MONTE_CARLO
	for (int i = 0; i < vectorsToUse; i++)
	{
#else
	int i = Random(renderData->AOVectorsCount - 1, &calcParam->randomSeed);
		...
		for (float r = start_dist; r < end_dist; r += dist * 2.0f)
		{
			...
			if (consts->params.iterFogEnabled)
				opacity =
					IterOpacity(dist * 2.0f, outF.iters, consts->params.iterFogEffectiveN,
