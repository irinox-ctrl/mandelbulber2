 ...
 *
 * INTEGRATION ANALYSIS (added for ticket #9):
 *
 * Current state: Standalone kernel for GPU perturbation theory (3D Mandelbulb)
 * - Reference orbit computed at arbitrary precision (MPFR) on CPU
 * - GPU operates on float3 deltaZ (small perturbation stays within float precision)
 * - Includes rebasing (RebaseGPU) and series approximation (MatVecMul)
 *
 * Integration requirements for nebula.cl:
 * 1. CPU-side: Compute reference orbit using MPFR, store in cl_buffer
 * 2. Kernel interface: Pass reference orbit buffer + current iteration index
 * 3. Iteration modification: Replace z with z_ref + deltaZ, iterate deltaZ
 * 4. Rebasing: When |deltaZ| exceeds threshold, rebase to nearest reference point
 *
 * Performance characteristics:
 * - Additional memory: O(maxIter * 12 bytes) per reference orbit
 * - Additional computation: ~10% overhead for rebasing checks
 * - Benefit: Extends usable zoom from ~1e7 to ~1e30+
 *
 * Trigger threshold recommendation: zoom > 1e8 (float precision exhausted)
 */
