...
typedef struct
{
	...
	cl_int volumetricMaxSteps;
	cl_int volumetricPreviewActive;
	cl_float volumetricPreviewStepMul;
	cl_int volFogMode;
	...
	cl_int volumetricBlueNoiseJitter;
	cl_int SSAO_useNormalBuffer;
	cl_int voxelFogCacheEnabled;
	cl_int voxelFogCacheResolution;
	cl_float3 voxelFogCacheMin;
	cl_float3 voxelFogCacheSize;
	...
	cl_float cloudsDensityGamma;
	cl_float cloudsSoftness;
	cl_float constantFactor;
	cl_float DEFactor; // factor for distance estimation steps
	cl_float marchingOmega; // sphere tracing over-relaxation factor
	cl_float progressiveAdaptiveThreshold; // adaptive MC refinement threshold
	...
	cl_float volumetricJitterStrength;
	...
	// Deep zoom extended precision fields (hi + lo pairs)
	// Used when zoom > DEEP_ZOOM_THRESHOLD (1e7)
	cl_int useDeepZoom; // Flag: 1 = use extended precision, 0 = standard
	cl_float cameraDistanceHi; // High part of camera distance
	cl_float cameraDistanceLo; // Low part of camera distance
	cl_float3 cameraHi; // High parts of camera position
	cl_float3 cameraLo; // Low parts of camera position
	cl_float3 targetHi; // High parts of target position
	cl_float3 targetLo; // Low parts of target position
	cl_float resolutionHi; // High part of pixel resolution
	cl_float resolutionLo; // Low part of pixel resolution
} sParamRenderCl;

#ifdef DEEP_ZOOM_DOUBLE_PRECISION
// Double-precision version of sParamRenderCl for deep zoom scenarios
// Enabled via -DDEEP_ZOOM_DOUBLE_PRECISION cmake flag
typedef struct
{
	cl_double cameraDistanceToTarget;
	cl_double3 camera;
	cl_double3 target;
	cl_double resolution;
	// Note: Other critical precision fields can be added here as needed
	// (e.g., DE factors, thinning, etc.)
} sParamRenderClDouble;
#endif
...
inline sParamRenderCl clCopySParamRenderCl(const sParamRender &source)
{
	...
	target.volumetricMaxSteps = source.volumetricMaxSteps;
	target.volumetricPreviewActive = source.volumetricPreviewActive;
	target.volumetricPreviewStepMul = source.volumetricPreviewStepMul;
	target.volFogMode = source.volFogMode;
	...
	target.cloudsAdaptiveDetail = source.cloudsAdaptiveDetail;
	...
	target.volumetricBlueNoiseJitter = source.volumetricBlueNoiseJitter;
	target.SSAO_useNormalBuffer = source.SSAO_useNormalBuffer;
	target.voxelFogCacheEnabled = source.voxelFogCacheEnabled;
	target.voxelFogCacheResolution = source.voxelFogCacheResolution;
	target.voxelFogCacheMin = toClFloat3(source.voxelFogCacheMin);
	target.voxelFogCacheSize = toClFloat3(source.voxelFogCacheSize);
	...
	// WARNING: Precision loss occurs here for deep zoom (>1e7).
	// CPU holds zoom as double (64-bit), but GPU receives cl_float (32-bit).
	// At 1e-10 pixel scale, float32 has only ~7 significant digits,
	// causing visible quantization artifacts in rendered images.
	// See deep_zoom_perturbation.cl for perturbation theory approach.
	...
	target.cloudsDensityGamma = source.cloudsDensityGamma;
	target.cloudsSoftness = source.cloudsSoftness;
	target.constantFactor = source.constantFactor;
	target.DEFactor = source.DEFactor;
	target.marchingOmega = source.marchingOmega;
	target.progressiveAdaptiveThreshold = source.progressiveAdaptiveThreshold;
	...
	target.iterFogEffectiveN = source.iterFogEffectiveN;
	...
	target.volumetricJitterStrength = source.volumetricJitterStrength;
	...
	// Deep zoom threshold: beyond this, float32 loses significant precision
	const double DEEP_ZOOM_THRESHOLD = 1e7;

	if (source.cameraDistanceToTarget > DEEP_ZOOM_THRESHOLD) {
		target.useDeepZoom = 1;
		
		// Split double into hi + lo float pair (Dekker splitting)
		auto splitDouble = [](double d, float& hi, float& lo) {
			hi = static_cast<float>(d);
			lo = static_cast<float>(d - static_cast<double>(hi));
		};
		
		splitDouble(source.cameraDistanceToTarget, target.cameraDistanceHi, target.cameraDistanceLo);
		
		splitDouble(source.camera.x, target.cameraHi.s[0], target.cameraLo.s[0]);
		splitDouble(source.camera.y, target.cameraHi.s[1], target.cameraLo.s[1]);
		splitDouble(source.camera.z, target.cameraHi.s[2], target.cameraLo.s[2]);
		
		splitDouble(source.target.x, target.targetHi.s[0], target.targetLo.s[0]);
		splitDouble(source.target.y, target.targetHi.s[1], target.targetLo.s[1]);
		splitDouble(source.target.z, target.targetHi.s[2], target.targetLo.s[2]);
		
		double resolution = 1.0 / source.imageHeight;
		splitDouble(resolution, target.resolutionHi, target.resolutionLo);
	} else {
		target.useDeepZoom = 0;
		// Standard single-precision path (existing behavior)
		target.cameraDistanceHi = 0.0f;
		target.cameraDistanceLo = 0.0f;
	}
