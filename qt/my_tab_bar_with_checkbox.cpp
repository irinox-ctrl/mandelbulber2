...
void MyTabBarWithCheckBox::RemoveCheckBox(int i)
{
	...
	const QString checkBoxName = listOfCheboxes.key(i, QString());

	// Detach before delete — QTabBar::removeTab() crashes if the tab button widget is already gone.
	setTabButton(i, QTabBar::LeftSide, nullptr);

	if (!checkBoxName.isEmpty())
	{
		if (QWidget *checkBox = findChild<QWidget *>(checkBoxName))
			delete checkBox;
		listOfCheboxes.remove(checkBoxName);
	}

	for (auto it = listOfCheboxes.begin(); it != listOfCheboxes.end(); ++it)
	{
		if (it.value() > i) it.value()--;
	}
}
