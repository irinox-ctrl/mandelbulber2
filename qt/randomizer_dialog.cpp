 ...
 *                                             ,B" ]L,,p%%%,,,§;, "K
 * Copyright (C) 2019-23 Mandelbulber Team     §R-==%w["'~5]m%=L.=~5N
 *                                        ,=mm=§M ]=4 yJKA"/-Nsaj  "Bw,==,,
...
namespace
{
void RenderPreviewWidget(cThumbnailWidget *widget,
	const std::shared_ptr<cParameterContainer> &params,
	const std::shared_ptr<cFractalContainer> &fractalParams)
{
	widget->SetForceRender(true);
	widget->AssignParameters(params, fractalParams);
	if (!widget->IsRendered())
	{
		widget->slotRender();
		widget->WaitUntilRendered();
	}
	widget->update();
}
} // namespace
...
{
	blockClose = true;
	RenderPreviewWidget(ui->previewwidget_actual, actualParams, actualFractParams);
	blockClose = false;
}
...
void cRandomizerDialog::RefreshReferenceSkyImage()
{
	...
	*parFractSky = *actualFractParams;
	...
	blockClose = true;
	RenderPreviewWidget(referenceSkyPreview.get(), parSky, parFractSky);
	blockClose = false;
}
...
void cRandomizerDialog::CalcReferenceNoise()
{
	...
	blockClose = true;
	RenderPreviewWidget(referenceNoisePreview.get(), actualParams, actualFractParams);
	blockClose = false;
...
void cRandomizerDialog::slotClickedSaveButton()
{
	...
	connect(dialog, &QDialog::accepted, this, [dialog, parSettings]() {
...
void cRandomizerDialog::slotCleanUp()
{
	...
	*actualFractParamsCleaned = *actualFractParams;
	...
	for (int i = 0; i < list.size(); i++)
	{
		...
		blockClose = true;
		RenderPreviewWidget(cleanedPreview.get(), actualParamsCleaned, actualFractParamsCleaned);
		std::shared_ptr<cImage> actualImage = ui->previewwidget_actual->GetImage();
