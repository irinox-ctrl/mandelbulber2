...
void cVoxelExportDialog::on_pushButton_start_render_layers_clicked()
{
	if (!slicerBusy)
	{
		...
		if (folder.exists())
		{
			...
			QObject::connect(voxelExport, &cVoxelExport::updateProgressAndStatus, this,
				&cVoxelExportDialog::slotUpdateProgressAndStatus);

			QThread *thread = new QThread; // deleted by deleteLater()
			voxelExport->moveToThread(thread);
			QObject::connect(thread, &QThread::started, voxelExport, &cVoxelExport::ProcessVolume);
			thread->setObjectName("Slicer");
			thread->start();
			QObject::connect(voxelExport, &cVoxelExport::finished, this, &cVoxelExportDialog::slotSlicerFinished);
			QObject::connect(voxelExport, &cVoxelExport::finished, voxelExport, &QObject::deleteLater);
			QObject::connect(voxelExport, &cVoxelExport::finished, thread, &QThread::quit);
