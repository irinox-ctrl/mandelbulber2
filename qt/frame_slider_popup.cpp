		...
		: QFrame(parent), ui(new Ui::cFrameSliderPopup)
{
	...
	connect(ui->slider, &QSlider::sliderReleased, this, &cFrameSliderPopup::slotSliderReleased);
	connect(ui->slider, &QSlider::sliderPressed, this, &cFrameSliderPopup::sliderPressed);
	connect(ui->slider, &QSlider::sliderMoved, this, &cFrameSliderPopup::sliderMoved);
	connect(ui->buClose, &QAbstractButton::pressed, this, &cFrameSliderPopup::hide);
	connect(ui->buDouble, &QAbstractButton::pressed, this, &cFrameSliderPopup::doublePressed);
	connect(ui->buHalf, &QAbstractButton::pressed, this, &cFrameSliderPopup::halfPressed);
	connect(ui->buInteger, &QAbstractButton::pressed, this, &cFrameSliderPopup::intPressed);
	connect(ui->buReset, &QAbstractButton::pressed, this, &cFrameSliderPopup::resetPressed);
	connect(ui->buZero, &QAbstractButton::pressed, this, &cFrameSliderPopup::zeroPressed);
	connect(ui->buMinus, &QAbstractButton::pressed, this, &cFrameSliderPopup::minusPressed);
	...
	connect(sliderTimer, &QTimer::timeout, this, &cFrameSliderPopup::slotSliderTimerTrigger);
...
void cFrameSliderPopup::SetIntegerMode(int min, int max, int val)
{
	...
	if (!integerMode)
	{
		...
		ui->buMinus->hide();
		sliderTimer->stop();
		connect(ui->slider, QOverload<int>::of(&QSlider::valueChanged), this,
		QOverload<int>::of(&cFrameSliderPopup::valueChanged));
	}
}
...
void cFrameSliderPopup::SetDialMode(int scale, double val)
{
	...
	dialScale = scale;
	dialMode = true;
	connect(ui->dial, QOverload<int>::of(&QDial::valueChanged), this, &cFrameSliderPopup::slotDialValueChanged);
	connect(ui->buUp, &QAbstractButton::pressed, this, &cFrameSliderPopup::upPressed);
	connect(ui->buDown, &QAbstractButton::pressed, this, &cFrameSliderPopup::downPressed);
	connect(ui->buLeft, &QAbstractButton::pressed, this, &cFrameSliderPopup::leftPressed);
	connect(ui->buRight, &QAbstractButton::pressed, this, &cFrameSliderPopup::rightPressed);
}
