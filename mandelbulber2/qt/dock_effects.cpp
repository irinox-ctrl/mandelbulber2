...
#define TRAP_UI (static_cast<cEffectsSingleTrapTab*>(ui->tab_single_trap_lights)->trapUi())
#define VOL_UI (static_cast<cEffectsVolumetricTab*>(ui->tab_volumetric)->volumetricUi())
#define RAY_UI (static_cast<cEffectsRaytracingTab*>(ui->tab_raytracing)->raytracingUi())
#define POST_UI (static_cast<cEffectsPostTab*>(ui->tab_postEffects)->postUi())
#define LIGHTS_UI (static_cast<cEffectsLightsTab*>(ui->tab_lights)->lightsUi())
#define BG_UI (static_cast<cEffectsBackgroundTab*>(ui->tab_background)->backgroundUi())
		...
		: QWidget(parent), cMyWidgetWithParams(), ui(new Ui::cDockEffects), dofLivePreviewEnabled(false)
{
	...
	if (TRAP_UI->comboBox_single_trap_lights_combine_mode)
	{
		TRAP_UI->comboBox_single_trap_lights_combine_mode->clear();
		TRAP_UI->comboBox_single_trap_lights_combine_mode->addItems(
...
void cDockEffects::EnsureSingleTrapEngineOnAndActiveThrough(int layerIndex)
{
	...
	if (TRAP_UI->groupCheck_single_trap_lights_enabled)
		TRAP_UI->groupCheck_single_trap_lights_enabled->setChecked(true);
	if (TRAP_UI->spinboxInt_single_trap_lights_active_count)
		TRAP_UI->spinboxInt_single_trap_lights_active_count->setValue(n);
}
...
void cDockEffects::SyncSingleTrapActiveCountToHighestEnabledLayer()
{
	...
	if (TRAP_UI->spinboxInt_single_trap_lights_active_count)
		TRAP_UI->spinboxInt_single_trap_lights_active_count->setValue(maxOn);
}

void cDockEffects::RefreshSingleTrapLightPresetCombo()
{
	if (!TRAP_UI->comboBox_single_trap_presets) return;
	TRAP_UI->comboBox_single_trap_presets->clear();
	...
	for (const QString &name : files)
		TRAP_UI->comboBox_single_trap_presets->addItem(name, QVariant(dir.absoluteFilePath(name)));
}
...
void cDockEffects::slotSingleTrapPresetLoad()
{
	...
	if (!TRAP_UI->comboBox_single_trap_presets) return;
	const int idx = TRAP_UI->comboBox_single_trap_presets->currentIndex();
	if (idx < 0) return;
	const QString path = TRAP_UI->comboBox_single_trap_presets->itemData(idx).toString();
...
void cDockEffects::slotSingleTrapPresetSave()
{
	...
	RefreshSingleTrapLightPresetCombo();
	const int newIdx = TRAP_UI->comboBox_single_trap_presets->findData(QVariant(path));
	if (newIdx >= 0) TRAP_UI->comboBox_single_trap_presets->setCurrentIndex(newIdx);
}
...
{
	connect(VOL_UI->button_calculateFog, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonAutoFog);
	connect(VOL_UI->pushButton_autoFog_iteration, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonAutoFogIteration);

	connect(RAY_UI->comboBox_ambient_occlusion_mode, QOverload<int>::of(&QComboBox::currentIndexChanged),
		this, &cDockEffects::slotChangedComboAmbientOcclusionMode);

	connect(LIGHTS_UI->logedit_aux_light_manual_placement_dist, &QLineEdit::textChanged, this,
		&cDockEffects::slotEditedLineEditManualLightPlacementDistance);

	connect(RAY_UI->pushButton_DOF_set_focus, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonAutoFocusCenter);
	connect(RAY_UI->pushButton_DOF_update, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonUpdatePostEffects);
	connect(POST_UI->pushButton_post_effects_update, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonUpdatePostEffects);

	connect(VOL_UI->pushButton_set_fog_by_mouse, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonSetFogByMouse);
	connect(LIGHTS_UI->pushButton_place_random_lights_by_mouse, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonPlaceRandomLightsByMouse);

	connect(VOL_UI->toolButton_fog_distance_double, &QToolButton::clicked, this,
		&cDockEffects::slotPressedButtonFogDistDouble);
	connect(VOL_UI->toolButton_fog_distance_half, &QToolButton::clicked, this,
		&cDockEffects::slotPressedButtonFogDistHalf);

	connect(LIGHTS_UI->checkBox_aux_light_place_behind, QOverload<int>::of(&QCheckBox::stateChanged), this,
		&cDockEffects::slotChangedPlaceLightBehindObjects);

	connect(RAY_UI->groupCheck_DOF_monte_carlo, &QGroupBox::toggled, this,
		&cDockEffects::slotChangedEnableMCDOF);
	connect(RAY_UI->checkBox_DOF_MC_global_illumination, QOverload<int>::of(&QCheckBox::stateChanged), this,
		&cDockEffects::slotChangedEnableGI);
	connect(RAY_UI->checkBox_MC_global_illumination_volumetric, QOverload<int>::of(&QCheckBox::stateChanged),
		this, &cDockEffects::slotChangedEnableGIVolumetric);
	connect(RAY_UI->checkBox_MC_fog_illumination, QOverload<int>::of(&QCheckBox::stateChanged), this,
		&cDockEffects::slotChangedEnableGIFog);

	connect(RAY_UI->logedit_DOF_focus, &QLineEdit::textChanged, this,
		&cDockEffects::slotDOFParameterChanged);
	connect(RAY_UI->spinbox_DOF_radius, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
		&cDockEffects::slotDOFParameterChanged);
	connect(RAY_UI->spinboxInt_DOF_number_of_passes, QOverload<int>::of(&QSpinBox::valueChanged), this,
		&cDockEffects::slotDOFParameterChanged);
	connect(RAY_UI->spinbox_DOF_blur_opacity, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
		&cDockEffects::slotDOFParameterChanged);
	connect(RAY_UI->spinbox_DOF_max_radius, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
		&cDockEffects::slotDOFParameterChanged);

	connect(LIGHTS_UI->comboBox_random_lights_coloring_type, QOverload<int>::of(&QComboBox::currentIndexChanged),
		this, &cDockEffects::slotRandomLightColoringTypeChanged);

	connect(VOL_UI->pushButton_clouds_randomize, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonCloudsRandomize);
	connect(VOL_UI->pushButton_clouds_apply_preset, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonCloudsApplyPreset);

	connect(LIGHTS_UI->widget_light_sources_manager,
		&cLightSourcesManager::signalChangeLightPlacementDistance, this,
		&cDockEffects::slotSetAuxLightManualPlacementDistance);

	connect(RAY_UI->groupCheck_env_mapping_enable, &QGroupBox::toggled, this,
	...
	for (int i = 1; i <= 20; i++)
	{
		...
		if (shapeCombo)
		{
			connect(shapeCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
				&cDockEffects::slotChangedSingleTrapLightShape);
		}
		...
		if (groupCheck)
		{
			connect(groupCheck, &QGroupBox::clicked, this,
				&cDockEffects::slotSingleTrapLayerEnabledToggled);
		}
	}

	connect(TRAP_UI->pushButton_single_trap_copy, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonCopySingleTrapLight);

	connect(TRAP_UI->pushButton_single_trap_randomize, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonRandomizeSingleTrapLight);

	connect(TRAP_UI->pushButton_single_trap_world_anchor, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonSingleTrapWorldAnchor);

	connect(TRAP_UI->pushButton_single_trap_randomize_all, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonRandomizeAllSingleTrapLights);

	connect(TRAP_UI->pushButton_single_trap_preset_refresh, &QPushButton::clicked, this,
		&cDockEffects::slotSingleTrapPresetRefresh);
	connect(TRAP_UI->pushButton_single_trap_preset_load, &QPushButton::clicked, this,
		&cDockEffects::slotSingleTrapPresetLoad);
	connect(TRAP_UI->comboBox_single_trap_presets, QOverload<int>::of(&QComboBox::activated), this,
		&cDockEffects::slotSingleTrapPresetLoad);
	connect(TRAP_UI->pushButton_single_trap_preset_save, &QPushButton::clicked, this,
		&cDockEffects::slotSingleTrapPresetSave);

	connect(TRAP_UI->pushButton_single_trap_reset_all, &QPushButton::clicked, this,
		&cDockEffects::slotPressedButtonSingleTrapResetAll);

	connect(ui->pushButton_local_navi, &QPushButton::clicked, this, &cDockEffects::slotPressedButtonNavi);

	connect(RAY_UI->checkB_MCShowNoiseLevels, &QCheckBox::toggled, this,
...
{
	SynchronizeInterfaceWindow(VOL_UI->groupCheck_basic_fog_enabled, par, qInterface::write);
}

void cDockEffects::slotSynchronizeInterfaceDOF(std::shared_ptr<cParameterContainer> par) const
{
	SynchronizeInterfaceWindow(RAY_UI->groupCheck_DOF_enabled, par, qInterface::write);
}

void cDockEffects::slotSynchronizeInterfaceLights(std::shared_ptr<cParameterContainer> par) const
{
	SynchronizeInterfaceWindow(LIGHTS_UI->widget_light_sources_manager, par, qInterface::write);
}

void cDockEffects::slotSynchronizeInterfaceRandomLights(
	std::shared_ptr<cParameterContainer> par) const
{
	SynchronizeInterfaceWindow(LIGHTS_UI->groupCheck_random_lights_group, par, qInterface::write);
}

double cDockEffects::GetAuxLightManualPlacementDistance() const
{
	return systemData.locale.toDouble(LIGHTS_UI->logedit_aux_light_manual_placement_dist->text());
}

void cDockEffects::slotSetAuxLightManualPlacementDistance(double dist) const
{
	LIGHTS_UI->logedit_aux_light_manual_placement_dist->setText(QString("%L1").arg(dist));
}
...
void cDockEffects::slotChangedComboAmbientOcclusionMode(int index) const
{
	...
	RAY_UI->frame_lightmap_texture->setEnabled(enabled);
	enabled = index == params::AOModeFast ? true : false;
	RAY_UI->spinbox_ambient_occlusion_fast_tune->setEnabled(enabled);
}
...
void cDockEffects::slotPressedButtonAutoFogIteration()
{
	...
	const int qualityIndex = std::max(0, std::min(params->Get<int>("auto_fog_quality"), 2));
	...
	bool ok = autoFog.AutoTuneAll(renderParams.get(), fractals.get(), nullptr, quality,
	...
	if (ok)
	{
		autoFog::WriteAutoFogResults(params, *renderParams, autoFog::eAutoFogTuneMode::All, true);
	}
	else
	{
...
void cDockEffects::slotChangedEnableMCDOF(bool state)
{
	if (state)
	{
		params::enumAOMode AOMode =
			params::enumAOMode(RAY_UI->comboBox_ambient_occlusion_mode->currentIndex());
		if (AOMode == params::AOModeScreenSpace)
		{
			RAY_UI->groupCheck_ambient_occlusion_enabled->setChecked(false);
		}
	}
}
...
{
	RAY_UI->checkBox_MC_global_illumination_volumetric->setEnabled(state);
	RAY_UI->checkBox_MC_fog_illumination->setEnabled(state);
}

void cDockEffects::slotChangedEnableGIVolumetric(int state)
{
	if (state == Qt::Checked) RAY_UI->checkBox_MC_fog_illumination->setChecked(false);
}

void cDockEffects::slotChangedEnableGIFog(int state)
{
	if (state == Qt::Checked) RAY_UI->checkBox_MC_global_illumination_volumetric->setChecked(false);
}
...
void cDockEffects::slotPressedButtonCloudsRandomize()
{
	...
	VOL_UI->spinboxInt_clouds_random_seed->setValue(rnd);
}

void cDockEffects::slotPressedButtonCloudsApplyPreset()
{
	CHECK_MAIN_INTERFACE();
	SynchronizeInterfaceWindow(this, params, qInterface::read);

	const int presetIndex = std::max(0, std::min(VOL_UI->comboBox_clouds_preset->currentIndex(), 3));
	cloudPresets::ApplyPreset(
		params, static_cast<cloudPresets::ePreset>(presetIndex));
...
{
	LIGHTS_UI->widget_light_sources_manager->Regenerate();
}
	...
	std::shared_ptr<cParameterContainer> _params, std::shared_ptr<cFractalContainer> _fractalParams)
{
	...
	LIGHTS_UI->widget_light_sources_manager->AssignParameterContainers(_params, _fractalParams);

	LIGHTS_UI->widget_light_sources_manager->Init();
}
	...
	RenderedImage *_renderedImage, QComboBox *_mouseFunctionCombo)
{
	...
	LIGHTS_UI->widget_light_sources_manager->AssignSpecialWidgets(
...
{
	LIGHTS_UI->label_random_lights_first_color->setVisible(index > 0);
	LIGHTS_UI->colorButton_random_lights_color->setVisible(index > 0);
	LIGHTS_UI->label_random_lights_second_color->setVisible(index >= 2);
	LIGHTS_UI->colorButton_random_lights_color_2->setVisible(index >= 2);
}

void cDockEffects::slotUpdatePrimitivesCombos()
{
	cPrimitives::PrepareComboBox(VOL_UI->comboBox_basic_fog_primitive, params);
	cPrimitives::PrepareComboBox(VOL_UI->comboBox_distance_fog_primitive, params);
	cPrimitives::PrepareComboBox(VOL_UI->comboBox_iteration_fog_primitive, params);
	cPrimitives::PrepareComboBox(VOL_UI->comboBox_clouds_primitive, params);
}
...
void cDockEffects::slotEnvMappingToggled(bool state)
{
	if (state)
	{
		RAY_UI->groupCheck_raytraced_reflections->setChecked(false);
	}
}
...
void cDockEffects::slotPressedButtonPlaceLightAtCamera()
{
	...
	if (TRAP_UI->spinboxInt_single_trap_lights_solo_layer)
		TRAP_UI->spinboxInt_single_trap_lights_solo_layer->setValue(layerNum);
...
{
	CHECK_MAIN_INTERFACE();
	int from = TRAP_UI->spinboxInt_single_trap_copy_from->value();
	int to = TRAP_UI->spinboxInt_single_trap_copy_to->value();
...
void cDockEffects::slotPressedButtonSingleTrapResetAll()
{
	...
	if (TRAP_UI->spinboxInt_single_trap_lights_solo_layer)
		TRAP_UI->spinboxInt_single_trap_lights_solo_layer->setValue(0);
	if (TRAP_UI->comboBox_single_trap_lights_combine_mode)
		TRAP_UI->comboBox_single_trap_lights_combine_mode->setCurrentIndex(0);
