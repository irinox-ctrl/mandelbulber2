...
cDockNavigation::cDockNavigation(QWidget *parent) : QWidget(parent), ui(new Ui::cDockNavigation)
{
	...
	cameraDebounce = new QTimer(this);
	cameraDebounce->setSingleShot(true);
	cameraDebounce->setInterval(150);
	connect(cameraDebounce, &QTimer::timeout, this, [this]() {
		if (manipulations) manipulations->CameraDistanceEdited();
	});

	if (ui->bu_move_forward)
		ui->bu_move_forward->setToolTip(tr("Move camera forward"));
	if (ui->bu_move_backward)
		ui->bu_move_backward->setToolTip(tr("Move camera backward"));
	if (ui->bu_move_left)
		ui->bu_move_left->setToolTip(tr("Strafe camera left"));
	if (ui->bu_move_right)
		ui->bu_move_right->setToolTip(tr("Strafe camera right"));
	if (ui->bu_move_up)
		ui->bu_move_up->setToolTip(tr("Move camera up"));
	if (ui->bu_move_down)
		ui->bu_move_down->setToolTip(tr("Move camera down"));
}
...
{
	auto connectMove = [&](QAbstractButton *button) {
		if (button)
			connect(button, &QAbstractButton::clicked, this, &cDockNavigation::slotCameraMove);
	};
	connectMove(ui->bu_move_up);
	connectMove(ui->bu_move_down);
	connectMove(ui->bu_move_left);
	connectMove(ui->bu_move_right);
	connectMove(ui->bu_move_forward);
	connectMove(ui->bu_move_backward);

	auto connectRotate = [&](QAbstractButton *button) {
		if (button)
			connect(button, &QAbstractButton::clicked, this, &cDockNavigation::slotCameraRotation);
	};
	connectRotate(ui->bu_rotate_up);
	connectRotate(ui->bu_rotate_down);
	connectRotate(ui->bu_rotate_left);
	connectRotate(ui->bu_rotate_right);
	connectRotate(ui->bu_rotate_roll_left);
	connectRotate(ui->bu_rotate_roll_right);

	auto connectCameraEdit = [&](QLineEdit *edit) {
		if (edit)
			connect(edit, &QLineEdit::editingFinished, this, &cDockNavigation::slotCameraOrTargetEdited);
	};
	connectCameraEdit(ui->vect3_camera_x);
	connectCameraEdit(ui->vect3_camera_y);
	connectCameraEdit(ui->vect3_camera_z);
	connectCameraEdit(ui->vect3_target_x);
	connectCameraEdit(ui->vect3_target_y);
	connectCameraEdit(ui->vect3_target_z);

	auto connectRotationEdit = [&](QLineEdit *edit) {
		if (edit)
			connect(edit, &QLineEdit::editingFinished, this, &cDockNavigation::slotRotationEdited);
	};
	connectRotationEdit(ui->vect3_camera_rotation_x);
	connectRotationEdit(ui->vect3_camera_rotation_y);
	connectRotationEdit(ui->vect3_camera_rotation_z);

	if (ui->logedit_camera_distance_to_target)
		connect(ui->logedit_camera_distance_to_target, &QLineEdit::editingFinished, this,
			&cDockNavigation::slotCameraDistanceEdited);
	if (ui->comboBox_camera_absolute_distance_mode)
		connect(ui->comboBox_camera_absolute_distance_mode,
			QOverload<int>::of(&QComboBox::currentIndexChanged), this,
			&cDockNavigation::slotMovementStepModeChanged);
	if (ui->comboBox_camera_movement_mode)
		connect(ui->comboBox_camera_movement_mode, QOverload<int>::of(&QComboBox::currentIndexChanged),
			this, &cDockNavigation::slotCameraMovementModeChanged);
	connect(ui->pushButton_undo, &QPushButton::clicked, this, &cDockNavigation::slotUndo);
	connect(ui->pushButton_redo, &QPushButton::clicked, this, &cDockNavigation::slotRedo);
	connect(ui->pushButton_render, &QPushButton::clicked, this, &cDockNavigation::slotStartRender);
	connect(ui->pushButton_stop, &QPushButton::clicked, this, &cDockNavigation::slotStopRender);
	connect(ui->pushButton_reset_view, &QPushButton::clicked, this,
		&cDockNavigation::slotPressedButtonResetView);
	if (ui->comboBox_opencl_mode)
		connect(ui->comboBox_opencl_mode, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
			&cDockNavigation::slotChangedOpenCLMode);
	connect(ui->pushButton_openNavigator, &QPushButton::clicked, this,
		&cDockNavigation::slotOpenNavigator);
}
...
{
	(void)value;
	if (cameraDebounce) cameraDebounce->start();
}
...
void cDockNavigation::SetupDeepZoom()
{
	...
	QLayout *dockLayout = ui->verticalLayout;
	if (!dockLayout) dockLayout = layout();
	if (dockLayout)
...
void cDockNavigation::slotDeepZoomCompute()
{
	...
	// Warn if orbit is too short for useful perturbation
	if (refOrbit.GetLength() <= 2)
	{
		status += "\n\nWARNING: Reference orbit too short (<3 iters).\n"
			"The target point escapes immediately — try zooming\n"
			"closer to the fractal surface before computing.";
	}
	...
	if (deepZoomCheckBox && deepZoomCheckBox->isChecked())
	{
		...
		{
			deep_zoom_integration::Disable();
			status += "\nGPU Integration: DISABLED (orbit too short)";
		}
	}
...
{
	(void)index;
	// Auto-recompute when precision changes and deep zoom is active
	if (deepZoomCheckBox && deepZoomCheckBox->isChecked() && deepZoomManager)
	{
		slotDeepZoomCompute();
	}
}
