		...
		: QWidget(parent), CommonMyWidgetWrapper(this)
{
	...
	connect(comboInterpolationMode, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
		&cGradientEditWidget::interpolationModeChanged);
	...
	connect(sliderMidpointIntensity, &QSlider::valueChanged, this,
		&cGradientEditWidget::midpointIntensityChanged);
	...
	connect(buttonRandomColors, &QPushButton::clicked, this, &cGradientEditWidget::pressedButtonRandomColors);
	connect(buttonRandomColorsAndPositions, &QPushButton::clicked, this,
		&cGradientEditWidget::pressedButtonRandomColorsAndPositions);
	connect(buttonBrightnessInc, &QPushButton::clicked, this, &cGradientEditWidget::pressedButtonBrightnessInc);
	connect(buttonBrightnessDec, &QPushButton::clicked, this, &cGradientEditWidget::pressedButtonBrightnessDec);
	connect(buttonPaletteInvert, &QPushButton::clicked, this, &cGradientEditWidget::pressedButtonInvert);
	connect(buttonSaturationInc, &QPushButton::clicked, this, &cGradientEditWidget::pressedButtonSaturationInc);
	connect(buttonSaturationDec, &QPushButton::clicked, this, &cGradientEditWidget::pressedButtonSaturationDec);
		...
		if (!viewMode)
		{
			...
			else if (pressedColorIndex >= 0 && !mouseDragStarted && pressedMidpointIndex < 0)
			{
				...
				if (buttonUnderMouse == pressedColorIndex)
				{
					...
					const int colorIndex = pressedColorIndex;
					auto *colorDialog = new QColorDialog(this);
					colorDialog->setAttribute(Qt::WA_DeleteOnClose);
					colorDialog->setOption(QColorDialog::DontUseNativeDialog);
					sRGB colorRGB = listOfColors[colorIndex].color;
					QColor color(colorRGB.R, colorRGB.G, colorRGB.B);
					colorDialog->setCurrentColor(color);
					colorDialog->setWindowTitle(
						tr("Edit color #%1").arg(QString::number(colorIndex + 1)));
					connect(colorDialog, &QDialog::accepted, this, [this, colorDialog, colorIndex]() {
						QColor color = colorDialog->currentColor();
						sRGB colorRGB(color.red(), color.green(), color.blue());
						gradient.ModifyColor(colorIndex, colorRGB);

						if (colorIndex == 0) gradient.ModifyColor(1, colorRGB);

						if (colorIndex == 1) gradient.ModifyColor(0, colorRGB);
...
void cGradientEditWidget::mouseDoubleClickEvent(QMouseEvent *event)
{
	...
	auto *dialog = new cGradientEditDialog(
		this, popupMode, this->parentWidget() ? this->parentWidget()->window() : nullptr);
...
{
	auto *dialog = new PreviewFileDialog(this);
	dialog->setAttribute(Qt::WA_DeleteOnClose);
	...
	connect(dialog, &QDialog::accepted, this, [this, dialog]() {
		...
		{
			const int width = imagePalette.width();
			const int height = imagePalette.height();
			const int numberOfColors = gradient.GetNumberOfColors();
			gradient.DeleteAll();

			const float step = 1.0f / numberOfColors;

			for (int i = 0; i < numberOfColors; i++)
			{
				const double angle = double(i) / numberOfColors * M_PI * 2.0;
				const double x = width / 2 + cos(angle) * width * 0.4;
				const double y = height / 2 + sin(angle) * height * 0.4;
				const QRgb pixel = imagePalette.pixel(int(x), int(y));
				const sRGB pixelRGB(qRed(pixel), qGreen(pixel), qBlue(pixel));
...
void cGradientEditWidget::LoadColors()
{
	...
	connect(dialog, &QDialog::accepted, this, [this, dialog]() {
		...
		if (file.open(QIODevice::ReadOnly))
		{
			...
			const bool result = DecodeGradientFromFile(stringLoaded);
			if (!result)
			{
...
void cGradientEditWidget::SaveColors()
{
	...
	connect(dialog, &QDialog::accepted, this, [this, dialog]() {
		...
		{
			const QString string = gradient.GetColorsAsString();
			QTextStream stream(&file);
