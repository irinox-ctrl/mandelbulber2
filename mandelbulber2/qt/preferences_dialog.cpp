...
void cPreferencesDialog::on_pushButton_generate_thumbnail_cache_clicked()
{
	...
	if (reply == QMessageBox::Yes)
	{
		...
		QObject::connect(thumbWidget.get(), &cThumbnailWidget::updateProgressAndStatus,
			gMainInterface->mainWindow,
			[](const QString &text, const QString &progressText, double progress) {
				gMainInterface->mainWindow->slotUpdateProgressAndStatus(text, progressText, progress);
			});
