...
#include "src/parameter_render_fingerprint.hpp"
...
cNavigatorWindow::cNavigatorWindow(QWidget *parent) : QDialog(parent), ui(new Ui::cNavigatorWindow)
{
	...
	connect(renderDebounceTimer, &QTimer::timeout, this, &cNavigatorWindow::slotDebouncedStartRender);

	navigatorQualityRestoreTimer = new QTimer(this);
	navigatorQualityRestoreTimer->setSingleShot(true);
	navigatorQualityRestoreTimer->setInterval(250);
	connect(navigatorQualityRestoreTimer, &QTimer::timeout, this,
		&cNavigatorWindow::slotQualityRestoreAfterNavigation);

	connect(ui->widgetNavigationButtons, &cDockNavigation::signalRender, this,
		&cNavigatorWindow::RequestStartRender);
	connect(
		ui->widgetNavigationButtons, &cDockNavigation::signalStop, this, &cNavigatorWindow::StopRender);
	connect(manipulations, &cManipulations::signalRender, this, &cNavigatorWindow::RequestStartRender);
	...
	connect(ui->comboBox_mouse_click_function, QOverload<int>::of(&QComboBox::currentIndexChanged),
		this, &cNavigatorWindow::slotChangedComboMouseClickFunction);
	...
	std::shared_ptr<cParameterContainer> _params, std::shared_ptr<cFractalContainer> _fractalParams)
{
	...
	*params = *_params;
	...
	connect(ui->comboBox_navigator_preview_size, QOverload<int>::of(&QComboBox::currentIndexChanged),
		this, &cNavigatorWindow::slotChangedPreviewSize);
...
void cNavigatorWindow::SynchronizeInterface(qInterface::enumReadWrite mode)
{
	...
	if (leftWidget)
	{
		...
		{
			const int tabIndex = fractalWidget->GetTabIndex();
			fractalWidget->SynchronizeInterface(params, mode);
			if (mode == qInterface::write)
				fractalWidget->EnsureFormulaWidgetMatchesParams(false);
...
void cNavigatorWindow::RequestStartRender()
{
	renderDebounceTimer->start();
}

void cNavigatorWindow::slotDebouncedStartRender()
{
	StartRender();
}

void cNavigatorWindow::slotQualityRestoreAfterNavigation()
{
	if (!manipulations->isDraggingStarted() && autoRefreshEnabled) RequestStartRender();
}
...
void cNavigatorWindow::StartRender()
{
	...
	autoRefreshLastFingerprint =
		ComputeRenderParameterFingerprint(params.get(), fractalParams.get());
	...
	*tempFractalParams = *fractalParams;
	...
	if (manipulations->isDraggingStarted() && ui->comboBox_navigator_preview_quality->currentIndex() == 0)
	{
		intSizeFactor = qMax(intSizeFactor, 10);
		lastSizefactor = qMax(lastSizefactor, 10.0);
	}
	...
	connect(renderJob, &cRenderJob::updateImage, ui->widgetRenderedImage,
...
void cNavigatorWindow::slotMouseDragFinish()
{
	...
	if (wasAuxOffsetDrag)
		...
		navigatorQualityRestoreTimer->start();
}
...
void cNavigatorWindow::slotPeriodicRefresh()
{
	if (!manipulations->isDraggingStarted() && autoRefreshEnabled)
	{
		...
		if (newFingerprint != autoRefreshLastFingerprint)
		{
			autoRefreshLastFingerprint = newFingerprint;
			stopRequest = true;
			RequestStartRender();
		}
	}
