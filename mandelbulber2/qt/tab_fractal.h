...
public:
	...
	void EnsureFormulaWidgetMatchesParams(bool requestRender = true);
	void ApplyFormulaListIndex(int fractalListIndex, bool requestRender = true);
...
private:
	void ConnectSignals();
	void ClearFormulaLayout();
	QWidget *GetOrCreateFormulaWidget(const QString &cacheKey, const QString &formulaName,
		const QString &uiFilename, int fractalListIndex, bool *created);
	void ShowFormulaWidget(QWidget *widget, int fractalListIndex);
	void ConnectFormulaWidgetSignals(QWidget *widget, int fractalListIndex);

	Ui::cTabFractal *ui;

	int tabIndex;
	int activeFormulaListIndex = -1;
	QWidget *activeFractalWidget = nullptr;
	QHash<QString, QWidget *> formulaWidgetCache;
	QSet<QString> formulaWidgetsConnected;
