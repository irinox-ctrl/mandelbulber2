...
cDockStatistics::cDockStatistics(QWidget *parent) : QWidget(parent), ui(new Ui::cDockStatistics)
{
	...
	validationStatusLabel = new QLabel(this);
	validationStatusLabel->setObjectName("label_validation_status");
	validationStatusLabel->setWordWrap(true);
	validationStatusLabel->setTextInteractionFlags(Qt::TextSelectableByMouse);
	validationStatusLabel->setStyleSheet("color: #88CC88; padding: 4px;");
	if (QVBoxLayout *layout = qobject_cast<QVBoxLayout *>(this->layout()))
		layout->addWidget(validationStatusLabel);
...
void cDockStatistics::UpdateValidationStatus() const
{
	if (!validationStatusLabel) return;

	const sUiParameterRepairResult &repair = GetLastUiParameterRepairResult();
	const sWidgetContractReport &contracts = GetLastWidgetContractReport();

	QStringList statusParts;
	if (!contracts.issues.isEmpty())
		statusParts.append(FormatWidgetContractSummary(contracts));
	if (!repair.issues.isEmpty())
		statusParts.append(FormatValidationIssuesSummary(repair));

	const bool allOk = contracts.issues.isEmpty() && repair.issues.isEmpty();
	validationStatusLabel->setText(allOk
		? QStringLiteral("Validation: %1").arg(QObject::tr("Parameters OK"))
		: QStringLiteral("Validation: %1").arg(statusParts.join(" | ")));

	if (allOk)
	{
		validationStatusLabel->setStyleSheet("color: #88CC88; padding: 4px;");
		validationStatusLabel->setToolTip(QObject::tr("Parameters and widget bindings are valid."));
	}
	else
	{
		validationStatusLabel->setStyleSheet("color: #FFAA44; padding: 4px; font-weight: bold;");
		QStringList tips;
		for (const auto &issue : contracts.issues)
			tips.append(QString("%1: %2").arg(issue.widgetName, issue.message));
		for (const auto &issue : repair.issues)
			tips.append(QString("%1: %2 → %3").arg(issue.parameter, issue.message, issue.autoFix));
		validationStatusLabel->setToolTip(tips.join('\n'));
	}
}
