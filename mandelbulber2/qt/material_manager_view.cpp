		...
		: QWidget(parent), ui(new Ui::cMaterialManagerView)
{
	...
	connect(ui->pushButton_newMaterial, &QPushButton::clicked, this, &cMaterialManagerView::slotAddMaterial);
	connect(ui->pushButton_deleteMaterial, &QPushButton::clicked, this, &cMaterialManagerView::slotDeleteMaterial);
	connect(ui->pushButton_editMaterial, &QPushButton::clicked, this, &cMaterialManagerView::slotEditMaterial);
	connect(ui->pushButton_LoadMaterial, &QPushButton::clicked, this, &cMaterialManagerView::slotLoadMaterial);
	connect(ui->pushButton_SaveMaterial, &QPushButton::clicked, this, &cMaterialManagerView::slotSaveMaterial);
	connect(itemView, &cMaterialItemView::activated, this, &cMaterialManagerView::slotItemSelected);
}
...
void cMaterialManagerView::slotLoadMaterial()
{
	...
	connect(dialog, &QDialog::accepted, this, [this, dialog, parSettings]() mutable {
		const QStringList filenames = dialog->selectedFiles();
		for (int i = 0; i < filenames.size(); i++)
		{
			const QString filename = QDir::toNativeSeparators(filenames[i]);
...
void cMaterialManagerView::slotSaveMaterial()
{
	...
	connect(dialog, &QDialog::accepted, this, [dialog, settingsToSave]() {
		const QStringList filenames = dialog->selectedFiles();
