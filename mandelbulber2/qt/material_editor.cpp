...
#define MATERIAL_UI (static_cast<cMaterialEditorPanel*>(ui->widget_material_editor_panel)->materialPanelUi())
...
cMaterialEditor::cMaterialEditor(QWidget *parent) : QWidget(parent), ui(new Ui::cMaterialEditor)
{
	...
	MATERIAL_UI->colorpalette_diffuse_gradient->SetGrayscale();
	MATERIAL_UI->colorpalette_roughness_gradient->SetGrayscale();

	// MATERIAL_UI->colorpalette_surface_color_gradient->SetViewModeOnly();
...
{
	connect(MATERIAL_UI->comboBox_fractal_coloring_algorithm, QOverload<int>::of(&QComboBox::currentIndexChanged),
		this, &cMaterialEditor::slotChangedComboFractalColoringAlgorithm);
	connect(ui->widget_material_preview, &cMaterialWidget::materialChanged, this,
		&cMaterialEditor::materialChanged);
...
void cMaterialEditor::slotChangedComboFractalColoringAlgorithm(int index) const
{
	...
	MATERIAL_UI->spinbox_fractal_coloring_sphere_radius->setEnabled(selection == fractalColoring_Sphere);
	MATERIAL_UI->vect4_fractal_coloring_line_direction_x->setEnabled(selection == fractalColoring_Line);
	MATERIAL_UI->vect4_fractal_coloring_line_direction_y->setEnabled(selection == fractalColoring_Line);
	MATERIAL_UI->vect4_fractal_coloring_line_direction_z->setEnabled(selection == fractalColoring_Line);
	MATERIAL_UI->vect4_fractal_coloring_line_direction_w->setEnabled(selection == fractalColoring_Line);
}
