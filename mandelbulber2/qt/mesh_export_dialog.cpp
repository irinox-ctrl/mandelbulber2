...
void cMeshExportDialog::on_pushButton_start_render_layers_clicked()
{
	if (!slicerBusy)
	{
		...
		QObject::connect(thread, &QThread::started, meshExport, &cMeshExport::ProcessVolume);
		thread->setObjectName("MeshExport");
		thread->start();
		QObject::connect(meshExport, &cMeshExport::finished, this, &cMeshExportDialog::slotSlicerFinished);
		QObject::connect(meshExport, &cMeshExport::finished, meshExport, &QObject::deleteLater);
		QObject::connect(meshExport, &cMeshExport::finished, thread, &QThread::quit);
