...
void cAudioSelector::slotLoadAudioFile()
{
	...
	connect(dialog, &QDialog::accepted, this, [this, dialog]() {
		...
		connect(audio.get(), &cAudioTrack::loadingFinished, this, &cAudioSelector::slotAudioLoaded);
...
{
	connect(ui->pushButton_loadAudioFile, &QPushButton::clicked, this, &cAudioSelector::slotLoadAudioFile);
	connect(ui->pushButton_delete_audiotrack, &QPushButton::clicked, this, &cAudioSelector::slotDeleteAudioTrack);
	connect(ui->spinbox_animsound_bandwidth, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
		&cAudioSelector::slotFreqChanged);
	connect(ui->spinbox_animsound_midfreq, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
		&cAudioSelector::slotFreqChanged);
	connect(ui->checkBox_animsound_pitchmode, &QCheckBox::stateChanged, this, &cAudioSelector::slotFreqChanged);
	connect(ui->groupCheck_animsound_decayfilter, &QGroupBox::toggled, this, &cAudioSelector::slotFreqChanged);
	connect(ui->groupCheck_animsound_smoothfilter, &QGroupBox::toggled, this, &cAudioSelector::slotFreqChanged);
	connect(ui->groupCheck_animsound_binaryfilter, &QGroupBox::toggled, this, &cAudioSelector::slotFreqChanged);
	connect(ui->spinbox_animsound_decaystrength, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
		&cAudioSelector::slotFreqChanged);
	connect(ui->spinbox_animsound_smoothstrength, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
		&cAudioSelector::slotFreqChanged);
	connect(ui->spinbox_animsound_binarythresh, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
		&cAudioSelector::slotFreqChanged);
	connect(ui->spinbox_animsound_binarylength, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
		&cAudioSelector::slotFreqChanged);
	connect(this, &cAudioSelector::frequencyChanged, ui->fft, &cFFTView::slotFreqChanged);
	connect(ui->pushButton_playback_start, &QPushButton::clicked, this, &cAudioSelector::slotPlaybackStart);
	connect(ui->pushButton_playback_stop, &QPushButton::clicked, this, &cAudioSelector::slotPlaybackStop);
	connect(this, &cAudioSelector::loadingProgress, ui->waveForm, &cWaveFormView::slotLoadingProgress);
	connect(this, &cAudioSelector::playPositionChanged, ui->animAudioView,
		[view = ui->animAudioView](qint64 position) {
			QMetaObject::invokeMethod(view, "positionChanged", Q_ARG(qint64, position));
		});
	connect(ui->audio_position_slider, &QSlider::sliderMoved, this, &cAudioSelector::slotSeekTo);
	connect(ui->fft, &cFFTView::newFrequencySelected, this, &cAudioSelector::slotChangedFrequencyBand);
	connect(ui->spinboxInt_animsound_sounddelay, QOverload<int>::of(&QSpinBox::valueChanged), this,
		&cAudioSelector::slotUpdateSoundDelay);
}
...
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
	connect(audioOutput.get(), &QAudioOutput::notify, this, [this]() { slotPlayPositionChanged(); });
#else
	connect(&timer, &QTimer::timeout, this, &cAudioSelector::slotTimerTimeout);
	timer.start(200);
#endif

	connect(audioOutput.get(), &QAudioOutput::stateChanged, this, &cAudioSelector::slotPlaybackStateChanged);
...
void cAudioSelector::AssignAnimation(std::shared_ptr<cAnimationFrames> _animationFrames)
{
	...
	{
		audio = animationFrames->GetAudioPtr(parameterName);
		connect(audio.get(), &cAudioTrack::loadingProgress, this, &cAudioSelector::loadingProgress);
		connect(audio.get(), &cAudioTrack::loadingFailed, ui->waveForm, &cWaveFormView::slotLoadingFailed);
