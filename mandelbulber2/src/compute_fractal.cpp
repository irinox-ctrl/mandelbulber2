...
namespace
{
struct sFoldTransformResult
{
	CVector4 z;
	double deScale = 1.0;
};

sFoldTransformResult ApplyFoldTransform(CVector4 z, enumMutationFoldType foldType, double foldLimit,
	double foldValue, int kaleidoscopeSides, int iteration)
{
	sFoldTransformResult result;
	result.z = z;

	switch (foldType)
	{
		case mutFoldBox:
			if (fabs(z.x) > foldLimit) result.z.x = sign(z.x) * foldValue - z.x;
			if (fabs(z.y) > foldLimit) result.z.y = sign(z.y) * foldValue - z.y;
			if (fabs(z.z) > foldLimit) result.z.z = sign(z.z) * foldValue - z.z;
			break;
		case mutFoldSphere:
		{
			double rr = z.x * z.x + z.y * z.y + z.z * z.z;
			double minR2 = foldLimit * foldLimit;
			double fixR2 = foldValue * foldValue;
			if (rr < minR2)
			{
				result.z *= fixR2 / minR2;
				result.deScale *= fixR2 / minR2;
			}
			else if (rr < fixR2)
			{
				result.z *= fixR2 / rr;
				result.deScale *= fixR2 / rr;
			}
			break;
		}
		case mutFoldMenger:
		{
			...
			result.z.y = fabs(z.y);
			result.z.z = fabs(z.z);
			if (result.z.x - result.z.y < 0)
			{
				double t = result.z.y;
				result.z.y = result.z.x;
				result.z.x = t;
			}
			if (result.z.x - result.z.z < 0)
			{
				double t = result.z.z;
				result.z.z = result.z.x;
				result.z.x = t;
			}
			if (result.z.y - result.z.z < 0)
			{
				double t = result.z.z;
				result.z.z = result.z.y;
				result.z.y = t;
			}
			double s = foldValue;
			result.z.x = result.z.x * s - foldLimit * (s - 1.0);
			result.z.y = result.z.y * s - foldLimit * (s - 1.0);
			result.z.z = result.z.z * s;
			if (result.z.z > 0.5 * foldLimit * (s - 1.0)) result.z.z -= foldLimit * (s - 1.0);
			result.deScale *= s;
			break;
		}
		case mutFoldSierpinski:
		{
			...
			double s = foldValue;
			result.z *= s;
			result.z.x -= foldLimit * (s - 1.0);
			result.z.y -= foldLimit * (s - 1.0);
			result.z.z -= foldLimit * (s - 1.0);
			result.deScale *= s;
			break;
		}
		case mutFoldAbs:
			result.z.x = fabs(z.x + foldLimit) - fabs(z.x - foldLimit) - z.x;
			result.z.y = fabs(z.y + foldLimit) - fabs(z.y - foldLimit) - z.y;
			result.z.z = fabs(z.z + foldLimit) - fabs(z.z - foldLimit) - z.z;
			break;
		case mutFoldKaleidoscope:
		{
			if (kaleidoscopeSides >= 3)
			{
				double angle = M_PI / kaleidoscopeSides;
				double pAngle = atan2(z.y, z.x);
				double r = sqrt(z.x * z.x + z.y * z.y);
				pAngle = fmod(pAngle + angle, 2.0 * angle) - angle;
				result.z.x = r * cos(pAngle);
				result.z.y = r * sin(pAngle);
				result.z.y = fabs(result.z.y);
			}
			break;
		}
		case mutFoldOctahedral:
			...
			if (result.z.x - result.z.y < 0)
			{
				double tx = result.z.y;
				result.z.y = result.z.x;
				result.z.x = tx;
			}
			if (result.z.x - result.z.z < 0)
			{
				double tx = result.z.z;
				result.z.z = result.z.x;
				result.z.x = tx;
			}
			break;
		case mutFoldSmooth:
		{
			double k = (foldLimit > 0.0) ? foldLimit : 1.0;
			result.z.x -= tanh(k * z.x);
			result.z.y -= tanh(k * z.y);
			result.z.z -= tanh(k * z.z);
			break;
		}
		case mutFoldPolynomial:
			result.z.x = z.x * z.x * z.x - 3.0 * z.x;
			result.z.y = z.y * z.y * z.y - 3.0 * z.y;
			result.z.z = z.z * z.z * z.z - 3.0 * z.z;
			break;
		case mutFoldCircular:
		{
			double rad = (foldValue > 0.0) ? foldValue : 1.0;
		...
		{
			double r = sqrt(z.x * z.x + z.y * z.y);
			double angle = r * foldLimit;
			double ca = cos(angle);
			double sa = sin(angle);
			result.z.x = z.x * ca - z.y * sa;
			result.z.y = z.x * sa + z.y * ca;
			break;
		}
		case mutFoldSinusoidal:
		{
			double a = foldValue;
			double b = (foldLimit > 0.0) ? foldLimit : 1.0;
			result.z.x = z.x + a * sin(b * z.x);
			result.z.y = z.y + a * sin(b * z.y);
			result.z.z = z.z + a * sin(b * z.z);
			break;
		}
		case mutFoldExponential:
			result.z.x = z.x * exp(-z.x * z.x);
			result.z.y = z.y * exp(-z.y * z.y);
			result.z.z = z.z * exp(-z.z * z.z);
			break;
		case mutFoldLogarithmic:
		{
			...
			double ry = fabs(z.y);
			double rz = fabs(z.z);
			result.z.x = z.x * log(1.0 + rx);
			result.z.y = z.y * log(1.0 + ry);
			result.z.z = z.z * log(1.0 + rz);
			break;
		}
		case mutFoldPower:
		{
			double p = (foldValue > 0.0) ? foldValue : 2.0;
			double ax = fabs(z.x);
			double ay = fabs(z.y);
			double az = fabs(z.z);
			result.z.x = (z.x >= 0 ? 1.0 : -1.0) * pow(max(ax, 1e-21), p);
			result.z.y = (z.y >= 0 ? 1.0 : -1.0) * pow(max(ay, 1e-21), p);
			result.z.z = (z.z >= 0 ? 1.0 : -1.0) * pow(max(az, 1e-21), p);
			break;
		}
		case mutFoldSmoothAbs:
		{
			double k = (foldLimit > 0.0) ? foldLimit : 5.0;
			result.z.x = z.x * tanh(k * z.x);
			result.z.y = z.y * tanh(k * z.y);
			result.z.z = z.z * tanh(k * z.z);
			break;
		}
		case mutFoldModuloWrap:
		{
			double period = (foldValue > 0.0) ? foldValue : 2.0;
			result.z.x = fmod(z.x + period * 0.5, period) - period * 0.5;
			result.z.y = fmod(z.y + period * 0.5, period) - period * 0.5;
			result.z.z = fmod(z.z + period * 0.5, period) - period * 0.5;
			break;
		}
		case mutFoldNestedAbs:
		{
			double c = (foldLimit > 0.0) ? foldLimit : 1.0;
			result.z.x = fabs(fabs(z.x) - c);
			result.z.y = fabs(fabs(z.y) - c);
			result.z.z = fabs(fabs(z.z) - c);
			break;
		}
		case mutFoldSawtooth:
			result.z.x = 2.0 * (z.x * 0.5 - floor(z.x * 0.5 + 0.5));
			result.z.y = 2.0 * (z.y * 0.5 - floor(z.y * 0.5 + 0.5));
			result.z.z = 2.0 * (z.z * 0.5 - floor(z.z * 0.5 + 0.5));
			break;
		case mutFoldBipolar:
		{
			double c1 = foldLimit;
			double c2 = -foldLimit;
			result.z.x = fabs(z.x - c1) - fabs(z.x - c2);
			result.z.y = fabs(z.y - c1) - fabs(z.y - c2);
			result.z.z = fabs(z.z - c1) - fabs(z.z - c2);
			break;
		}
		case mutFoldRadialBox:
		{
			double rOld = sqrt(z.x * z.x + z.y * z.y);
			double theta = atan2(z.y, z.x);
			double foldR = foldLimit;
			double r = rOld;
			if (r > foldR) r = 2.0 * foldR - r;
			if (r < -foldR) r = -2.0 * foldR - r;
			result.z.x = r * cos(theta);
			result.z.y = r * sin(theta);
			result.deScale *= r / max(rOld, 1e-21);
			break;
		}
		case mutFoldShear:
		{
			double lim = foldLimit;
			result.z = z;
			if (result.z.x > lim)
				result.z.x = 2.0 * lim - result.z.x;
			else if (result.z.x < -lim)
				result.z.x = -2.0 * lim - result.z.x;
			if (result.z.y > lim)
				result.z.y = 2.0 * lim - result.z.y;
			else if (result.z.y < -lim)
				result.z.y = -2.0 * lim - result.z.y;
			result.z.x += foldValue * result.z.y;
			result.z.y += foldValue * result.z.z;
			break;
		}
		case mutFold3DCross:
		{
			...
			if (phase == 0)
			{
				if (result.z.x > lim) result.z.x = 2.0 * lim - result.z.x;
				if (result.z.x < -lim) result.z.x = -2.0 * lim - result.z.x;
				if (result.z.y > lim) result.z.y = 2.0 * lim - result.z.y;
				if (result.z.y < -lim) result.z.y = -2.0 * lim - result.z.y;
			}
			else if (phase == 1)
			{
				if (result.z.y > lim) result.z.y = 2.0 * lim - result.z.y;
				if (result.z.y < -lim) result.z.y = -2.0 * lim - result.z.y;
				if (result.z.z > lim) result.z.z = 2.0 * lim - result.z.z;
				if (result.z.z < -lim) result.z.z = -2.0 * lim - result.z.z;
			}
			else
			{
		...
		case mutFoldConformal:
		{
			...
			if (r2 > 1e-21)
			{
				result.z.x = z.x + z.x / r2;
				result.z.y = z.y + z.y / r2;
				result.z.z = z.z + z.z / r2;
				result.deScale *= fabs(1.0 - 1.0 / r2);
			}
			break;
		}
		...
		case mutFoldScalePulse:
		{
			...
			if (result.z.z > lim) result.z.z = 2.0 * lim - result.z.z;
			if (result.z.z < -lim) result.z.z = -2.0 * lim - result.z.z;
			double pulse = 1.0 + 0.2 * sin((double)iteration * foldValue);
			result.z *= pulse;
			result.deScale *= fabs(pulse);
			break;
		}
		case mutFoldTriangleWave:
			result.z.x = 2.0 * fabs(z.x * 0.5 - floor(z.x * 0.5 + 0.5));
			result.z.y = 2.0 * fabs(z.y * 0.5 - floor(z.y * 0.5 + 0.5));
			result.z.z = 2.0 * fabs(z.z * 0.5 - floor(z.z * 0.5 + 0.5));
			break;
		case mutFoldNone:
		default: break;
	}
	...
	const sFractalIn &in, sFractalOut *out)
{
	...
	{
		const int globalCap = fractals.GetMaxN() * in.maxiterMultiplier;
	...
	// v7.5 — Julia start mode (z₀ override)
	{
		...
		if (mut0.enabled && mut0.juliaStart != mutJuliaStartRay)
		{
			...
			switch (mut0.juliaStart)
			{
				...
				case mutJuliaStartInversion:
				{
					double r2 = c.Dot(c);
					if (r2 > 1e-21) z = c * (1.0 / r2);
					else z = c;
					break;
				}
				case mutJuliaStartSpherical:
				{
					double radius = mut0.juliaCRadius != 0.0 ? mut0.juliaCRadius : 1.0;
					double cLen = c.Length();
					if (cLen > 1e-21) z = c * (radius / cLen);
					else z = CVector4(radius, 0.0, 0.0, 0.0);
					break;
				}
				case mutJuliaStartBipolar:
				{
					if (z.x >= 0.0) z = c;
					else z = CVector4(mut0.juliaBipolarCRx, mut0.juliaBipolarCRy, mut0.juliaBipolarCRz, 0.0);
					break;
				}
				case mutJuliaStartLattice:
				{
					double grid = mut0.juliaCRadius != 0.0 ? mut0.juliaCRadius : 1.0;
					z.x = floor(c.x / grid + 0.5) * grid;
					z.y = floor(c.y / grid + 0.5) * grid;
					z.z = floor(c.z / grid + 0.5) * grid;
					break;
				}
				case mutJuliaStartPerturb:
				{
					double eps = mut0.juliaStartEpsilon != 0.0 ? mut0.juliaStartEpsilon : 0.001;
					z = z + c * eps;
					break;
				}
				default: break;
	...
	// Cinema4D-style accumulator for calcModeCubeOrbitTrap
	CVector3 accumZ(0.0, 0.0, 0.0);
	double accumWeight = 0.0;
	double closestSdf = 1e10;
	CVector3 closestZ(0.0, 0.0, 0.0);
	int closestIter = 0;
	...
	for (i = 0; i < maxN; i++)
	{
		...
		if (!fractals.IsHybrid() || effectiveWeight > 0.0)
		{
			...
			if (mutationActive)
			{
				...
				if (mut.adaptiveStrength)
				{
					const double orbitMag = z.Length();
					if (orbitMag > mut.adaptiveThreshold)
					{
						adaptiveFactor =
							std::max(0.01, std::min(1.0, mut.adaptiveThreshold / orbitMag));
					}
				}
				const double effectiveMathP1 = mut.mathP1 * adaptiveFactor;
				const double effectiveMathP2 = mut.mathP2 * adaptiveFactor;
				const double effectiveWarpAmp = mut.warpAmplitude * adaptiveFactor;
				...
				if (mut.inversionEnabled && i >= mut.invIterStart && i < mut.invIterStop && mut.inversionType != 0)
				{
					...
					double rr;
					CVector4 z_orig_inv = z; double de_orig_inv = aux.DE;
					...
					z = CVector4(zz, z.w);
					aux.DE *= fabs(mde);
					double inv_blend = std::min(std::max(mut.inversionWeight * mut.masterWeight, 0.0), 1.0);
				...
				{
					const sFoldTransformResult foldResult = ApplyFoldTransform(z, mut.foldType,
						mut.foldLimit, mut.foldValue, mut.kaleidoscopeSides, aux.i);
					z = foldResult.z;
					aux.DE *= foldResult.deScale;
				}
				...
				{
					z.x += effectiveWarpAmp * sin(z.y * mut.warpFrequency);
					z.y += effectiveWarpAmp * sin(z.z * mut.warpFrequency);
					z.z += effectiveWarpAmp * sin(z.x * mut.warpFrequency);
				}
				else if (mut.warpType == mutWarpTwist)
				{
					double angle = z.z * mut.warpFrequency * M_PI / 180.0;
					double ca = cos(angle * effectiveWarpAmp);
					double sa = sin(angle * effectiveWarpAmp);
				...
				else if (mut.warpType == mutWarpRadial)
				{
					...
					if (r > 1e-21)
					{
						double warp = effectiveWarpAmp * sin(r * mut.warpFrequency);
						z.x += z.x / r * warp;
						z.y += z.y / r * warp;
						z.z += z.z / r * warp;
					}
				...
				else if (mut.warpType == mutWarpCylindrical)
				{
					...
					if (r > 1e-21)
					{
						...
						angle += effectiveWarpAmp * sin(z.z * mut.warpFrequency);
						z.x = r * cos(angle);
						z.y = r * sin(angle);
					}
				}
				else if (mut.warpType == mutWarpSphericalInversion)
				{
					...
					if (rr > 1e-21)
					{
						...
						factor = 1.0 + (factor - 1.0) * effectiveWarpAmp;
						z *= factor;
						aux.DE *= fabs(factor);
					}
				}
				else if (mut.warpType == mutWarpMobius)
				{
					...
					if (r2 > 1e-21)
					{
						double angle = effectiveWarpAmp * mut.warpFrequency / (r2 + 1.0);
						double ca = cos(angle);
						double sa = sin(angle);
						double nx = z.x * ca - z.y * sa;
						double ny = z.x * sa + z.y * ca;
				...
				else if (mut.warpType == mutWarpIFSRotation)
				{
					...
					double angle = goldenAngle * aux.i * effectiveWarpAmp;
					...
					&& mut.mathType != mutMathNone)
				{
					...
					switch (mut.mathType)
					{
						...
						{
							double freq = effectiveMathP1;
							double amp = effectiveMathP2;
						...
						case mutMathExpMap:
						{
							...
							if (r > 1e-21)
							{
								double er = exp(effectiveMathP1 * log(r + 1.0));
								double theta = atan2(sqrt(z.x * z.x + z.y * z.y), z.z);
								double phi = atan2(z.y, z.x);
								mathZ.x = er * sin(theta + effectiveMathP2) * cos(phi + mut.mathP3);
								mathZ.y = er * sin(theta + effectiveMathP2) * sin(phi + mut.mathP3);
								mathZ.z = er * cos(theta + effectiveMathP2);
							}
							break;
						}
						case mutMathLogSpiral:
						{
							...
							if (r > 1e-21)
							{
								double lr = log(r + 1e-21) * effectiveMathP1;
								double theta = atan2(sqrt(z.x * z.x + z.y * z.y), z.z);
								double phi = atan2(z.y, z.x);
								double spiralAngle = phi + lr * effectiveMathP2;
								double newR = exp(lr);
						...
						case mutMathPowerN:
						{
							...
							if (r > 1e-21)
							{
								double n = effectiveMathP1;
								double theta = acos(z.z / r);
								double phi = atan2(z.y, z.x);
								double rn = pow(r, n);
								double nTheta = n * theta + effectiveMathP2;
						...
						{
							double cr = effectiveMathP1;
							double ci = effectiveMathP2;
						...
						{
							double qr = effectiveMathP1, qi = effectiveMathP2;
						...
						{
							double a = effectiveMathP1, b = effectiveMathP2;
						...
						{
							double angle = effectiveMathP1 * M_PI / 180.0;
							double scale = effectiveMathP2 != 0.0 ? effectiveMathP2 : 1.0;
							double ca = cos(angle);
							double sa = sin(angle);
						...
						{
							// T(x) = c + r²*(x-c)/|x-c|²
							double cx = effectiveMathP1, cy = effectiveMathP2, cz = mut.mathP3;
						...
						case mutMathLoxodromic:
						{
							...
							double s = effectiveMathP1 != 0.0 ? effectiveMathP1 : 1.0;
							double theta = effectiveMathP2 * M_PI / 180.0;
						...
						case mutMathParabolic:
						{
							...
							double a = effectiveMathP1, b = effectiveMathP2, c = mut.mathP3;
						...
						case mutMathSchottkyDual:
						{
							...
							double c1x = effectiveMathP1, c1y = effectiveMathP2, c1z = mut.mathP3;
						...
						case mutMathFibonacciWord:
						{
							...
							if (useT1)
							{
								...
								double angle = effectiveMathP1 * M_PI / 180.0;
								double sc = effectiveMathP2 != 0.0 ? effectiveMathP2 : 1.0;
						...
						case mutMathMaskitBend:
						{
							...
							double muRe = effectiveMathP1, muIm = effectiveMathP2;
						...
						case mutMathEllipsoidInversion:
						{
							...
							double ax = effectiveMathP1 != 0.0 ? effectiveMathP1 : 1.0;
							double ay = effectiveMathP2 != 0.0 ? effectiveMathP2 : 1.0;
						...
						case mutMathTorusInversion:
						{
							...
							double R = effectiveMathP1 != 0.0 ? effectiveMathP1 : 2.0; // major radius
							double r = effectiveMathP2 != 0.0 ? effectiveMathP2 : 1.0; // minor radius
							double rxy = sqrt(z.x*z.x + z.y*z.y);
							if (rxy > 1e-21)
							{
						...
						case mutMathQuatJuliaKleinian:
						{
							...
							double nj = 2.0*qr*qj;
							double nk = 2.0*qr*qk;
							mathZ.x = ni + effectiveMathP1;
							mathZ.y = nj + effectiveMathP2;
						...
						case mutMathPoincareBall:
						{
							...
							double r2 = z.x*z.x + z.y*z.y + z.z*z.z;
							double curvature = effectiveMathP1 != 0.0 ? effectiveMathP1 : 1.0;
							if (r2 < 1.0 - 1e-12)
							{
								// Poincaré metric factor: 2/(1-|z|²)
								double metricFactor = 2.0 / (1.0 - r2);
								double hypR = curvature * log((1.0 + sqrt(r2)) / (1.0 - sqrt(r2)));
								double scale = tanh(hypR * effectiveMathP2) / (sqrt(r2) + 1e-21);
								mathZ.x = z.x * scale;
								mathZ.y = z.y * scale;
								mathZ.z = z.z * scale;
						...
						case mutMathLorentzBoost:
						{
							...
							double vx = effectiveMathP1, vy = effectiveMathP2, vz = mut.mathP3;
							double v = mut.mathP4;
						...
						{
							double r = sqrt(z.x*z.x + z.y*z.y + z.z*z.z);
							double harmonic = (r > 1e-21) ? effectiveMathP1 / r : 0.0;
							double radial = effectiveMathP2 * r;
						...
						case mutMathFractionalPower:
						{
							...
							double p = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 2.718281828;
							double r = sqrt(z.x*z.x + z.y*z.y + z.z*z.z);
							if (r > 1e-21)
							{
								double theta = acos(z.z / r);
						...
						case mutMathAnisotropePower:
						{
							...
							double px = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 2.0;
							double py = (effectiveMathP2 != 0.0) ? effectiveMathP2 : 2.0;
						...
						case mutMathHyperbolicTrigPower:
						{
							...
							double p = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 2.0;
							double r = sqrt(z.x*z.x + z.y*z.y + z.z*z.z);
							if (r > 1e-21)
							{
								double theta = acos(z.z / r);
						...
						{
							double amp = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 0.1;
							double freq = (effectiveMathP2 != 0.0) ? effectiveMathP2 : 10.0;
							double r = sqrt(z.x*z.x + z.y*z.y + z.z*z.z);
							if (r > 1e-21)
							{
								double theta = acos(z.z / r);
						...
						case mutMathDualQuaternion:
						{
							...
							double q1w = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 1.0;
							double q1i = effectiveMathP2; double q1j = mut.mathP3; double q1k = mut.mathP4;
						...
						case mutMathOctonionPower:
						{
							...
							double p = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 2.0;
							double r = sqrt(z.x*z.x + z.y*z.y + z.z*z.z
								+ effectiveMathP2*effectiveMathP2 + mut.mathP3*mut.mathP3);
							if (r > 1e-21)
							{
								double theta = acos(z.z / r);
								double phi = atan2(z.y, z.x);
						...
						case mutMathQuaternionMobius:
						{
							...
							double a = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 1.0;
							double b = effectiveMathP2;
							double c = mut.mathP3;
						...
						case mutMathFordCircles:
						{
							...
							int N = (effectiveMathP1 > 0.5) ? (int)effectiveMathP1 : 3;
							double r2 = z.x*z.x + z.y*z.y + z.z*z.z;
							if (r2 > 1e-21)
							{
								for (int n = 1; n <= N; n++)
						...
						case mutMathApollonianNet:
						{
							...
							double rad = (effectiveMathP1 > 0.0) ? effectiveMathP1 : 1.0;
							CVector4 centers[4];
						...
						case mutMathConformalWedge:
						{
							...
							double alpha = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 2.0;
							double r = sqrt(z.x*z.x + z.y*z.y + z.z*z.z);
							if (r > 1e-21)
							{
								double theta = acos(z.z / r);
						...
						case mutMathCircleInvChain:
						{
							...
							double r1 = (effectiveMathP1 > 0.0) ? effectiveMathP1 : 1.0;
							double r2c = (effectiveMathP2 > 0.0) ? effectiveMathP2 : 0.8;
						...
						case mutMathHyperbolicSine:
						{
							...
							double amp = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 1.0;
						...
						case mutMathBesselApprox:
						{
							...
							double freq = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 1.0;
							double r = sqrt(z.x*z.x + z.y*z.y + z.z*z.z);
							if (r > 1e-21)
							{
						...
						case mutMathErrorFunction:
						{
							...
							double scale = (effectiveMathP1 != 0.0) ? effectiveMathP1 : 1.0;
				...
				&& i >= mut.juliaIterStart && i < mut.juliaIterStop)
			{
				...
				switch (mut.juliaCTransform)
				{
					...
					case mutJuliaCLogarithmic:
					{
						double cLen = juliaC.Length();
						if (cLen > 1e-21)
							juliaC = juliaC * (log(cLen) / cLen);
						break;
					}
					case mutJuliaCExponential:
					{
						juliaC.x = exp(juliaC.x);
						juliaC.y = exp(juliaC.y);
						juliaC.z = exp(juliaC.z);
						break;
					}
					case mutJuliaCInversion:
					{
						double r2 = juliaC.Dot(juliaC);
						if (r2 > 1e-21) juliaC = juliaC * (1.0 / r2);
						break;
					}
					case mutJuliaCBoxFold:
					{
						double lim = mut.juliaBoxFoldLimit != 0.0 ? mut.juliaBoxFoldLimit : 1.0;
						if (juliaC.x > lim) juliaC.x = 2.0 * lim - juliaC.x;
						else if (juliaC.x < -lim) juliaC.x = -2.0 * lim - juliaC.x;
						if (juliaC.y > lim) juliaC.y = 2.0 * lim - juliaC.y;
						else if (juliaC.y < -lim) juliaC.y = -2.0 * lim - juliaC.y;
						if (juliaC.z > lim) juliaC.z = 2.0 * lim - juliaC.z;
						else if (juliaC.z < -lim) juliaC.z = -2.0 * lim - juliaC.z;
						break;
					}
					case mutJuliaCSpiral:
					{
						double angle = mut.juliaSpiralAngle * M_PI / 180.0;
						double cs = cos(angle), sn = sin(angle);
						double nx = juliaC.x * cs - juliaC.y * sn;
						double ny = juliaC.x * sn + juliaC.y * cs;
						juliaC.x = nx; juliaC.y = ny;
						break;
					}
					case mutJuliaCMandelbrot:
					{
						double cx = juliaC.x, cy = juliaC.y, cz = juliaC.z;
						juliaC.x = cx*cx - cy*cy + aux.const_c.x * mut.juliaCMul;
						juliaC.y = 2.0*cx*cy + aux.const_c.y * mut.juliaCMul;
						juliaC.z = cz + aux.const_c.z * mut.juliaCMul;
						break;
					}
					default: break;
				}
				// Blend C-transform by strength
				if (mut.juliaCTransformStrength != 1.0)
					juliaC = juliaCPre + (juliaC - juliaCPre) * mut.juliaCTransformStrength;
				} // end C-transform iter range check

				// Dynamic modulation (with per-subsystem iter range + strength)
				if (mut.juliaDynamic != mutJuliaDynNone
					&& i >= mut.juliaDynamicIterStart && i < mut.juliaDynamicIterStop)
				{
				CVector4 juliaCPreDyn = juliaC;
				switch (mut.juliaDynamic)
				{
					...
					case mutJuliaDynOrbitMemory:
						...
						break;
					case mutJuliaDynExpDecay:
					{
						double decay = mut.juliaDecayRate != 0.0 ? mut.juliaDecayRate : 0.1;
						juliaC *= exp(-decay * (double)i);
						break;
					}
					case mutJuliaDynSaw:
					{
						double freq = mut.juliaPulseFreq != 0.0 ? mut.juliaPulseFreq : 0.5;
						double t = (double)i * freq;
						juliaC *= (t - floor(t));
						break;
					}
					case mutJuliaDynSmoothStep:
					{
						double range = (double)(mut.juliaIterStop - mut.juliaIterStart);
						if (range > 0.0) {
							double t = ((double)i - mut.juliaIterStart) / range;
							t = t * t * (3.0 - 2.0 * t);
							juliaC *= t;
						}
						break;
					}
					case mutJuliaDynChaos:
					{
						double r = mut.juliaChaosR != 0.0 ? mut.juliaChaosR : 3.8;
						double x = fmod(fabs(juliaC.Length()) * 0.1, 1.0);
						x = r * x * (1.0 - x);
						juliaC *= x;
						break;
					}
					case mutJuliaDynSpiral:
					{
						double angle = mut.juliaSpiralAngle * M_PI / 180.0 * (double)i;
						double cs = cos(angle), sn = sin(angle);
						double nx = juliaC.x * cs - juliaC.y * sn;
						double ny = juliaC.x * sn + juliaC.y * cs;
						juliaC.x = nx; juliaC.y = ny;
					...
					case mutJuliaDynBounce:
					{
						double freq = mut.juliaPulseFreq != 0.0 ? mut.juliaPulseFreq : 1.0;
						juliaC *= fabs(sin((double)i * freq));
						break;
					}
					default: break;
				}
				// Blend dynamic by strength
				if (mut.juliaDynamicStrength != 1.0)
					juliaC = juliaCPreDyn + (juliaC - juliaCPreDyn) * mut.juliaDynamicStrength;
				} // end dynamic iter range check

				// Multi-C modes (with per-subsystem iter range + strength)
				if (mut.juliaMulti != mutJuliaMultiNone
					&& i >= mut.juliaMultiIterStart && i < mut.juliaMultiIterStop)
				{
				CVector4 juliaCPreMulti = juliaC;
				switch (mut.juliaMulti)
				{
					...
					case mutJuliaMultiRecursive:
						...
						break;
					case mutJuliaMultiOctave:
					{
						double n1 = sin(z.x * mut.juliaNoiseFreq) * mut.juliaNoiseAmp;
						double n2 = sin(z.y * mut.juliaNoiseFreq * 2.0) * mut.juliaNoiseAmp * 0.5;
						double n3 = sin(z.z * mut.juliaNoiseFreq * 4.0) * mut.juliaNoiseAmp * 0.25;
						juliaC += CVector4(n1 + n2 + n3, n2 + n3 + n1 * 0.5, n3 + n1 * 0.25 + n2 * 0.5, 0.0);
						break;
					}
					case mutJuliaMultiPhaseShift:
					{
						double phase = (double)i * 2.0 * M_PI / 3.0;
						juliaC += CVector4(mut.juliaFourierC2x, mut.juliaFourierC2y, mut.juliaFourierC2z, 0.0) * sin(phase)
								+ CVector4(mut.juliaFourierC3x, mut.juliaFourierC3y, mut.juliaFourierC3z, 0.0) * cos(phase);
						break;
					}
					case mutJuliaMultiOrbitSplit:
					{
						int quadrant = (z.x >= 0.0 ? 1 : 0) + (z.y >= 0.0 ? 2 : 0);
						if (quadrant == 1) juliaC = CVector4(mut.juliaBipolarCRx, mut.juliaBipolarCRy, mut.juliaBipolarCRz, 0.0);
						else if (quadrant == 2) juliaC = CVector4(mut.juliaFourierC2x, mut.juliaFourierC2y, mut.juliaFourierC2z, 0.0);
						else if (quadrant == 3) juliaC = CVector4(mut.juliaFourierC3x, mut.juliaFourierC3y, mut.juliaFourierC3z, 0.0);
						break;
					}
					case mutJuliaMultiIterModulo:
					{
						int phase = i % 3;
						if (phase == 1) juliaC = CVector4(mut.juliaFourierC2x, mut.juliaFourierC2y, mut.juliaFourierC2z, 0.0);
						else if (phase == 2) juliaC = CVector4(mut.juliaFourierC3x, mut.juliaFourierC3y, mut.juliaFourierC3z, 0.0);
						break;
					}
					case mutJuliaMultiDistBand:
					{
						double d = z.Length();
						double mid = mut.juliaDistBandMid != 0.0 ? mut.juliaDistBandMid : 1.0;
						double far = mut.juliaDistBandFar != 0.0 ? mut.juliaDistBandFar : 2.0;
						if (d > far) juliaC = CVector4(mut.juliaFourierC3x, mut.juliaFourierC3y, mut.juliaFourierC3z, 0.0);
						else if (d > mid) juliaC = CVector4(mut.juliaFourierC2x, mut.juliaFourierC2y, mut.juliaFourierC2z, 0.0);
						break;
					}
					case mutJuliaMultiRandom:
					{
						double seed = sin((double)i * 12.9898 + z.x * 78.233) * 43758.5453;
						seed = seed - floor(seed);
						if (seed > 0.66) juliaC = CVector4(mut.juliaFourierC3x, mut.juliaFourierC3y, mut.juliaFourierC3z, 0.0);
						else if (seed > 0.33) juliaC = CVector4(mut.juliaFourierC2x, mut.juliaFourierC2y, mut.juliaFourierC2z, 0.0);
						break;
					}
					default: break;
				}
				// Blend multi-C by strength
				if (mut.juliaMultiStrength != 1.0)
					juliaC = juliaCPreMulti + (juliaC - juliaCPreMulti) * mut.juliaMultiStrength;
				} // end multi-C iter range check

				// Apply injection strength
				if (mut.juliaInjectionStrength != 1.0) juliaC *= mut.juliaInjectionStrength;
				...
				else if (mut.juliaInjection == mutJuliaInjectPostFold)
				{
					z += juliaC; // applied at pre-fold position, formula hasn't run yet
				}
				else if (mut.juliaInjection == mutJuliaInjectAlternating)
				{
					if (i % 2 == 0) z += juliaC;
				}
				else if (mut.juliaInjection == mutJuliaInjectRadial)
				{
					double r = z.Length();
					double bail = fractals.GetBailout(sequence);
					z += juliaC * (bail > 1e-21 ? r / bail : 1.0);
				}
				else if (mut.juliaInjection == mutJuliaInjectConditional)
				{
					double threshold = mut.juliaThreshold != 0.0 ? mut.juliaThreshold : 1.0;
					if (z.Length() > threshold) z += juliaC;
				}
				else if (mut.juliaInjection == mutJuliaInjectGradient)
				{
					double range = (double)(mut.juliaIterStop - mut.juliaIterStart);
					double t = range > 0.0 ? ((double)i - mut.juliaIterStart) / range : 1.0;
					double endMul = mut.juliaGradientEnd != 0.0 ? mut.juliaGradientEnd : 0.0;
					z += juliaC * (1.0 - t + endMul * t);
				}
				else if (mut.juliaInjection == mutJuliaInjectLayered)
				{
					int mid = (mut.juliaIterStart + mut.juliaIterStop) / 2;
					if (i >= mid)
						z += CVector4(mut.juliaLayerC2x, mut.juliaLayerC2y, mut.juliaLayerC2z, 0.0);
					else
						z += juliaC;
				}
			...
			if (fractals.IsAddCConstant(sequence))
			{
				...
				CVector3 mul = fractals.GetConstantMultiplier(sequence);
				CVector3 cStart = fractals.GetConstCIterStart(sequence);
				CVector3 cStop = fractals.GetConstCIterStop(sequence);
				CVector3 cWeight = fractals.GetConstCWeight(sequence);
				double mx = (i >= cStart.x && i < cStop.x) ? mul.x * cWeight.x : 1.0;
				double my = (i >= cStart.y && i < cStop.y) ? mul.y * cWeight.y : 1.0;
				double mz = (i >= cStart.z && i < cStop.z) ? mul.z * cWeight.z : 1.0;

				if (fractals.IsJuliaEnabled(sequence))
				{
					CVector3 juliaC = fractals.GetJuliaConstant(sequence);
					juliaC.x *= mx;
					juliaC.y *= my;
					juliaC.z *= mz;

					// Per-component iteration range + weight for Julia c
					CVector3 jStart = fractals.GetJuliaCIterStart(sequence);
					CVector3 jStop = fractals.GetJuliaCIterStop(sequence);
					CVector3 jWeight = fractals.GetJuliaCWeight(sequence);
					if (i < jStart.x || i >= jStop.x) juliaC.x = 0.0;
					else juliaC.x *= jWeight.x;
					if (i < jStart.y || i >= jStop.y) juliaC.y = 0.0;
					else juliaC.y *= jWeight.y;
					if (i < jStart.z || i >= jStop.z) juliaC.z = 0.0;
		...
		{
			const CVector3 &pos = in.common->fractalPosition;
			const CVector3 &ps = in.common->positionIterStart;
			const CVector3 &pe = in.common->positionIterStop;
			if (pos.x != 0.0 && i >= ps.x && i < pe.x) z.x += pos.x;
			if (pos.y != 0.0 && i >= ps.y && i < pe.y) z.y += pos.y;
			if (pos.z != 0.0 && i >= ps.z && i < pe.z) z.z += pos.z;

			const CVector3 &rot = in.common->fractalRotation;
			const CVector3 &rs = in.common->rotationIterStart;
			const CVector3 &re = in.common->rotationIterStop;
			if (rot.x != 0.0 && i >= rs.x && i < re.x)
			{
				CRotationMatrix mx; mx.SetRotation2(CVector3(rot.x * M_PI / 180.0, 0, 0));
				CVector3 zv(z.x, z.y, z.z); zv = mx.RotateVector(zv);
				z.x = zv.x; z.y = zv.y; z.z = zv.z;
			}
			if (rot.y != 0.0 && i >= rs.y && i < re.y)
			{
				CRotationMatrix my; my.SetRotation2(CVector3(0, rot.y * M_PI / 180.0, 0));
				CVector3 zv(z.x, z.y, z.z); zv = my.RotateVector(zv);
				z.x = zv.x; z.y = zv.y; z.z = zv.z;
			}
			if (rot.z != 0.0 && i >= rs.z && i < re.z)
			{
				CRotationMatrix mz; mz.SetRotation2(CVector3(0, 0, rot.z * M_PI / 180.0));
				CVector3 zv(z.x, z.y, z.z); zv = mz.RotateVector(zv);
				z.x = zv.x; z.y = zv.y; z.z = zv.z;
			}

			const CVector3 &rep = in.common->repeat;
			const CVector3 &rps = in.common->repeatIterStart;
			const CVector3 &rpe = in.common->repeatIterStop;
			if (rep.x != 0.0 && i >= rps.x && i < rpe.x)
				z.x = z.x - rep.x * round(z.x / rep.x);
			if (rep.y != 0.0 && i >= rps.y && i < rpe.y)
				z.y = z.y - rep.y * round(z.y / rep.y);
			if (rep.z != 0.0 && i >= rps.z && i < rpe.z)
		...
		if (mut.enabled && i >= mut.iterationStart && i < mutIterStop
			...
			&& (mut.juliaInjection == mutJuliaInjectMidFold
				...
				|| mut.juliaInjection == mutJuliaInjectDual
				|| mut.juliaInjection == mutJuliaInjectPostFold
				|| mut.juliaInjection == mutJuliaInjectAlternating))
		{
			// Type 7 (alternating): only inject post on odd iterations
			bool doPost = (mut.juliaInjection != mutJuliaInjectAlternating) || (i % 2 == 1);
			if (doPost)
			{
			...
			switch (mut.juliaCTransform)
			{
				...
				{
					double cLen = juliaC.Length();
					if (cLen > 1e-21) juliaC = juliaC * (log(cLen) / cLen);
					break;
				}
				case mutJuliaCExponential:
					juliaC.x = exp(juliaC.x);
			...
			// Blend C-transform by strength
			if (mut.juliaCTransformStrength != 1.0)
				juliaC = juliaCPreCT + (juliaC - juliaCPreCT) * mut.juliaCTransformStrength;
			} // end C-transform iter range (mid/post)
			// Dynamic modulation (with per-subsystem iter range + strength)
			if (mut.juliaDynamic != mutJuliaDynNone
				&& i >= mut.juliaDynamicIterStart && i < mut.juliaDynamicIterStop)
			{
			CVector4 juliaCPreDynMP = juliaC;
			switch (mut.juliaDynamic)
			{
				case mutJuliaDynOrbitMod:
					juliaC *= (1.0 + 0.1 * z.Length());
			...
			// Blend dynamic by strength
			if (mut.juliaDynamicStrength != 1.0)
				juliaC = juliaCPreDynMP + (juliaC - juliaCPreDynMP) * mut.juliaDynamicStrength;
		...
		// -------------- Formula Mutation post-processing ---------------
		{
			...
			if (mutationActive)
			{
				...
				if (mut.clipEnabled && i >= mut.clipIterStart && i < mut.clipIterStop && mut.clipType != 0)
				{
					...
					CVector4 z_orig_clip = z; double de_orig_clip = aux.DE;
					switch (mut.clipType)
					{
						...
						case 51: { clipDist=sin(cz.x*ca)*cos(cz.y*ca)+sin(cz.y*ca)*cos(cz.z*ca)+sin(cz.z*ca)*cos(cz.x*ca)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Gyroid surface
						case 52: { clipDist=cos(cz.x*ca)+cos(cz.y*ca)+cos(cz.z*ca)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Schwarz P surface
						case 53: { clipDist=sin(cz.x*ca)*sin(cz.y*ca)*sin(cz.z*ca)+sin(cz.x*ca)*cos(cz.y*ca)*cos(cz.z*ca)+cos(cz.x*ca)*sin(cz.y*ca)*cos(cz.z*ca)+cos(cz.x*ca)*cos(cz.y*ca)*sin(cz.z*ca)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Diamond surface
						case 54: { clipDist=0.5*(sin(2.0*cz.x*ca)*cos(cz.y*ca)*sin(cz.z*ca)+sin(2.0*cz.y*ca)*cos(cz.z*ca)*sin(cz.x*ca)+sin(2.0*cz.z*ca)*cos(cz.x*ca)*sin(cz.y*ca))-0.5*(cos(2.0*cz.x*ca)*cos(2.0*cz.y*ca)+cos(2.0*cz.y*ca)*cos(2.0*cz.z*ca)+cos(2.0*cz.z*ca)*cos(2.0*cz.x*ca))-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Lidinoid surface
						case 55: { clipDist=3.0*(cos(cz.x*ca)+cos(cz.y*ca)+cos(cz.z*ca))+4.0*cos(cz.x*ca)*cos(cz.y*ca)*cos(cz.z*ca)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Neovius surface
						case 56: { double t56=atan2(cz.y,cz.x); double r56xy=sqrt(cz.x*cz.x+cz.y*cz.y); double kx=cos(2.0*t56)*(cR+cr*cos(3.0*t56)); double ky=sin(2.0*t56)*(cR+cr*cos(3.0*t56)); double kz=cr*sin(3.0*t56); clipDist=sqrt((cz.x-kx)*(cz.x-kx)+(cz.y-ky)*(cz.y-ky)+(cz.z-kz)*(cz.z-kz))-ca; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Torus knot (2,3)
						case 57: { double r57=cz.Length(); clipDist=cz.x*cz.x*cz.z+cz.y*cz.y*cz.z-ca*cz.x*cz.y-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Cross-cap surface
						case 58: { double phi58=0.5*(1.0+sqrt(5.0)); double p2=phi58*phi58; clipDist=4.0*(p2*cz.x*cz.x-cz.y*cz.y)*(p2*cz.y*cz.y-cz.z*cz.z)*(p2*cz.z*cz.z-cz.x*cz.x)-(1.0+2.0*phi58)*(cz.x*cz.x+cz.y*cz.y+cz.z*cz.z-ca*ca)*(cz.x*cz.x+cz.y*cz.y+cz.z*cz.z-ca*ca); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Barth sextic
						case 59: { clipDist=cz.x*cz.x*cz.y*cz.y+cz.y*cz.y*cz.z*cz.z+cz.z*cz.z*cz.x*cz.x-ca*cz.x*cz.y*cz.z; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Steiner surface
						case 60: { double r60=cz.Length(); if(r60<1e-15)r60=1e-15; double x60=cz.x/r60,y60=cz.y/r60,z60=cz.z/r60; clipDist=64.0*x60*x60*x60*z60-48.0*x60*z60*(3.0*x60*x60+z60*z60)+12.0*z60*(x60*x60-z60*z60)+x60*x60+y60*y60+z60*z60-cr*cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Boy surface approximation
						case 61: { double r61=cz.Length(); clipDist=r61*exp(-ca*r61)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Exponential decay shell
						case 62: { double r62=sqrt(cz.x*cz.x+cz.y*cz.y); double t62=atan2(cz.y,cz.x); double spiralR=ca*exp(cb*t62); clipDist=fabs(r62-spiralR)+fabs(cz.z)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Logarithmic spiral shell
						case 63: { double t63=atan2(cz.y,cz.x); double r63=sqrt(cz.x*cz.x+cz.y*cz.y); double h1=r63-cR-cr*cos(ca*cz.z); double h2=r63-cR+cr*cos(ca*cz.z+M_PI); clipDist=fmin(sqrt(h1*h1+pow(cr*sin(ca*cz.z),2.0)),sqrt(h2*h2+pow(cr*sin(ca*cz.z+M_PI),2.0)))-cb; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Double helix
						case 64: { double dx64=fabs(cz.x)-sx; double dy64=fabs(cz.y)-sy; double dz64=fabs(cz.z)-sz; double chamfer=ca; clipDist=fmax(fmax(dx64,fmax(dy64,dz64)),(dx64+dy64+dz64)*chamfer); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Chamfered cube
						case 65: { double dx65=fmax(fabs(cz.x)-sx,0.0); double dy65=fmax(fabs(cz.y)-sy,0.0); double dz65=fmax(fabs(cz.z)-sz,0.0); clipDist=sqrt(dx65*dx65+dy65*dy65+dz65*dz65)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Rounded box
						case 66: { clipDist=fabs(cz.x)+fabs(cz.y)+fabs(cz.z)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Octahedron
						case 67: { double phi67=0.5*(1.0+sqrt(5.0)); double iphi67=1.0/phi67; clipDist=fmax(fmax(fabs(cz.x)+phi67*fabs(cz.y),fabs(cz.y)+phi67*fabs(cz.z)),fabs(cz.z)+phi67*fabs(cz.x))-cr*(1.0+phi67); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Dodecahedron approx
						case 68: { double phi68=0.5*(1.0+sqrt(5.0)); clipDist=fmax(fabs(cz.x)+phi68*fabs(cz.z),fmax(fabs(cz.y)+phi68*fabs(cz.x),fabs(cz.z)+phi68*fabs(cz.y)))-cr*phi68*phi68; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Icosahedron approx
						case 69: { double d69a=fmax(fabs(cz.x),fmax(fabs(cz.y),fabs(cz.z)))-sx; double d69b=(fabs(cz.x)+fabs(cz.y)+fabs(cz.z))*0.577-cr; clipDist=fmax(d69a,d69b); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Truncated octahedron
						case 70: { double q70x=fabs(cz.x); double q70y=fabs(cz.y); clipDist=fmax(q70x*0.866+q70y*0.5,q70y)-cr; clipDist=fmax(clipDist,fabs(cz.z)-sz); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Hex prism
						case 71: { clipDist=fmax(fabs(cz.z)-sz,fmax(fabs(cz.x)*0.866+cz.y*0.5,-cz.y)-cr); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Triangular prism
						case 72: { double rxy72a=sqrt(cz.x*cz.x+cz.y*cz.y); double d72a=sqrt((rxy72a-cR)*(rxy72a-cR)+cz.z*cz.z)-cr; double ryz72=sqrt(cz.y*cz.y+cz.z*cz.z); double d72b=sqrt((ryz72-cR)*(ryz72-cR)+cz.x*cz.x)-cr; clipDist=fmin(d72a,d72b); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Link (two interlocked tori)
						case 73: { double r73=sqrt(cz.x*cz.x+cz.y*cz.y)-cr; double h73=fabs(cz.z)-sz; clipDist=fmin(fmax(r73,h73),0.0)+sqrt(fmax(r73,0.0)*fmax(r73,0.0)+fmax(h73,0.0)*fmax(h73,0.0)); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Capped cylinder
						case 74: { double capH=fmax(0.0,fmin(sz,cz.z)); clipDist=sqrt(cz.x*cz.x+cz.y*cz.y+(cz.z-capH)*(cz.z-capH))-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Capsule
						case 75: { double t75=atan2(cz.y,cz.x); double r75=sqrt(cz.x*cz.x+cz.y*cz.y); double knotR=cR+cr*cos(3.0*t75); clipDist=sqrt((r75-knotR)*(r75-knotR)+cz.z*cz.z)-ca; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Trefoil knot
						case 76: { double t76=atan2(cz.y,cz.x); double r76=sqrt(cz.x*cz.x+cz.y*cz.y)-cR; double localZ=cz.z*cos(t76*0.5)-r76*sin(t76*0.5); clipDist=fabs(localZ)-ca; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Möbius strip approx
						case 77: { double sc77x=sin(cz.x*ca)*sin(cz.x*ca); double sc77y=sin(cz.y*ca)*sin(cz.y*ca); double sc77z=sin(cz.z*ca)*sin(cz.z*ca); clipDist=sc77x+sc77y+sc77z-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Sine cage
						case 78: { clipDist=cz.x*cz.x*cz.x*cz.x+cz.y*cz.y*cz.y*cz.y+cz.z*cz.z*cz.z*cz.z-ca*(cz.x*cz.x+cz.y*cz.y+cz.z*cz.z); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Pillow shape
						case 79: { double n79=sin(ca*cz.x)*sin(ca*cz.y)*sin(ca*cz.z); clipDist=cz.Length()-cr-cb*n79; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Displacement sphere (noise)
						case 80: { double r80=sqrt(cz.x*cz.x+cz.y*cz.y)-cR; double c80=r80*r80+(cz.z-ca*atan2(cz.y,cz.x)/(2.0*M_PI))*(cz.z-ca*atan2(cz.y,cz.x)/(2.0*M_PI))-cr*cr; clipDist=c80; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Corkscrew
						case 81: { double u81=atan2(cz.y,cz.x); double v81=atan2(cz.z,sqrt(cz.x*cz.x+cz.y*cz.y)-cR); double kb=ca*(1.0-0.5*cos(u81))*sin(v81)-cb*cos(v81); clipDist=fabs(kb)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Klein bottle cross-section
						case 82: { double r82=sqrt(cz.x*cz.x+cz.y*cz.y); double wave82=cr+ca*sin(cb*cz.z); clipDist=r82-wave82; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Wavy cylinder
						case 83: { double twist83=ca*cz.z; double rx83=cz.x*cos(twist83)-cz.y*sin(twist83); double ry83=cz.x*sin(twist83)+cz.y*cos(twist83); clipDist=fmax(fabs(rx83)-sx,fmax(fabs(ry83)-sy,fabs(cz.z)-sz)); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Twisted box
						case 84: { double bend84=ca*cz.x; double bz84=cz.z+bend84*bend84; double rxy84=sqrt(cz.x*cz.x+cz.y*cz.y)-cR; clipDist=sqrt(rxy84*rxy84+bz84*bz84)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Bent torus
						case 85: { clipDist=cz.z-ca*sin(cb*cz.x)-cc*cos(cb*cz.y)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Corrugated plane
						case 86: { double r86=cz.Length(); if(r86<1e-15)r86=1e-15; double theta86=acos(cz.z/r86); double phi86=atan2(cz.y,cz.x); double m86=(double)mut.clipNPoints; double st86=pow(pow(fabs(cos(m86*theta86/4.0)),cb)+pow(fabs(sin(m86*theta86/4.0)),cc),-1.0/ca); double sp86=pow(pow(fabs(cos(m86*phi86/4.0)),cb)+pow(fabs(sin(m86*phi86/4.0)),cc),-1.0/ca); clipDist=r86-cr*st86*sp86; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Supershape 3D
						case 87: { double r87=cz.Length(); double cosTheta87=cz.z/(r87+1e-15); clipDist=r87-cr*(1.0+ca*cosTheta87); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Egg shape
						case 88: { double x2_88=cz.x*cz.x,y2_88=cz.y*cz.y,z2_88=cz.z*cz.z; clipDist=2.0*y2_88*(y2_88-ca*ca)*(y2_88-ca*ca)-x2_88*x2_88+2.0*ca*ca*(x2_88-z2_88*3.0)*(x2_88-z2_88); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Genus-2 surface
						case 89: { double fx89=fabs(fmod(cz.x+ca,2.0*ca))-ca; double fy89=fabs(fmod(cz.y+ca,2.0*ca))-ca; double fz89=fabs(fmod(cz.z+ca,2.0*ca))-ca; clipDist=sqrt(fx89*fx89+fy89*fy89+fz89*fz89)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Folded sphere
						case 90: { clipDist=fmax(fmax(fabs(cz.x+cz.y),fabs(cz.x-cz.y)),fmax(fabs(cz.y+cz.z),fmax(fabs(cz.y-cz.z),fmax(fabs(cz.x+cz.z),fabs(cz.x-cz.z)))))-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Rhombic dodecahedron
						case 91: { clipDist=fmax(fabs(cz.x+cz.y+cz.z),fmax(fabs(cz.x-cz.y-cz.z),fmax(fabs(-cz.x+cz.y-cz.z),fabs(-cz.x-cz.y+cz.z))))-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Tetrahedral symmetry clip
						case 92: { clipDist=sin(ca*cz.x)+sin(ca*cz.y)+sin(ca*cz.z)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Sine-wave egg crate
						case 93: { clipDist=sin(ca*cz.x)-sinh(ca*cz.y)*sinh(ca*cz.z)/(cosh(ca*cz.y)*cosh(ca*cz.z)+1e-10); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Scherk surface
						case 94: { double u94=cz.x,v94=cz.y; double ex94=u94-u94*u94*u94/3.0+u94*v94*v94; double ey94=v94-v94*v94*v94/3.0+v94*u94*u94; double ez94=u94*u94-v94*v94; clipDist=sqrt((cz.x-ex94)*(cz.x-ex94)+(cz.y-ey94)*(cz.y-ey94)+(cz.z-ez94)*(cz.z-ez94))-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Enneper surface level set
						case 95: { clipDist=cz.x*cz.x/fmax(sx*sx,1e-10)-cz.y*cz.y/fmax(sy*sy,1e-10)-cz.z; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Saddle clip
						case 96: { clipDist=cz.x*(cz.x*cz.x-3.0*cz.y*cz.y)-ca*cz.z; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Monkey saddle
						case 97: { clipDist=fabs(cz.x)+fabs(cz.y)+fabs(cz.z)+fabs(z.w)-cr; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Cross-polytope (hyperoctahedron)
						case 98: { clipDist=cz.x*cz.x/fmax(sx*sx,1e-10)+cz.y*cz.y/fmax(sy*sy,1e-10)-cz.z; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Elliptic paraboloid
						case 99: { double d99s=cz.Length()-cr; double d99b=fmax(fabs(cz.x)-sx,fmax(fabs(cz.y)-sy,fabs(cz.z)-sz)); double k99=mut.clipSmoothK; double h99=fmax(0.0,fmin(1.0,0.5-0.5*(d99s-d99b)/fmax(k99,1e-10))); clipDist=d99s*(1.0-h99)+d99b*h99+k99*h99*(1.0-h99); if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Smooth intersection (sphere-box)
						case 100: { CVector3 ifs=cz; double ifsScale=1.0; for(int k=0;k<6;k++){if(ifs.x+ifs.y<0.0){double tx=-ifs.y;ifs.y=-ifs.x;ifs.x=tx;} if(ifs.x+ifs.z<0.0){double tx=-ifs.z;ifs.z=-ifs.x;ifs.x=tx;} if(ifs.y+ifs.z<0.0){double ty=-ifs.z;ifs.z=-ifs.y;ifs.y=ty;} ifs=ifs*ca-CVector3(cb,cb,cb)*(ca-1.0); ifsScale*=ca;} clipDist=(ifs.Length()-cr)/ifsScale; if (clipDist < 0.0) { cz.x *= 0.0; cz.y *= 0.0; cz.z *= 0.0; } break; } // Fractal IFS (Sierpinski tetrahedron)
					}
					// Apply the transformed z vector back to the original z
					cz.x += mut.clipCenterX; cz.y += mut.clipCenterY; cz.z += mut.clipCenterZ;
					z = CVector4(cz, z.w);
					double clip_blend = std::min(std::max(mut.clipWeight * mut.masterWeight, 0.0), 1.0);
				...
				{
					double jf = mut.josFactor * mut.josLeysWeight * mut.masterWeight;
				...
				{
					double pf = mut.pkFactor * mut.pkWeight * mut.masterWeight;
				...
				{
					double mf = mut.mbFactor * mut.mbMathWeight * mut.masterWeight;
				...
				{
					double wf = mut.wdFactor * mut.warpDistWeight * mut.masterWeight;
				...
				{
					double sf = mut.skFactor * mut.symmetryWeight * mut.masterWeight;
					...
					double zx = z.x, zy = z.y, zz2 = z.z;
					double r = sqrt(zx*zx + zy*zy + zz2*zz2 + 1e-21);
					CVector4 z_orig_sk = z; double de_orig_sk = aux.DE;
					...
					double sk_blend = std::min(std::max(mut.symmetryWeight * mut.masterWeight, 0.0), 1.0);
				...
				if (i >= mut.abIterStart && i < mut.abIterStop && mut.aboxType != 0)
				{
					...
					double ab_blend = std::min(std::max(mut.aboxWeight * mut.masterWeight, 0.0), 1.0);
				...
				if (i >= mut.noiseIterStart && i < mut.noiseIterStop && mut.noiseType != 0)
				{
					...
					double nf = mut.noiseFactor * mut.noiseWeight * mut.masterWeight, nfq = mut.noiseFreq, nam = mut.noiseAmp;
					double r = sqrt(z.x*z.x + z.y*z.y + z.z*z.z);
				...
				if (i >= mut.orbitIterStart && i < mut.orbitIterStop && mut.orbitTrapType != 0)
				{
					...
					double of = mut.orbitFactor * mut.orbitTrapWeight * mut.masterWeight;
				...
				{
					double tf = mut.torusFactor * mut.torusWeight * mut.masterWeight;
				...
				{
					double sf = mut.asFactor * mut.asWeight * mut.masterWeight;
					double ta = mut.asParamA, tb = mut.asParamB;
					double tc = mut.asParamC, td = mut.asParamD;
					CVector4 z_orig_as = z; double de_orig_as = aux.DE;
					switch(mut.asType) {
						case 1: { double k = ta; double DE0 = tb; aux.DE = 1.0/(1.0 + exp(-k*(aux.DE - DE0))); break; }
						case 2: { double DE0 = ta; double sigma = fmax(fabs(tb), 0.01); aux.DE *= exp(-(aux.DE-DE0)*(aux.DE-DE0)/(sigma*sigma)); break; }
						case 3: { double threshold = ta; double DE_max = fabs(tb)+1.0; double DE_min = fabs(tc)*0.01; aux.DE = (aux.DE > threshold) ? DE_max : DE_min; break; }
						case 4: { double b1=ta, b2=tb; double d1=fabs(tc),d2=fabs(td); if(aux.DE<b1) aux.DE=d1; else if(aux.DE<b2) aux.DE=d2; break; }
						case 5: { double h = sin(z.x*127.1+z.y*311.7+z.z*74.7)*43758.5453; h=h-floor(h); aux.DE += sf*(h*2.0-1.0)*ta; break; }
						case 6: { double jx=z.x*z.x-z.y*z.y+ta; double jy=2.0*z.x*z.y+tb; double julia_mag=sqrt(jx*jx+jy*jy); aux.DE *= (1.0 + sf*0.1*julia_mag); break; }
						case 7: { aux.DE *= (1.0 + sf*0.1*sin(ta*(double)i*0.1)); break; }
						case 8: { double alpha=fmin(fmax(ta,0.01),0.99); aux.DE = alpha*aux.DE + (1.0-alpha)*tb; break; }
						case 9: { double h1=sin(ta*z.x+tb*z.y+tc*z.z); double h2=sin(td*h1+ta*aux.DE); aux.DE *= (1.0 + sf*h2); break; }
						case 10: { double v=0,f=ta,a=1.0; for(int k=0;k<4;k++){double h=sin(z.x*f*127.1+z.y*f*311.7+z.z*f*74.7)*43758.5453;h=h-floor(h);v+=a*h;f*=2.0;a*=0.5;} aux.DE *= (1.0 + sf*0.1*v); break; }
						case 11: { double speed = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); aux.DE /= (1.0 + sf*speed*ta); break; }
						case 12: { double sc = fabs(ta) + 0.01; aux.DE *= sc; break; }
						case 13: { double angle = atan2(z.y,z.x); aux.DE *= (1.0 + sf*fabs(angle)*ta/M_PI); break; }
						case 14: { double shear = fabs(z.x*z.y*ta) + fabs(z.y*z.z*tb) + fabs(z.z*z.x*tc); aux.DE *= (1.0 + sf*shear); break; }
						case 15: { double progress = (double)i / fmax((double)(i+10), 1.0); aux.DE *= (1.0 + sf*(1.0-progress)*ta); break; }
						case 16: { double box = fmax(fabs(z.x),fmax(fabs(z.y),fabs(z.z))); if(box > fabs(ta)) aux.DE *= (1.0 + sf*tb); break; }
						case 17: { double d = sqrt((z.x-ta)*(z.x-ta)+(z.y-tb)*(z.y-tb)+(z.z-tc)*(z.z-tc)); if(d < fabs(td)) aux.DE *= (1.0 + sf); break; }
						case 18: { double w = fabs(ta)+0.01; if(fabs(z.x)<w && fabs(z.y)<w) aux.DE *= (1.0 + sf*tb); break; }
						case 19: { double intensity = sin(z.x*ta)*sin(z.y*tb)*sin(z.z*tc); aux.DE *= (1.0 + sf*0.1*intensity*intensity); break; }
						case 20: { double ao = 1.0/(1.0 + (double)i*ta*0.01); aux.DE *= (1.0 - sf*ao*fabs(tb)); break; }
						case 21: { double gi = sin(z.x*ta+z.y*tb)*0.5+0.5; aux.DE /= (1.0 + sf*gi*fabs(tc)); break; }
						case 22: { double depth = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); aux.DE *= (1.0 + sf*ta*depth); break; }
						case 23: { double light_accum = sin(z.x*ta)*sin(z.y*ta)*0.5+0.5; aux.DE /= (1.0 + sf*light_accum*tb); break; }
						case 24: { double ss_depth = exp(-fabs(ta)*sqrt(z.x*z.x+z.y*z.y+z.z*z.z)); aux.DE *= (1.0 + sf*ss_depth*tb); break; }
						case 25: { double wl_diff = fabs(z.x*ta - tb); aux.DE *= (1.0 + sf*tc*wl_diff); break; }
						case 26: { double caustic = sin(z.x*ta*10.0)*sin(z.y*tb*10.0); aux.DE *= (1.0 + sf*caustic*caustic*tc); break; }
						case 27: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double view_dot = z.z/fmax(r,1e-21); double fresnel = pow(fmax(1.0-fabs(view_dot),0.0), ta); aux.DE *= (1.0 + sf*fresnel*tb); break; }
						case 28: { double thin_film = sin(ta*sqrt(z.x*z.x+z.y*z.y+z.z*z.z)*M_PI*2.0); aux.DE *= (1.0 + sf*thin_film*thin_film*tb); break; }
						case 29: { double tangent_dot = fabs(z.x*cos(ta)+z.y*sin(ta))/fmax(sqrt(z.x*z.x+z.y*z.y),1e-21); aux.DE *= (1.0 + sf*tangent_dot*tb); break; }
						case 30: { double subdiv = fmax(1.0, fabs(ta)); aux.DE /= (1.0 + sf*subdiv*tb); break; }
						case 31: { double dist = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double lod = fmax(0.1, dist*ta); aux.DE *= lod; break; }
						case 32: { double edge_len = fabs(z.x-z.y) + fabs(z.y-z.z) + fabs(z.z-z.x); aux.DE *= (1.0 + sf*ta*edge_len*tb); break; }
						case 33: { double disp = sin(z.x*ta*5.0)*sin(z.y*tb*5.0)*sin(z.z*tc*5.0); aux.DE += sf*disp*td; break; }
						case 34: { double h = sin(z.x*ta*7.0+z.y*tb*11.0+z.z*tc*13.0)*43758.5453; h=h-floor(h); aux.DE *= (1.0 + sf*(h*2.0-1.0)*td*0.1); break; }
						case 35: { double parallax = fabs(ta)*z.z/fmax(fabs(tb),0.01); aux.DE *= (1.0 + sf*parallax*0.1); break; }
						case 36: { double n = fmax(fabs(ta)+1.0, 1.01); aux.DE /= n; break; }
						case 37: { double dist_traveled = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); aux.DE *= exp(-sf*fabs(ta)*dist_traveled); break; }
						case 38: { double cos_theta = z.z/fmax(sqrt(z.x*z.x+z.y*z.y+z.z*z.z),1e-21); double phase = (1.0+cos_theta*cos_theta)*0.75; aux.DE *= (1.0 + sf*phase*ta); break; }
						case 39: { double emission = sin(z.x*ta)*sin(z.y*ta)*0.5+0.5; aux.DE /= (1.0 + sf*emission*fabs(tb)); break; }
						case 40: { double curl_approx = sin(z.y*ta)*cos(z.z*tb) - cos(z.y*ta)*sin(z.z*tb); aux.DE *= (1.0 + sf*fabs(curl_approx)*tc); break; }
						case 41: { double grad = (sin(z.x*ta)-sin((z.x+0.01)*ta))/0.01; aux.DE /= (1.0 + sf*fabs(grad)*tb); break; }
						case 42: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double mass_d = fmax(r - ta, 0.01); aux.DE *= (1.0 + sf*tb/(mass_d*mass_d)); break; }
						case 43: { double v2 = (z.x*z.x+z.y*z.y+z.z*z.z)*ta*ta; double lorentz = sqrt(fmax(1.0 - fmin(v2, 0.99), 0.01)); aux.DE *= lorentz; break; }
						case 44: { double barrier = fabs(z.x - ta); aux.DE *= exp(-sf*fabs(tb)*barrier); break; }
						case 45: { double psi2 = sin(ta*z.x)*sin(tb*z.y)*sin(tc*z.z); aux.DE *= (1.0 + sf*psi2*psi2); break; }
						case 46: { double momentum = sqrt(z.x*z.x+z.y*z.y+z.z*z.z)*ta; double pos = fmax(fabs(z.x), 0.01); aux.DE *= (1.0 + sf*tb/(2.0*momentum*pos+0.01)); break; }
						case 47: { double spin_dot = z.x*sin(ta)+z.y*cos(ta)+z.z*sin(tb); aux.DE *= (1.0 + sf*tc*spin_dot); break; }
						case 48: { double corr = sin(z.x*ta)*sin((-z.x)*ta) + sin(z.y*tb)*sin((-z.y)*tb); aux.DE *= (1.0 + sf*fabs(corr)*tc); break; }
						case 49: { double decay = exp(-sf*fabs(ta)*(double)i*0.01); aux.DE *= decay; break; }
						case 50: { double entropy = sin(z.x*ta*3.0)*sin(z.y*tb*5.0)*sin(z.z*tc*7.0); double temp = fabs(td)+0.01; aux.DE *= (1.0 + sf*temp*entropy*entropy); break; }
						case 51: { double kT = fmax(fabs(ta)*fabs(tb), 0.01); aux.DE *= (1.0 + sf*tc/kT); break; }
						case 52: { double vel_grad = fabs(z.x-z.y)*ta + fabs(z.y-z.z)*tb; aux.DE *= (1.0 + sf*tc*vel_grad); break; }
						case 53: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double curv = 1.0/fmax(r,0.01); aux.DE *= (1.0 + sf*ta*curv); break; }
						case 54: { double tube_r = fmax(fabs(sqrt(z.x*z.x+z.y*z.y) - ta), 0.01); aux.DE *= (1.0 + sf*cos(tb)/tube_r); break; }
						case 55: { double conc_grad = fabs(sin(z.x*ta) - sin(z.y*ta)); aux.DE *= (1.0 + sf*tb*conc_grad); break; }
						case 56: { double lap = sin((z.x+0.01)*ta)+sin((z.x-0.01)*ta)-2.0*sin(z.x*ta); aux.DE += sf*tb*lap; break; }
						case 57: { double vel_dot = sin(z.x*ta)*cos(z.y*tb); aux.DE *= (1.0 + sf*vel_dot*tc); break; }
						case 58: { double v=0,f=ta,a=1.0; for(int k=0;k<4;k++){double h=sin(z.x*f*12.9+z.y*f*78.2+z.z*f*45.1)*43758.5453;h=h-floor(h);v+=a*h;f*=2.0;a*=0.5;} aux.DE *= (1.0 + sf*v*tb); break; }
						case 59: { double front = fabs(z.x - ta*i*0.01); double shock = (front < fabs(tb)) ? tc : 1.0; aux.DE *= shock; break; }
						case 60: { double freq_diff = fabs(sin(z.x*ta) - sin(z.x*tb)); double Q = fmax(fabs(tc), 0.1); aux.DE *= (1.0 + sf*Q*1.0/fmax(freq_diff+0.01, 0.01)); break; }
						case 61: { double phase_diff = ta*z.x - tb*z.y; aux.DE *= (1.0 + sf*cos(phase_diff)*tc); break; }
						case 62: { double v_source = ta * 0.1; double angle = atan2(z.y, z.x); aux.DE *= (1.0 + sf*v_source*cos(angle)*tb); break; }
						case 63: { double theta = atan2(z.y, z.x); aux.DE *= (1.0 + sf*ta*cos(2.0*theta)); break; }
						case 64: { double phi2 = atan2(z.z, sqrt(z.x*z.x+z.y*z.y)); aux.DE *= (1.0 + sf*ta*fabs(sin(2.0*phi2))); break; }
						case 65: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double path = r * fabs(ta) * fabs(tb); aux.DE *= (1.0 + sf*tc*path); break; }
						case 66: { double lat_pt = fmod(fabs(z.x*ta), 1.0); double bandgap = (lat_pt < 0.5) ? 1.0 : tb; aux.DE *= bandgap; break; }
						case 67: { double surf_dist = fabs(sqrt(z.x*z.x+z.y*z.y+z.z*z.z) - ta); double skin = fmax(fabs(tb), 0.01); aux.DE *= (1.0 + sf*tc*exp(-surf_dist/skin)); break; }
						case 68: { double neg_idx = fabs(ta); aux.DE *= (1.0 + sf*neg_idx*fabs(tb)); break; }
						case 69: { double winding = sin(z.x*ta)*cos(z.y*tb) + cos(z.x*ta)*sin(z.y*tb); aux.DE *= (1.0 + sf*fabs(winding)*tc); break; }
						case 70: { double charge = sin(z.x*ta*5.0)*sin(z.y*tb*5.0)*sin(z.z*tc*5.0); aux.DE *= (1.0 + sf*charge*charge*td); break; }
						case 71: { double curl_mag = fabs(sin(z.y*ta)-sin(z.z*ta)) + fabs(sin(z.z*tb)-sin(z.x*tb)); aux.DE *= (1.0 + sf*curl_mag*tc); break; }
						case 72: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double sech_val = 1.0/cosh(fmin(ta*(r-tb), 20.0)); aux.DE *= (1.0 + sf*tc*sech_val); break; }
						case 73: { double S = fabs(ta)*sqrt(z.x*z.x+z.y*z.y+z.z*z.z); aux.DE *= (1.0 + sf*tb*exp(-fmin(S,20.0))); break; }
						case 74: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double monopole = fabs(ta)/fmax(r,0.01); aux.DE *= (1.0 + sf*monopole*tb); break; }
						case 75: { double d_axis = sqrt(z.y*z.y+z.z*z.z); aux.DE *= (1.0 + sf*ta*log(fmax(d_axis,0.01))*tb); break; }
						case 76: { double d_brane = fabs(z.x - ta); double delta_approx = exp(-d_brane*d_brane*100.0); aux.DE *= (1.0 + sf*tb*delta_approx); break; }
						case 77: { double throat_d = (z.x-ta)*(z.x-ta)+(z.y-tb)*(z.y-tb)+(z.z-tc)*(z.z-tc); double throat_r = fmax(fabs(td), 0.01); aux.DE *= (1.0 + sf*exp(-throat_d/(throat_r*throat_r))); break; }
						case 78: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double rs = fmax(fabs(ta), 0.01); aux.DE /= fmax(1.0 - rs/fmax(r,rs+0.01), 0.01); break; }
						case 79: { double jet_d = z.y*z.y+z.z*z.z; double jet_w = fmax(fabs(tb), 0.01); aux.DE *= (1.0 + sf*ta*exp(-jet_d/(jet_w*jet_w))); break; }
						case 80: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double halo = 1.0/(1.0 + r*r/(fmax(ta*ta,0.01))); aux.DE *= (1.0 + sf*tb*halo); break; }
						case 81: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); aux.DE *= exp(sf*fabs(ta)*r*0.01); break; }
						case 82: { double d_string = sqrt(z.y*z.y+z.z*z.z); double horizon = fmax(fabs(tb), 0.01); aux.DE *= (1.0 + sf*ta*log(fmax(d_string/horizon,0.01))); break; }
						case 83: { double d_wall = fabs(z.x - ta); double thickness = fmax(fabs(tb), 0.01); aux.DE *= (1.0 + sf*tc*tanh(d_wall/thickness)); break; }
						case 84: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double theta2 = atan2(z.y,z.x); double phi2 = acos(z.z/fmax(r,1e-21)); double Ylm = sin(ta*theta2)*cos(tb*phi2); aux.DE *= (1.0 + sf*tc*Ylm*Ylm); break; }
						case 85: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double efold = r*fabs(ta); aux.DE *= exp(sf*fmin(efold*tb, 10.0)); break; }
						case 86: { double decay_t = exp(-fabs(ta)*(double)i*0.01); aux.DE *= (1.0 + sf*tb*decay_t*tc); break; }
						case 87: { double baryon = sin(z.x*ta)*sin(z.y*tb); double rate = exp(-fabs(tc)*(double)i*0.01); aux.DE *= (1.0 + sf*baryon*baryon*rate*td); break; }
						case 88: { double ion_frac = exp(-fabs(ta)/fmax(fabs(tb)*(double)i*0.01+0.01, 0.01)); aux.DE *= (1.0 + sf*ion_frac*tc); break; }
						case 89: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double scale_a = fmax(r*ta, 0.01); double energy = 1.0/(scale_a*scale_a); aux.DE *= (1.0 + sf*sqrt(energy)*tb); break; }
						case 90: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double scale_a = fmax(r*ta, 0.01); aux.DE /= fmax(scale_a*tb, 0.01); break; }
						case 91: { double ent_max = fabs(ta); double heat_time = fmax(fabs(tb), 0.01); aux.DE *= (1.0 + sf*ent_max*(1.0-exp(-(double)i*0.01/heat_time))); break; }
						case 92: { double bubble = sin(z.x*ta*5.0)*sin(z.y*tb*5.0)*sin(z.z*tc*5.0); aux.DE *= (1.0 + sf*fabs(bubble)*td); break; }
						case 93: { double barrier2 = fabs(ta); double tunnel = exp(-barrier2*sqrt(z.x*z.x+z.y*z.y+z.z*z.z)); aux.DE *= (1.0 + sf*tunnel*tb); break; }
						case 94: { double order = sin(z.x*ta)*sin(z.y*tb); double latent = fabs(tc); aux.DE *= (1.0 + sf*latent*order*order); break; }
						case 95: { double d_crit = sqrt((z.x-ta)*(z.x-ta)+(z.y-tb)*(z.y-tb)+(z.z-tc)*(z.z-tc)); double expo = fmax(fabs(td), 0.1); aux.DE *= pow(fmax(d_crit, 0.01), -expo); break; }
						case 96: { double cutoff = fmax(fabs(ta), 0.01); double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); aux.DE *= (1.0 + sf*tb*log(fmax(cutoff/fmax(r,0.01), 0.01))); break; }
						case 97: { double div_approx = cos(z.x*ta)*sin(z.y*tb)*cos(z.z*tc); aux.DE *= (1.0 + sf*fabs(div_approx)*td); break; }
						case 98: { double vev = sqrt(z.x*z.x+z.y*z.y+z.z*z.z)*ta; aux.DE *= (1.0 + sf*vev*vev*tb); break; }
					...
					double as_blend = std::min(std::max(mut.asWeight * mut.masterWeight, 0.0), 1.0);
				...
				{
					double sf = mut.smFactor * mut.smWeight * mut.masterWeight;
					double ta = mut.smParamA, tb = mut.smParamB;
					double tc = mut.smParamC, td = mut.smParamD;
					CVector4 z_orig_sm = z; double de_orig_sm = aux.DE;
					switch(mut.smType) {
						case 1: { double branch = 2.0 + floor(3.0*sin((double)i*ta*0.1)); aux.DE *= fmax(branch*tb, 0.01); break; }
						case 2: { double bx=fmax(ta,1.0),by=fmax(tb,1.0),bz=fmax(tc,1.0); aux.DE *= fmax(fmax(bx,by),bz)*sf; break; }
						case 3: { double angle = (double)i * 2.39996322972865332 * ta; double cosA=cos(angle),sinA=sin(angle); double nx=z.x*cosA-z.y*sinA; z.y=z.x*sinA+z.y*cosA; z.x=nx; break; }
						case 4: { double jm = sin(z.x*ta)*cos(z.y*tb); double r_ratio = 1.0 + sf*0.3*jm; aux.DE *= r_ratio; break; }
						case 5: { double h = sin(z.x*ta*127.1+z.y*ta*311.7+z.z*ta*74.7)*43758.5453; h=h-floor(h); aux.DE *= (1.0 + sf*(h*2.0-1.0)*tb); break; }
						case 6: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double d1 = r - ta; double d2 = r - tb; aux.DE *= (1.0 + sf*exp(-fmin(d1*d1,d2*d2)*tc)); break; }
						case 7: { double w = fabs(ta)+0.01; double wd = w*sin(z.x*tb)*cos(z.y*tc); aux.DE *= (1.0 + sf*wd*td); break; }
						case 8: { double ax=fmax(fabs(ta),0.01),ay=fmax(fabs(tb),0.01),az=fmax(fabs(tc),0.01); double ell = sqrt((z.x/ax)*(z.x/ax)+(z.y/ay)*(z.y/ay)+(z.z/az)*(z.z/az))-1.0; aux.DE *= (1.0 + sf*exp(-ell*ell*td)); break; }
						case 9: { double r2d = sqrt(z.x*z.x+z.y*z.y); double R=fabs(ta)+0.1,r=fabs(tb)+0.01; double td2 = (r2d-R)*(r2d-R)+z.z*z.z-r*r; aux.DE *= (1.0 + sf*exp(-fabs(td2)*tc)); break; }
						case 10: { double px=z.x,py=z.y,pz=z.z; double seg_d=fabs(pz)-fabs(ta); double cap_d=sqrt(px*px+py*py+fmax(seg_d,0.0)*fmax(seg_d,0.0))-fabs(tb); aux.DE *= (1.0 + sf*exp(-cap_d*cap_d*tc)); break; }
						case 11: { double theta=atan2(z.y,z.x); double phi2=acos(z.z/fmax(sqrt(z.x*z.x+z.y*z.y+z.z*z.z),1e-21)); double m=fmax(ta,1.0); double sf_r=pow(fmax(pow(fabs(cos(m*theta/4.0)),tc)+pow(fabs(sin(m*theta/4.0)),tc),1e-21),-1.0/fmax(tb,0.1)); double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); aux.DE *= (1.0 + sf*exp(-fabs(r-sf_r*td)*5.0)); break; }
						case 12: { double jx=z.x*z.x-z.y*z.y+ta; double jy=2.0*z.x*z.y+tb; z.x=jx; z.y=jy; aux.DE *= 2.0*sqrt(jx*jx+jy*jy); break; }
						case 13: { double mx=z.x*z.x-z.y*z.y+z.x; double my=2.0*z.x*z.y+z.y; z.x=mx; z.y=my; aux.DE *= 2.0*sqrt(mx*mx+my*my); break; }
						case 14: { double a1=ta*M_PI/180.0; double a2=tb*M_PI/180.0; double ca1=cos(a1),sa1=sin(a1); double newx=z.x+z.x*ca1-z.y*sa1; double newy=z.y+z.x*sa1+z.y*ca1; z.x=newx*0.5; z.y=newy*0.5; z.z*=tc; aux.DE *= fabs(tc); break; }
						case 15: { double r = sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double koch_amp = ta*sin(r*tb*10.0); aux.DE *= (1.0 + sf*koch_amp*koch_amp); break; }
						case 16: { double scale4 = fabs(ta)+1.0; if(z.x+z.y<0.0){double t=z.x;z.x=-z.y;z.y=-t;} if(z.x+z.z<0.0){double t=z.x;z.x=-z.z;z.z=-t;} if(z.y+z.z<0.0){double t=z.y;z.y=-z.z;z.z=-t;} z = z*scale4 - CVector4(1,1,1,0)*(scale4-1.0)*tb; aux.DE = aux.DE*scale4+1.0; break; }
						case 17: { double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); if(r<1e-21)r=1e-21; double mde=ta*ta/r/r; z*=mde; aux.DE*=mde; break; }
						case 18: { double h=sin(z.x*ta*12.9898+z.y*tb*78.233+z.z*tc*45.164)*43758.5453; h=h-floor(h); z.x+=h*td*0.1; z.y+=(1.0-h)*td*0.1; break; }
						case 19: { double min_dist=fabs(ta)+0.1; double h=sin(z.x*12.9898+z.y*78.233)*43758.5453; h=h-floor(h); z.x=floor(z.x/min_dist+h)*min_dist; z.y=floor(z.y/min_dist+h)*min_dist; break; }
						case 20: { double h=sin(z.x*12.9898+z.y*78.233+z.z*45.164)*43758.5453; h=fabs(h-floor(h)); z.x+=h*ta*0.1; z.y+=(1.0-h)*tb*0.1; z.z+=fabs(sin(h*tc))*td*0.1; break; }
						case 21: { double theta = (double)i * 2.39996322972865332; double r = sqrt((double)i+0.5)*ta; z.x += r*cos(theta)*sf*0.1; z.y += r*sin(theta)*sf*0.1; break; }
						case 22: { double phi3 = 1.6180339887; double r_n = ta/pow(phi3, (double)i); aux.DE *= 1.0/phi3; z *= r_n/fmax(sqrt(z.x*z.x+z.y*z.y+z.z*z.z),0.01); break; }
						case 23: { double r_n = ta * exp(-(double)i * fabs(tb)); aux.DE *= exp(-fabs(tb)); z *= r_n/fmax(sqrt(z.x*z.x+z.y*z.y+z.z*z.z),0.01); break; }
						case 24: { double n = fmax((double)i+1.0, 1.0); double r_n = ta / (1.0 + log(n)*tb); aux.DE *= 1.0/(1.0+log(n)*tb); z *= r_n/fmax(sqrt(z.x*z.x+z.y*z.y+z.z*z.z),0.01); break; }
						case 25: { double n = fmax((double)i+1.0, 1.0); double p = fmax(fabs(ta), 0.1); aux.DE *= 1.0/pow(n,p); break; }
						case 26: { double sc = 3.0 + 0.5*sin((double)i*ta); z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 27: { double sx=fmax(ta,1.0),sy=fmax(tb,1.0),sz=fmax(tc,1.0); z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z.x=z.x*sx-(sx-1.0); z.y=z.y*sy-(sy-1.0); z.z=z.z*sz-(sz-1.0); aux.DE=aux.DE*fmax(fmax(sx,sy),sz)+1.0; break; }
						case 28: { double h=sin(z.x*ta*127.1+z.y*ta*311.7+z.z*ta*74.7)*43758.5453; h=h-floor(h); double sc=3.0; z=fabs(z)+CVector4(h*tb,h*tb,h*tb,0)*0.1; if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 29: { double angle=ta*M_PI/180.0; double ca=cos(angle),sa=sin(angle); double nx=z.x*ca-z.y*sa; double ny=z.x*sa+z.y*ca; z.x=fabs(nx);z.y=fabs(ny);z.z=fabs(z.z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} double sc=3.0; z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 30: { double eps=fmax(fabs(ta),0.01); double sc=3.0; z.x=fabs(z.x);z.y=fabs(z.y);z.z=fabs(z.z); double t; if(z.x<z.y){t=z.x+(z.y-z.x)*0.5*(1.0+tanh((z.y-z.x)/eps));z.y=z.y-(z.y-z.x)*0.5*(1.0+tanh((z.y-z.x)/eps));z.x=z.y;z.y=t;} if(z.x<z.z){t=z.x;z.x=z.z;z.z=t;} if(z.y<z.z){t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 31: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)+CVector4(ta,tb,tc,0); aux.DE=aux.DE*sc+1.0; break; }
						case 32: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); z.x+=aux.const_c.x*ta; z.y+=aux.const_c.y*ta; z.z+=aux.const_c.z*ta; aux.DE=aux.DE*sc+1.0; break; }
						...
						case 34: { double sc=3.0; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<1e-21)r2=1e-21; double inv=ta*ta/r2; z*=inv; aux.DE*=inv; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 35: { double lim=fabs(ta)+0.5; if(z.x>lim)z.x=2.0*lim-z.x; if(z.x<-lim)z.x=-2.0*lim-z.x; if(z.y>lim)z.y=2.0*lim-z.y; if(z.y<-lim)z.y=-2.0*lim-z.y; if(z.z>lim)z.z=2.0*lim-z.z; if(z.z<-lim)z.z=-2.0*lim-z.z; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 36: { double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=fabs(ta)*fabs(ta); double fixedR2=1.0; double m; if(r2<minR2)m=fixedR2/minR2; else if(r2<fixedR2)m=fixedR2/r2; else m=1.0; z*=m; aux.DE*=m; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 37: { int n=(int)fmax(ta,2.0); double angle=M_PI/n; for(int k=0;k<n;k++){double a=k*2.0*angle; double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x-=2.0*d*cos(a);z.y-=2.0*d*sin(a);}} double sc=3.0; z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*tb; aux.DE=aux.DE*sc+1.0; break; }
						case 38: { z.x=fabs(z.x);z.y=-fabs(z.y);z.z=fabs(z.z); double sc=3.0; if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 39: { double twist_k=ta*0.01; double angle=twist_k*(double)i; double ca=cos(angle),sa=sin(angle); double nx=z.x*ca-z.y*sa; z.y=z.x*sa+z.y*ca; z.x=nx; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 40: { double bend=ta*0.001; z.z+=bend*(z.x*z.x+z.y*z.y); double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 41: { double shear=ta*0.1; z.x+=shear*z.y; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 42: { double taper=ta*0.01; z.x*=(1.0-taper*z.z); z.y*=(1.0-taper*z.z); double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 43: { double period=fmax(fabs(ta),0.1); z=CVector4(fmod(z.x+100.0*period,period)-period*0.5, fmod(z.y+100.0*period,period)-period*0.5, fmod(z.z+100.0*period,period)-period*0.5, z.w); double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 44: { double sc1=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc1-CVector4(1,1,1,0)*(sc1-1.0); aux.DE=aux.DE*sc1+1.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc1-CVector4(1,1,1,0)*(sc1-1.0); aux.DE=aux.DE*sc1+1.0; break; }
						case 45: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*fabs(sc)+1.0; double surfScale=fabs(ta)+0.5; z.x=fabs(z.x+surfScale)-fabs(z.x-surfScale)-z.x; z.y=fabs(z.y+surfScale)-fabs(z.y-surfScale)-z.y; break; }
						...
						case 47: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; double k_angle=ta*M_PI/180.0; CVector3 nv(cos(k_angle),sin(k_angle),0); CVector3 zv=z.GetXYZ(); double dot=zv.Dot(nv); if(dot<0){zv=zv-nv*(2.0*dot); z=CVector4(zv,z.w);} break; }
						case 48: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; z+=CVector4(aux.const_c.x*ta,aux.const_c.y*ta,aux.const_c.z*ta,0)*tb; break; }
						case 49: { double sc=3.0+0.1*sin((double)i*ta); z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*fabs(sc)+1.0; z=CVector4(0.9*z.x+0.1*ta,0.9*z.y+0.1*tb,0.9*z.z+0.1*tc,z.w); break; }
						case 50: { double h1=sin(ta*z.x+tb*z.y+tc*z.z); double h2=sin(td*h1+ta*(double)i*0.1); double sc=3.0+sf*h2*0.1; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*fabs(sc)+1.0; break; }
						case 51: { double sc=3.0; CVector4 zz=fabs(z); double vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){double t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=CVector4(vals[0],vals[1],vals[2],vals[3])*sc-CVector4(1,1,1,1)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 52: { double sc=3.0; double v[5]={fabs(z.x),fabs(z.y),fabs(z.z),fabs(z.w),fabs(sin(z.x+z.y))}; for(int a=0;a<5;a++)for(int b=a+1;b<5;b++){if(v[a]<v[b]){double t=v[a];v[a]=v[b];v[b]=t;}} z=CVector4(v[0],v[1],v[2],v[3])*sc-CVector4(1,1,1,1)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 53: { double sc=3.0; z.w=ta; CVector4 zz=fabs(z); double vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){double t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=CVector4(vals[0],vals[1],vals[2],vals[3])*sc-CVector4(1,1,1,1)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 54: { double angle4d=ta*M_PI/180.0*(double)i; double ca=cos(angle4d),sa=sin(angle4d); double nx=z.x*ca-z.w*sa; z.w=z.x*sa+z.w*ca; z.x=nx; double sc=3.0; CVector4 zz=fabs(z); double vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){double t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=CVector4(vals[0],vals[1],vals[2],vals[3])*sc-CVector4(1,1,1,1)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 55: { double sc=3.0; CVector4 zz=fabs(z); double vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){double t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=CVector4(vals[0],vals[1],vals[2],vals[3])*sc-CVector4(1,1,1,1)*(sc-1.0)+CVector4(ta,tb,tc,td); aux.DE=aux.DE*sc+1.0; break; }
						case 56: { double sc=3.0; double qn=sqrt(z.x*z.x+z.y*z.y+z.z*z.z+z.w*z.w); z.w=qn*ta; CVector4 zz=fabs(z); double vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){double t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=CVector4(vals[0],vals[1],vals[2],vals[3])*sc-CVector4(1,1,1,1)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						...
						case 59: { double r4=sqrt(z.x*z.x+z.y*z.y+z.z*z.z+z.w*z.w); if(r4>1e-21){z/=r4;} double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 60: { double r2d=sqrt(z.x*z.x+z.y*z.y); double theta=atan2(z.y,z.x); z.x=r2d; z.y=theta*ta; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); z.x=z.x*cos(z.y/fmax(ta,0.01))-z.z*sin(z.y/fmax(ta,0.01)); aux.DE=aux.DE*sc+1.0; break; }
						case 61: { double R=fabs(ta)+0.5; double r2d=sqrt(z.x*z.x+z.y*z.y); z.x=r2d-R; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 62: { double theta4=ta*M_PI/180.0; double ca=cos(theta4),sa=sin(theta4); double nx=z.z*ca-z.w*sa; z.w=z.z*sa+z.w*ca; z.z=nx; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						...
						case 64: { double D=2.7268; double sc=pow(3.0,D/3.0); z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 65: { double h=sin(z.x*12.9898+z.y*78.233+z.z*45.164+(double)i*ta)*43758.5453; h=h-floor(h); double sc=3.0; z=fabs(z); double vals[3]={z.x,z.y,z.z}; if(h>0.5){if(vals[0]<vals[1]){double t=vals[0];vals[0]=vals[1];vals[1]=t;}} if(h>0.25){if(vals[0]<vals[2]){double t=vals[0];vals[0]=vals[2];vals[2]=t;}} if(h>0.75){if(vals[1]<vals[2]){double t=vals[1];vals[1]=vals[2];vals[2]=t;}} z=CVector4(vals[0],vals[1],vals[2],z.w)*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 66: { double sc=3.0; double sq32=0.866025; z=fabs(z); double d=z.x*sq32+z.y*0.5; if(d<0.0){z.x-=2.0*d*sq32;z.y-=d;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 67: { double sc=3.0; double a60=M_PI/3.0; for(int k=0;k<6;k++){double a=k*a60; double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x-=2.0*d*cos(a);z.y-=2.0*d*sin(a);}} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 68: { double sc=3.0; double a45=M_PI/4.0; for(int k=0;k<8;k++){double a=k*a45; double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x-=2.0*d*cos(a);z.y-=2.0*d*sin(a);}} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 69: { double sc=3.0; double a30=M_PI/6.0; for(int k=0;k<12;k++){double a=k*a30; double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x-=2.0*d*cos(a);z.y-=2.0*d*sin(a);}} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 70: { double sc=3.0; double a72=M_PI*2.0/5.0; for(int k=0;k<5;k++){double a=k*a72; double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x-=2.0*d*cos(a);z.y-=2.0*d*sin(a);}} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 71: { double sc=3.0; double a60=M_PI/3.0; for(int k=0;k<6;k++){double a=k*a60; double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x-=2.0*d*cos(a);z.y-=2.0*d*sin(a);}} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 72: { double sc=3.0; double a45=M_PI/4.0; for(int k=0;k<8;k++){double a=k*a45; double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x-=2.0*d*cos(a);z.y-=2.0*d*sin(a);}} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 73: { double sc=3.0; double a36=M_PI/5.0; for(int k=0;k<10;k++){double a=k*a36; double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x-=2.0*d*cos(a);z.y-=2.0*d*sin(a);}} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; break; }
						case 74: { int n=(int)fmax(ta,3.0); double sc=3.0; double aStep=M_PI*2.0/n; for(int k=0;k<n&&k<16;k++){double a=k*aStep; double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x-=2.0*d*cos(a);z.y-=2.0*d*sin(a);}} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*tb; aux.DE=aux.DE*sc+1.0; break; }
						case 75: { double sc=3.0; int n=(int)fmax(ta,3.0); double amp=fabs(tb); double theta=atan2(z.y,z.x); double r2d=sqrt(z.x*z.x+z.y*z.y); double star_r=r2d*(1.0+amp*cos(n*theta)); z.x=star_r*cos(theta); z.y=star_r*sin(theta); z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 76: { double sc=3.0; double theta=atan2(z.y,z.x); double r2d=sqrt(z.x*z.x+z.y*z.y); double heart_r=r2d*(1.0-sin(theta)*fabs(ta)); z.x=heart_r*cos(theta); z.y=heart_r*sin(theta); z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 77: { double spiral_a=ta*0.01*(double)i; double ca=cos(spiral_a),sa=sin(spiral_a); double nx=z.x*ca-z.y*sa; z.y=z.x*sa+z.y*ca; z.x=nx; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 78: { double r2=z.x*z.x+z.y*z.y; z.z+=ta*0.001*r2; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 79: { double r2d=sqrt(z.x*z.x+z.y*z.y); double taper_w=1.0-ta*0.01*fabs(r2d); z.x*=taper_w; z.y*=taper_w; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						...
						case 81: { double eps=fmax(fabs(ta),0.01); double sc=3.0; z=fabs(z); double t; if(z.x<z.y){double blend=0.5+0.5*tanh((z.y-z.x)/eps);t=z.x*(1.0-blend)+z.y*blend;z.y=z.y*(1.0-blend)+z.x*blend;z.x=t;} if(z.x<z.z){t=z.x;z.x=z.z;z.z=t;} if(z.y<z.z){t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 82: { double h=sin(z.x*ta*127.1+z.y*ta*311.7+z.z*ta*74.7)*43758.5453; h=h-floor(h); z+=CVector4(h,1.0-h,h*0.5,0)*tb*0.01; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 83: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)+CVector4(ta,tb,tc,0); aux.DE=aux.DE*sc+1.0; break; }
						case 84: { double period=fmax(fabs(ta),0.1); z=CVector4(fmod(fabs(z.x)+100.0*period,period)-period*0.5, fmod(fabs(z.y)+100.0*period,period)-period*0.5, fmod(fabs(z.z)+100.0*period,period)-period*0.5, z.w); double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 85: { double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<1e-21)r2=1e-21; double inv=ta*ta/r2; z*=inv; aux.DE*=inv; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 86: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); aux.DE*=(1.0+sf*exp(-r*ta)*tb); break; }
						case 87: { double sc=3.0; if(z.x+z.y<0.0){double t=z.x;z.x=-z.y;z.y=-t;} if(z.x+z.z<0.0){double t=z.x;z.x=-z.z;z.z=-t;} if(z.y+z.z<0.0){double t=z.y;z.y=-z.z;z.z=-t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0)*ta; aux.DE=aux.DE*sc+1.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 88: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; double koch_scale=2.0*ta; z.x=fabs(z.x)-koch_scale*0.5; z.y=fabs(z.y)-koch_scale*0.5; break; }
						case 89: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; if(z.x>0)z.x=-(z.x-1.0); break; }
						case 90: { double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; double fern_angle=ta*M_PI/180.0; double ca=cos(fern_angle),sa=sin(fern_angle); double nx=z.x*ca-z.z*sa; z.z=z.x*sa+z.z*ca; z.x=nx; z*=0.85*tb; aux.DE*=0.85*fabs(tb); break; }
						case 91: { double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; double shell=fabs(r-ta)-tb; aux.DE*=(1.0+sf*exp(-shell*shell*tc)); break; }
						case 92: { double period=fmax(fabs(ta),0.1); z=CVector4(fmod(z.x+100.0*period,period)-period*0.5, fmod(z.y+100.0*period,period)-period*0.5, fmod(z.z+100.0*period,period)-period*0.5, z.w); double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 93: { double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<1e-21)r2=1e-21; double inv=ta*ta/r2; z*=inv; aux.DE*=inv; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 94: { double clip=fabs(ta); z.x=fmin(fmax(z.x,-clip),clip); z.y=fmin(fmax(z.y,-clip),clip); z.z=fmin(fmax(z.z,-clip),clip); double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 95: { double lim=fabs(ta)+0.5; if(z.x>lim)z.x=2.0*lim-z.x; if(z.x<-lim)z.x=-2.0*lim-z.x; if(z.y>lim)z.y=2.0*lim-z.y; if(z.y<-lim)z.y=-2.0*lim-z.y; if(z.z>lim)z.z=2.0*lim-z.z; if(z.z<-lim)z.z=-2.0*lim-z.z; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 96: { double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=fabs(ta)*fabs(ta); double fixedR2=1.0; double m; if(r2<minR2)m=fixedR2/minR2; else if(r2<fixedR2)m=fixedR2/r2; else m=1.0; z*=m; aux.DE*=m; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 97: { double angle=ta*M_PI/180.0*(double)i; double ca=cos(angle),sa=sin(angle); double nx=z.x*ca-z.y*sa; z.y=z.x*sa+z.y*ca; z.x=nx; double sc=3.0; z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 98: { double sc=3.0*(1.0+sf*0.1*sin((double)i*ta)); z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*sc-CVector4(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*fabs(sc)+1.0; break; }
					...
					double sm_blend = std::min(std::max(mut.smWeight * mut.masterWeight, 0.0), 1.0);
				...
				{
					double sf = mut.blockifyFactor * mut.blockifyWeight * mut.masterWeight;
					double ta = mut.blockifyParamA, tb = mut.blockifyParamB;
					double tc = mut.blockifyParamC, td = mut.blockifyParamD;
					CVector4 z_orig_bl = z; double de_orig_bl = aux.DE;
					switch(mut.blockifyType) {
						case 1: { double grid = fmax(fabs(ta), 0.01); z.x = floor(z.x/grid)*grid; z.y = floor(z.y/grid)*grid; z.z = floor(z.z/grid)*grid; break; }
						case 2: { double grid = fmax(fabs(ta), 0.01); z.x = floor(z.x/grid+0.5)*grid; z.y = floor(z.y/grid+0.5)*grid; z.z = floor(z.z/grid+0.5)*grid; break; }
						case 3: { double grid = fmax(fabs(ta), 0.01); double qx=z.x-grid*floor(z.x/grid); double qy=z.y-grid*floor(z.y/grid); double qz=z.z-grid*floor(z.z/grid); double d=fmin(fmin(fmin(qx,grid-qx),fmin(qy,grid-qy)),fmin(qz,grid-qz)); aux.DE = fmin(aux.DE, d*sf); break; }
						case 4: { double grid = fmax(fabs(ta), 0.01); double blend=fmin(fmax(tb,0.0),1.0); double sx=floor(z.x/grid+0.5)*grid; double sy=floor(z.y/grid+0.5)*grid; double sz=floor(z.z/grid+0.5)*grid; z.x=z.x*(1.0-blend)+sx*blend; z.y=z.y*(1.0-blend)+sy*blend; z.z=z.z*(1.0-blend)+sz*blend; break; }
						case 5: { double grid = fmax(fabs(ta), 0.01); double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); r=floor(r/grid+0.5)*grid; double cur=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); if(cur>0.001){z.x*=r/cur;z.y*=r/cur;z.z*=r/cur;} break; }
						case 6: { double grid = fmax(fabs(ta), 0.01); double off=tb; z.x = floor((z.x+off)/grid)*grid-off; z.y = floor((z.y+off)/grid)*grid-off; z.z = floor((z.z+off)/grid)*grid-off; break; }
						case 7: { double gx=fmax(fabs(ta),0.01),gy=fmax(fabs(tb),0.01),gz=fmax(fabs(tc),0.01); z.x=floor(z.x/gx+0.5)*gx; z.y=floor(z.y/gy+0.5)*gy; z.z=floor(z.z/gz+0.5)*gz; break; }
						case 8: { double grid=fmax(fabs(ta),0.01); double t=fmin(fmax(sf,0.0),1.0); double sx=floor(z.x/grid+0.5)*grid; double sy=floor(z.y/grid+0.5)*grid; double sz=floor(z.z/grid+0.5)*grid; double progress=(double)i/fmax((double)(i+10),1.0); double blend=t*progress; z.x=z.x*(1.0-blend)+sx*blend; z.y=z.y*(1.0-blend)+sy*blend; z.z=z.z*(1.0-blend)+sz*blend; break; }
						default: break;
					}
					double bl_blend = std::min(std::max(mut.blockifyWeight * mut.masterWeight, 0.0), 1.0);
				...
				{
					double sf = mut.tileFactor * mut.tileWeight * mut.masterWeight;
					double ta = mut.tileParamA, tb = mut.tileParamB;
					double tc = mut.tileParamC, td = mut.tileParamD;
					CVector4 z_orig_ti = z; double de_orig_ti = aux.DE;
					switch(mut.tileType) {
						case 1: { double period = fmax(fabs(ta), 0.01); z.x = z.x - period*floor(z.x/period+0.5); z.y = z.y - period*floor(z.y/period+0.5); z.z = z.z - period*floor(z.z/period+0.5); break; }
						case 2: { double px=fmax(fabs(ta),0.01),py=fmax(fabs(tb),0.01),pz=fmax(fabs(tc),0.01); z.x=z.x-px*floor(z.x/px+0.5); z.y=z.y-py*floor(z.y/py+0.5); z.z=z.z-pz*floor(z.z/pz+0.5); break; }
						case 3: { double period=fmax(fabs(ta),0.01); z.x=fabs(z.x)-period*floor(fabs(z.x)/period); z.y=fabs(z.y)-period*floor(fabs(z.y)/period); z.z=fabs(z.z)-period*floor(fabs(z.z)/period); break; }
						case 4: { double period=fmax(fabs(ta),0.01); double half=period*0.5; double mx=fmod(z.x+half,period)-half; double my=fmod(z.y+half,period)-half; double mz=fmod(z.z+half,period)-half; z.x=mx;z.y=my;z.z=mz; break; }
						case 5: { double period=fmax(fabs(ta),0.01); double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); r=fmod(r,period); double cur=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); if(cur>0.001){z.x*=r/cur;z.y*=r/cur;z.z*=r/cur;} break; }
						case 6: { double angle=atan2(z.y,z.x); double period=fmax(fabs(ta),0.01)*M_PI/180.0; angle=fmod(angle+period*0.5,period)-period*0.5; double rr=sqrt(z.x*z.x+z.y*z.y); z.x=rr*cos(angle); z.y=rr*sin(angle); break; }
						case 7: { double period=fmax(fabs(ta),0.01); double blend=fmin(fmax(tb,0.0),1.0); double tx=z.x-period*floor(z.x/period+0.5); double ty=z.y-period*floor(z.y/period+0.5); double tz=z.z-period*floor(z.z/period+0.5); z.x=z.x*(1.0-blend)+tx*blend; z.y=z.y*(1.0-blend)+ty*blend; z.z=z.z*(1.0-blend)+tz*blend; break; }
						case 8: { double period=fmax(fabs(ta),0.01); int ix=(int)floor(z.x/period); int iy=(int)floor(z.y/period); z.x=z.x-period*floor(z.x/period+0.5); z.y=z.y-period*floor(z.y/period+0.5); z.z=z.z-period*floor(z.z/period+0.5); if((ix+iy)%2!=0){z.x=-z.x;z.y=-z.y;} break; }
						default: break;
					}
					double ti_blend = std::min(std::max(mut.tileWeight * mut.masterWeight, 0.0), 1.0);
				...
				{
					double sf = mut.clampFactor * mut.clampWeight * mut.masterWeight;
					double ta = mut.clampParamA, tb = mut.clampParamB;
					double tc = mut.clampParamC, td = mut.clampParamD;
					CVector4 z_orig_cl = z; double de_orig_cl = aux.DE;
...
case 1: { z.x=fmax(fmin(z.x,ta),-ta); z.y=fmax(fmin(z.y,ta),-ta); z.z=fmax(fmin(z.z,ta),-ta); aux.DE*=sf; break; }
						...
						case 3: { double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); if(r>ta){double s=ta/r; z*=s; aux.DE*=s*sf;} break; }
						case 4: { double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<ta*ta){double s=ta*ta/fmax(r2,1e-21); z*=s; aux.DE*=s;} break; }
						case 5: { if(z.x>ta)z.x=2.0*ta-z.x; if(z.x<-ta)z.x=-2.0*ta-z.x; if(z.y>ta)z.y=2.0*ta-z.y; if(z.y<-ta)z.y=-2.0*ta-z.y; if(z.z>ta)z.z=2.0*ta-z.z; if(z.z<-ta)z.z=-2.0*ta-z.z; break; }
						...
						case 8: { double lim=ta*(1.0+tb*(double)i/250.0); z.x=fmax(fmin(z.x,lim),-lim); z.y=fmax(fmin(z.y,lim),-lim); z.z=fmax(fmin(z.z,lim),-lim); break; }
						case 9: { double lim=ta*fabs(sin(tb*(double)i)); z.x=fmax(fmin(z.x,fmax(lim,0.01)),-fmax(lim,0.01)); z.y=fmax(fmin(z.y,fmax(lim,0.01)),-fmax(lim,0.01)); z.z=fmax(fmin(z.z,fmax(lim,0.01)),-fmax(lim,0.01)); break; }
						case 10: { double lim=ta*exp(-tb*(double)i/100.0); z.x=fmax(fmin(z.x,fmax(lim,0.01)),-fmax(lim,0.01)); z.y=fmax(fmin(z.y,fmax(lim,0.01)),-fmax(lim,0.01)); z.z=fmax(fmin(z.z,fmax(lim,0.01)),-fmax(lim,0.01)); break; }
						case 11: { double k=fmax(tb,0.01); z.x=z.x-fmax(0.0,fmin(1.0,0.5+(z.x-ta)/(2.0*k)))*(z.x-ta); z.y=z.y-fmax(0.0,fmin(1.0,0.5+(z.y-ta)/(2.0*k)))*(z.y-ta); z.z=z.z-fmax(0.0,fmin(1.0,0.5+(z.z-ta)/(2.0*k)))*(z.z-ta); break; }
						case 12: { if(fabs(z.x)>ta){double s=z.x>0?1:-1; z.x=s*(ta-(z.x-s*ta)*(z.x-s*ta)*tb);} if(fabs(z.y)>ta){double s=z.y>0?1:-1; z.y=s*(ta-(z.y-s*ta)*(z.y-s*ta)*tb);} if(fabs(z.z)>ta){double s=z.z>0?1:-1; z.z=s*(ta-(z.z-s*ta)*(z.z-s*ta)*tb);} break; }
						case 13: { double rxy=sqrt(z.x*z.x+z.y*z.y); if(rxy>ta){double s=ta/rxy; z.x*=s; z.y*=s; aux.DE*=s;} break; }
						case 14: { double rxy=sqrt(z.x*z.x+z.y*z.y); double lim=ta+tb*fabs(z.z); if(rxy>lim){double s=lim/rxy; z.x*=s; z.y*=s; aux.DE*=s;} break; }
						case 15: { double d=sqrt((z.x/fmax(ta,0.01))*(z.x/fmax(ta,0.01))+(z.y/fmax(tb,0.01))*(z.y/fmax(tb,0.01))+(z.z/fmax(tc,0.01))*(z.z/fmax(tc,0.01))); if(d>1.0){z/=d; aux.DE/=d;} break; }
						case 16: { double rxy=sqrt(z.x*z.x+z.y*z.y)-ta; double dt=sqrt(rxy*rxy+z.z*z.z); if(dt>tb){double s=tb/dt; rxy*=s; z.z*=s; double newR=rxy+ta; if(sqrt(z.x*z.x+z.y*z.y)>1e-10){double scale=newR/sqrt(z.x*z.x+z.y*z.y); z.x*=scale; z.y*=scale;} aux.DE*=s;} break; }
						case 17: { if(z.x>ta) z.x=ta; break; }
						case 18: { if(z.x<-ta) z.x=-ta; break; }
						case 19: { if(z.z>ta) z.z=ta; break; }
						case 20: { double p=fmax(ta,0.1); z.x=fmod(z.x+p,2.0*p)-p; z.y=fmod(z.y+p,2.0*p)-p; z.z=fmod(z.z+p,2.0*p)-p; break; }
						case 21: { double p=fmax(ta,0.1); z.x=2.0*fabs(z.x/(2.0*p)-floor(z.x/(2.0*p)+0.5))*p; z.y=2.0*fabs(z.y/(2.0*p)-floor(z.y/(2.0*p)+0.5))*p; z.z=2.0*fabs(z.z/(2.0*p)-floor(z.z/(2.0*p)+0.5))*p; break; }
						case 22: { z.x=ta*(2.0/(1.0+exp(-2.0*z.x/fmax(ta,0.01)))-1.0); z.y=ta*(2.0/(1.0+exp(-2.0*z.y/fmax(ta,0.01)))-1.0); z.z=ta*(2.0/(1.0+exp(-2.0*z.z/fmax(ta,0.01)))-1.0); break; }
						case 23: { z.x=ta*atan(z.x/fmax(ta,0.01))*(2.0/M_PI); z.y=ta*atan(z.y/fmax(ta,0.01))*(2.0/M_PI); z.z=ta*atan(z.z/fmax(ta,0.01))*(2.0/M_PI); break; }
						case 24: { double k24=1.0/fmax(ta,0.01); z.x=ta*tanh(k24*z.x*1.128); z.y=ta*tanh(k24*z.y*1.128); z.z=ta*tanh(k24*z.z*1.128); break; }
						case 25: { double p25=fmax(tb,0.1); if(fabs(z.x)>ta)z.x=z.x>0?ta*pow(ta/z.x,p25):-ta*pow(ta/fabs(z.x),p25); if(fabs(z.y)>ta)z.y=z.y>0?ta*pow(ta/z.y,p25):-ta*pow(ta/fabs(z.y),p25); if(fabs(z.z)>ta)z.z=z.z>0?ta*pow(ta/z.z,p25):-ta*pow(ta/fabs(z.z),p25); break; }
						case 26: { double lim26=ta*(1.0+tb*sin(aux.color*tc)); z.x=fmax(fmin(z.x,lim26),-lim26); z.y=fmax(fmin(z.y,lim26),-lim26); z.z=fmax(fmin(z.z,lim26),-lim26); break; }
						case 27: { double lim27=ta*(1.0+tb*log(1.0+fabs(aux.DE))); z.x=fmax(fmin(z.x,lim27),-lim27); z.y=fmax(fmin(z.y,lim27),-lim27); z.z=fmax(fmin(z.z,lim27),-lim27); break; }
						case 28: { double lim28=ta+tb*z.x; z.y=fmax(fmin(z.y,fabs(lim28)),-fabs(lim28)); z.z=fmax(fmin(z.z,fabs(lim28)),-fabs(lim28)); break; }
						case 29: { double theta29=atan2(z.y,z.x); double r29=sqrt(z.x*z.x+z.y*z.y); if(r29>ta){r29=ta; z.x=r29*cos(theta29); z.y=r29*sin(theta29); aux.DE*=ta/fmax(sqrt(z.x*z.x+z.y*z.y),1e-10);} break; }
						case 30: { double p30=fmax(ta,0.1); z.x=floor(z.x/p30+0.5)*p30; z.y=floor(z.y/p30+0.5)*p30; z.z=floor(z.z/p30+0.5)*p30; break; }
						case 31: { double r31=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); if(r31<ta&&r31>1e-15){double s=ta/r31; z*=s; aux.DE*=s;} break; }
						case 32: { double r32=sqrt(z.x*z.x+z.y*z.y); if(r32>ta&&r32<tb){}else if(r32<=ta){double s=ta/fmax(r32,1e-10); z.x*=s; z.y*=s;} else{double s=tb/r32; z.x*=s; z.y*=s;} break; }
						case 33: { double n33=sin(z.x*12.9898+z.y*78.233+z.z*45.164)*43758.5453; n33=n33-floor(n33); double lim33=ta*(1.0+tb*(n33-0.5)); z.x=fmax(fmin(z.x,lim33),-lim33); z.y=fmax(fmin(z.y,lim33),-lim33); z.z=fmax(fmin(z.z,lim33),-lim33); break; }
						case 34: { double theta34=atan2(z.y,z.x); double r34=sqrt(z.x*z.x+z.y*z.y); double spiralLim=ta+tb*theta34/(2.0*M_PI); if(r34>spiralLim){z.x*=spiralLim/r34; z.y*=spiralLim/r34;} break; }
						case 35: { double waveLim=ta+tb*sin(tc*z.x)*sin(tc*z.y); z.z=fmax(fmin(z.z,waveLim),-waveLim); break; }
						case 36: { if(fabs(z.x)>ta){double t=z.x; z.x=z.y; z.y=t;} if(fabs(z.y)>ta){double t=z.y; z.y=z.z; z.z=t;} break; }
						case 37: { if(z.x>ta)z.x=2.0*ta-z.x; if(z.y>ta)z.y=2.0*ta-z.y; if(z.z>ta)z.z=2.0*ta-z.z; break; }
						case 38: { double p38=fmax(2.0*ta,0.01); z.x=fabs(fmod(z.x+ta,p38)-ta); z.y=fabs(fmod(z.y+ta,p38)-ta); z.z=fabs(fmod(z.z+ta,p38)-ta); break; }
						case 39: { double s39=z.x>0?1:-1; z.x=s39*fmin(fabs(z.x),ta*log(1.0+fabs(z.x)/fmax(ta,0.01))); s39=z.y>0?1:-1; z.y=s39*fmin(fabs(z.y),ta*log(1.0+fabs(z.y)/fmax(ta,0.01))); s39=z.z>0?1:-1; z.z=s39*fmin(fabs(z.z),ta*log(1.0+fabs(z.z)/fmax(ta,0.01))); break; }
						case 40: { double step40=fmax(tb,0.01); z.x=floor(z.x/step40+0.5)*step40; z.x=fmax(fmin(z.x,ta),-ta); z.y=floor(z.y/step40+0.5)*step40; z.y=fmax(fmin(z.y,ta),-ta); z.z=floor(z.z/step40+0.5)*step40; z.z=fmax(fmin(z.z,ta),-ta); break; }
						case 41: { for(int k=0;k<3;k++){if(z.x>ta)z.x=2.0*ta-z.x; if(z.x<-ta)z.x=-2.0*ta-z.x; if(z.y>ta)z.y=2.0*ta-z.y; if(z.y<-ta)z.y=-2.0*ta-z.y; if(z.z>ta)z.z=2.0*ta-z.z; if(z.z<-ta)z.z=-2.0*ta-z.z; ta*=tb;} break; }
						case 42: { double r42=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); if(r42>ta){z*=ta/r42; aux.DE*=ta/r42;} if(r42<tb&&r42>1e-15){z*=tb/r42; aux.DE*=tb/r42;} break; }
						case 43: { z.x=ta*z.x/sqrt(z.x*z.x+tb*tb); z.y=ta*z.y/sqrt(z.y*z.y+tb*tb); z.z=ta*z.z/sqrt(z.z*z.z+tb*tb); break; }
						case 44: { z.x=fabs(z.x); z.y=fabs(z.y); z.z=fabs(z.z); z.x=fmin(z.x,ta); z.y=fmin(z.y,ta); z.z=fmin(z.z,ta); break; }
						case 45: { double k45=fmax(tb,0.01); if(fabs(z.x)>ta){double d=(fabs(z.x)-ta)/k45; double s=z.x>0?1:-1; z.x=s*(ta+k45*d/(1.0+d*d));} if(fabs(z.y)>ta){double d=(fabs(z.y)-ta)/k45; double s=z.y>0?1:-1; z.y=s*(ta+k45*d/(1.0+d*d));} if(fabs(z.z)>ta){double d=(fabs(z.z)-ta)/k45; double s=z.z>0?1:-1; z.z=s*(ta+k45*d/(1.0+d*d));} break; }
						case 46: { double sum46=fabs(z.x)+fabs(z.y)+fabs(z.z); if(sum46>ta){double s=ta/sum46; z*=s; aux.DE*=s;} break; }
						case 47: { if(fabs(z.x)+fabs(z.y)>ta){double s=ta/(fabs(z.x)+fabs(z.y)); z.x*=s; z.y*=s;} break; }
						case 48: { double rxy48=sqrt(z.x*z.x+z.y*z.y); if(rxy48>ta){z.x*=ta/rxy48; z.y*=ta/rxy48;} z.z=fmax(fmin(z.z,tb),-tb); break; }
						case 49: { double capH49=fmax(0.0,fmin(ta,z.z)); double rd49=sqrt(z.x*z.x+z.y*z.y+(z.z-capH49)*(z.z-capH49)); if(rd49>tb){double s=tb/rd49; z.x*=s; z.y*=s; z.z=capH49+(z.z-capH49)*s; aux.DE*=s;} break; }
						case 50: { double p50=fmax(tc,0.5); double d50=pow(pow(fabs(z.x/fmax(ta,0.01)),p50)+pow(fabs(z.y/fmax(ta,0.01)),p50)+pow(fabs(z.z/fmax(tb,0.01)),p50),1.0/p50); if(d50>1.0){z/=d50; aux.DE/=d50;} break; }
						default: break;
					}
					double cl_blend = std::min(std::max(mut.clampWeight * mut.masterWeight, 0.0), 1.0);
				...
				{
					double sf = mut.jbFactor * mut.jbWeight * mut.masterWeight;
					double ta = mut.jbParamA, tb = mut.jbParamB;
					double tc = mut.jbParamC, td = mut.jbParamD;
					CVector4 z_orig_jb = z; double de_orig_jb = aux.DE;
					switch(mut.jbType) {
case 1: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2jb=z.x*z.x+z.y*z.y+z.z*z.z; double minR2jb=tb*tb; double fixedR2jb=1.0; if(r2jb<minR2jb){z*=fixedR2jb/minR2jb; aux.DE*=fixedR2jb/minR2jb;} else if(r2jb<fixedR2jb){z*=fixedR2jb/r2jb; aux.DE*=fixedR2jb/r2jb;} z=z*tc+CVector4(td,td,td,0); aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 2: { double a2=ta*M_PI/180.0; double c2=cos(a2),s2=sin(a2); double nx2=z.x*c2-z.y*s2; z.y=z.x*s2+z.y*c2; z.x=nx2; z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; double r2jb2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2jb2<0.25){z*=4.0; aux.DE*=4.0;} else if(r2jb2<1.0){z*=1.0/r2jb2; aux.DE*=1.0/r2jb2;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 3: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2mb=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=tb*tb; if(r2mb<minR2){z*=1.0/minR2; aux.DE*=1.0/minR2;} else if(r2mb<1.0){z*=1.0/r2mb; aux.DE*=1.0/r2mb;} z=z*tc+aux.const_c; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 4: { z=fabs(z+CVector4(ta,ta,ta,0))-CVector4(ta,ta,ta,0); double r2ab=z.x*z.x+z.y*z.y+z.z*z.z; double minR2ab=tb*tb; if(r2ab<minR2ab){z*=1.0/minR2ab; aux.DE*=1.0/minR2ab;} else if(r2ab<1.0){z*=1.0/r2ab; aux.DE*=1.0/r2ab;} z=z*tc+aux.const_c; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 5: { double k5=fmax(td,0.01); z.x=z.x-k5*tanh((z.x-ta)/k5)+k5*tanh((z.x+ta)/k5)-z.x; z.y=z.y-k5*tanh((z.y-ta)/k5)+k5*tanh((z.y+ta)/k5)-z.y; z.z=z.z-k5*tanh((z.z-ta)/k5)+k5*tanh((z.z+ta)/k5)-z.z; double r2sb=z.x*z.x+z.y*z.y+z.z*z.z; if(r2sb<tb*tb){z*=1.0/(tb*tb); aux.DE*=1.0/(tb*tb);} else if(r2sb<1.0){z*=1.0/r2sb; aux.DE*=1.0/r2sb;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 6: { z.x=fabs(z.x)-ta; z.y=fabs(z.y)-ta; z.z=fabs(z.z)-ta; double r2af=z.x*z.x+z.y*z.y+z.z*z.z; if(r2af<tb*tb&&r2af>1e-21){z*=1.0/(r2af); aux.DE*=1.0/(r2af);} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 7: { if(z.x>ta)z.x=ta-z.x; if(z.x<-ta)z.x=-ta-z.x; if(z.y>ta)z.y=ta-z.y; if(z.y<-ta)z.y=-ta-z.y; if(z.z>ta)z.z=ta-z.z; if(z.z<-ta)z.z=-ta-z.z; double r2cf=z.x*z.x+z.y*z.y+z.z*z.z; double minR=tb*tb; if(r2cf<minR){z*=1.0/minR; aux.DE*=1.0/minR;} else if(r2cf<1.0){z*=1.0/r2cf; aux.DE*=1.0/r2cf;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 8: { for(int k=0;k<3;k++){z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z;} double r2mf=z.x*z.x+z.y*z.y+z.z*z.z; if(r2mf<tb*tb){z*=1.0/(tb*tb); aux.DE*=1.0/(tb*tb);} else if(r2mf<1.0){z*=1.0/r2mf; aux.DE*=1.0/r2mf;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 9: { if(z.x+z.y<0.0){double t=z.x;z.x=-z.y;z.y=-t;} if(z.x+z.z<0.0){double t=z.x;z.x=-z.z;z.z=-t;} if(z.y+z.z<0.0){double t=z.y;z.y=-z.z;z.z=-t;} z=z*ta-CVector4(tb,tb,tb,0)*(ta-1.0); aux.DE=aux.DE*fabs(ta)+1.0; break; }
						case 10: { z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z=z*ta-CVector4(1,1,1,0)*(ta-1.0)*tb; aux.DE=aux.DE*fabs(ta)+1.0; break; }
						case 11: { z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z=z*tc+CVector4(td,td,td,0); aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 12: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 13: { z=z*ta+CVector4(tb,tc,td,0); z.x=fabs(z.x+1.0)-fabs(z.x-1.0)-z.x; z.y=fabs(z.y+1.0)-fabs(z.y-1.0)-z.y; z.z=fabs(z.z+1.0)-fabs(z.z-1.0)-z.z; aux.DE=aux.DE*fabs(ta)+1.0; break; }
						case 14: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tc)-fabs(z.z-tc)-z.z; double r2df=z.x*z.x+z.y*z.y+z.z*z.z; if(r2df<0.25){z*=4.0;aux.DE*=4.0;} else if(r2df<1.0){z*=1.0/r2df;aux.DE*=1.0/r2df;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 15: { z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; double r2mh=z.x*z.x+z.y*z.y+z.z*z.z; if(r2mh<tb*tb){z*=1.0/(tb*tb);aux.DE*=1.0/(tb*tb);} else if(r2mh<1.0){z*=1.0/r2mh;aux.DE*=1.0/r2mh;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 16: { double tw16=ta*M_PI/180.0*z.z; double c16=cos(tw16),s16=sin(tw16); double nx16=z.x*c16-z.y*s16; z.y=z.x*s16+z.y*c16; z.x=nx16; z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 17: { double sc17=ta+tb*sin((double)i*tc); z.x=fabs(z.x+1.0)-fabs(z.x-1.0)-z.x; z.y=fabs(z.y+1.0)-fabs(z.y-1.0)-z.y; z.z=fabs(z.z+1.0)-fabs(z.z-1.0)-z.z; z*=sc17; aux.DE=aux.DE*fabs(sc17)+1.0; break; }
						case 18: { double r218=z.x*z.x+z.y*z.y+z.z*z.z; double minR218=ta*ta; double fixR218=tb*tb; if(r218<minR218){z*=fixR218/minR218;aux.DE*=fixR218/minR218;} else if(r218<fixR218){z*=fixR218/r218;aux.DE*=fixR218/r218;} double r218b=z.x*z.x+z.y*z.y+z.z*z.z; double minR218b=tc*tc; if(r218b<minR218b){z*=1.0/minR218b;aux.DE*=1.0/minR218b;} else if(r218b<1.0){z*=1.0/r218b;aux.DE*=1.0/r218b;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 19: { double rxy19=sqrt(z.x*z.x+z.y*z.y); if(rxy19<ta&&rxy19>1e-15){z.x*=ta/rxy19; z.y*=ta/rxy19; aux.DE*=ta/rxy19;} z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 20: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; double r2xz=z.x*z.x+z.z*z.z; if(r2xz<tb*tb){z*=1.0/(tb*tb);aux.DE*=1.0/(tb*tb);} else if(r2xz<1.0){z*=1.0/r2xz;aux.DE*=1.0/r2xz;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 21: { z=fabs(z)-CVector4(ta,ta,ta,0); z+=aux.const_c*tb; double r2ao=z.x*z.x+z.y*z.y+z.z*z.z; if(r2ao<tc*tc){z*=1.0/(tc*tc);aux.DE*=1.0/(tc*tc);} else if(r2ao<1.0){z*=1.0/r2ao;aux.DE*=1.0/r2ao;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 22: { if(i%2==0){z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z;} else{z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;}} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 23: { double r2so=z.x*z.x+z.y*z.y+z.z*z.z; double minR2so=ta*ta; double fixedR2so=tb*tb; if(r2so<minR2so){double m=fixedR2so/minR2so; z*=m; aux.DE*=m;} else if(r2so<fixedR2so){double m=fixedR2so/r2so; z*=m; aux.DE*=m;} break; }
						case 24: { if(z.x+z.y<0.0){double t=-z.y;z.y=-z.x;z.x=t;} if(z.x+z.z<0.0){double t=-z.z;z.z=-z.x;z.x=t;} if(z.y+z.z<0.0){double t=-z.z;z.z=-z.y;z.y=t;} z*=ta; z-=CVector4(tb,tb,tb,0)*(ta-1.0); aux.DE=aux.DE*fabs(ta)+1.0; break; }
						case 25: { z.x=fabs(fabs(z.x+ta)-tb)-ta; z.y=fabs(fabs(z.y+ta)-tb)-ta; z.z=fabs(fabs(z.z+ta)-tb)-ta; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 26: { double r2rb=z.x*z.x+z.y*z.y+z.z*z.z; double theta26=atan2(z.y,z.x); double phi26=atan2(sqrt(z.x*z.x+z.y*z.y),z.z); double rr26=sqrt(r2rb); rr26=fabs(rr26+ta)-fabs(rr26-ta)-rr26; z.x=rr26*sin(phi26)*cos(theta26); z.y=rr26*sin(phi26)*sin(theta26); z.z=rr26*cos(phi26); z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 27: { double p27=fmax(tb,0.5); z.x=z.x>0?pow(fabs(z.x),p27):-pow(fabs(z.x),p27); z.y=z.y>0?pow(fabs(z.y),p27):-pow(fabs(z.y),p27); z.z=z.z>0?pow(fabs(z.z),p27):-pow(fabs(z.z),p27); z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 28: { if(i%3==0){z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x;} else if(i%3==1){z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y;} else{z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z;} double r2il=z.x*z.x+z.y*z.y+z.z*z.z; if(r2il<tb*tb){z*=1.0/(tb*tb);aux.DE*=1.0/(tb*tb);} else if(r2il<1.0){z*=1.0/r2il;aux.DE*=1.0/r2il;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 29: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2mj=z.x*z.x+z.y*z.y+z.z*z.z; if(r2mj<0.25){z*=4.0;aux.DE*=4.0;} else if(r2mj<1.0){z*=1.0/r2mj;aux.DE*=1.0/r2mj;} z=z*tc+CVector4(td,td,td,0); aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 30: { z*=ta; aux.DE=aux.DE*fabs(ta)+1.0; double a30=tb*M_PI/180.0; double c30=cos(a30),s30=sin(a30); double nx30=z.x*c30-z.y*s30; z.y=z.x*s30+z.y*c30; z.x=nx30; z.x=fabs(z.x+tc)-fabs(z.x-tc)-z.x; z.y=fabs(z.y+tc)-fabs(z.y-tc)-z.y; z.z=fabs(z.z+tc)-fabs(z.z-tc)-z.z; break; }
						case 31: { double bp31=z.x*z.x+z.y*z.y; if(bp31>1e-21){double inv=ta*ta/bp31; z.x*=inv; z.y*=inv; aux.DE*=inv;} z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 32: { double r2cb=z.x*z.x+z.y*z.y+z.z*z.z+z.w*z.w; if(r2cb>1e-21){z*=ta/r2cb; aux.DE*=ta/r2cb;} z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 33: { double lim33=ta*(1.0+tb*(double)i/250.0); z.x=fabs(z.x+lim33)-fabs(z.x-lim33)-z.x; z.y=fabs(z.y+lim33)-fabs(z.y-lim33)-z.y; z.z=fabs(z.z+lim33)-fabs(z.z-lim33)-z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 34: { double n34=sin(z.x*12.9898+z.y*78.233)*43758.5453; n34=n34-floor(n34); double lim34=ta+tb*(n34-0.5); z.x=fabs(z.x+lim34)-fabs(z.x-lim34)-z.x; z.y=fabs(z.y+lim34)-fabs(z.y-lim34)-z.y; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 35: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; z.w=fabs(z.w+ta)-fabs(z.w-ta)-z.w; double r24d=z.x*z.x+z.y*z.y+z.z*z.z+z.w*z.w; if(r24d<tb*tb){z*=1.0/(tb*tb);aux.DE*=1.0/(tb*tb);} else if(r24d<1.0){z*=1.0/r24d;aux.DE*=1.0/r24d;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 36: { double q36x=z.x; double q36y=z.y*0.866+z.x*0.5; if(q36y>ta){q36y=2.0*ta-q36y;} if(q36x>ta){q36x=2.0*ta-q36x;} z.x=q36x; z.y=(q36y-z.x*0.5)/0.866; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 37: { double d37x=fabs(z.x)-ta; double d37y=fabs(z.y)-ta; double d37z=fabs(z.z)-ta; if(d37x>0&&d37y>0){z.x*=ta/(ta+d37x); z.y*=ta/(ta+d37y);} if(d37y>0&&d37z>0){z.y*=ta/(ta+d37y); z.z*=ta/(ta+d37z);} if(d37x>0&&d37z>0){z.x*=ta/(ta+d37x); z.z*=ta/(ta+d37z);} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 38: { z.x+=tb*sin(ta*z.y)*cos(ta*z.z); z.y+=tb*sin(ta*z.z)*cos(ta*z.x); z.z+=tb*cos(ta*z.y)*sin(ta*z.x); z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 39: { z.x=fabs(cos(z.x*ta))*tb; z.y=fabs(cos(z.y*ta))*tb; z.z=fabs(cos(z.z*ta))*tb; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 40: { z.x=z.x>0?log(1.0+z.x*ta):-log(1.0-z.x*ta); z.y=z.y>0?log(1.0+z.y*ta):-log(1.0-z.y*ta); z.z=z.z>0?log(1.0+z.z*ta):-log(1.0-z.z*ta); z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 41: { double lim41=ta; z.x=z.x>lim41?lim41-(z.x-lim41)*(z.x-lim41)*tb:z.x<-lim41?-lim41+(z.x+lim41)*(z.x+lim41)*tb:z.x; z.y=z.y>lim41?lim41-(z.y-lim41)*(z.y-lim41)*tb:z.y<-lim41?-lim41+(z.y+lim41)*(z.y+lim41)*tb:z.y; z.z=z.z>lim41?lim41-(z.z-lim41)*(z.z-lim41)*tb:z.z<-lim41?-lim41+(z.z+lim41)*(z.z+lim41)*tb:z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 42: { z.x=fmin(fmax(z.x,-ta),ta); z.y=fmin(fmax(z.y,-ta),ta); z.z=fmin(fmax(z.z,-ta),ta); z.x=fmax(fmin(z.x,tb),-tb); z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 43: { if(fabs(z.x)>ta)z.x=z.x>0?ta:-ta; if(fabs(z.y)>ta)z.y=z.y>0?ta:-ta; if(fabs(z.z)>ta)z.z=z.z>0?ta:-ta; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 44: { double p44=2.0*ta; z.x=z.x-p44*floor((z.x+ta)/p44); z.y=z.y-p44*floor((z.y+ta)/p44); z.z=z.z-p44*floor((z.z+ta)/p44); z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 45: { double r45=sqrt(z.x*z.x+z.y*z.y); double th45=atan2(z.y,z.x); th45=fabs(th45+ta)-fabs(th45-ta)-th45; z.x=r45*cos(th45); z.y=r45*sin(th45); z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 46: { double lim46=ta*(1.0+tb*log(1.0+fabs(aux.DE))); z.x=fabs(z.x+lim46)-fabs(z.x-lim46)-z.x; z.y=fabs(z.y+lim46)-fabs(z.y-lim46)-z.y; z.z=fabs(z.z+lim46)-fabs(z.z-lim46)-z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 47: { double lim47=ta*(1.0+tb*sin(aux.color*tc)); z.x=fabs(z.x+lim47)-fabs(z.x-lim47)-z.x; z.y=fabs(z.y+lim47)-fabs(z.y-lim47)-z.y; z.z=fabs(z.z+lim47)-fabs(z.z-lim47)-z.z; z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 48: { double la=ta,lb=tb; for(int k=0;k<3;k++){z.x=fabs(z.x+la)-fabs(z.x-la)-z.x; z.y=fabs(z.y+la)-fabs(z.y-la)-z.y; z.z=fabs(z.z+la)-fabs(z.z-la)-z.z; la*=lb;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 49: { z.x=z.x>ta?z.x-2.0*ta:z.x<-ta?z.x+2.0*ta:z.x; z.y=z.y>ta?z.y-2.0*ta:z.y<-ta?z.y+2.0*ta:z.y; z.z=z.z>ta?z.z-2.0*ta:z.z<-ta?z.z+2.0*ta:z.z; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 50: { z=fabs(z); if(z.x-z.y<0.0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){double t=z.y;z.y=z.z;z.z=t;} z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2ifs=z.x*z.x+z.y*z.y+z.z*z.z; if(r2ifs<tb*tb){z*=1.0/(tb*tb);aux.DE*=1.0/(tb*tb);} else if(r2ifs<1.0){z*=1.0/r2ifs;aux.DE*=1.0/r2ifs;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						default: break;
					}
					double jb_blend = std::min(std::max(mut.jbWeight * mut.masterWeight, 0.0), 1.0);
					z = z_orig_jb + (z - z_orig_jb) * jb_blend;
					aux.DE = de_orig_jb + (aux.DE - de_orig_jb) * jb_blend;
				}

				// v7.14 — T Mandalay system (per-section iteration range)
				if (mut.mdEnabled && i >= mut.mdIterStart && i < mut.mdIterStop && mut.mdType != 0)
				{
					double sf = mut.mdFactor * mut.mdWeight * mut.masterWeight;
					double ta = mut.mdParamA, tb = mut.mdParamB;
					double tc = mut.mdParamC, td = mut.mdParamD;
					CVector4 z_orig_md = z; double de_orig_md = aux.DE;
					switch (mut.mdType)
					{
						case 1: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=tb*tb; if(r2<minR2){z*=1.0/minR2;aux.DE*=1.0/minR2;} else if(r2<1.0){z*=1.0/r2;aux.DE*=1.0/r2;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; z+=CVector4(td,td,td,0); break; }
						case 2: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tc)-fabs(z.z-tc)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<0.25){z*=4.0;aux.DE*=4.0;} else if(r2<1.0){z/=r2;aux.DE/=r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 3: { double rxy=sqrt(z.x*z.x+z.y*z.y); if(rxy>ta){double s=ta/rxy; z.x*=s; z.y*=s; aux.DE*=s;} double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb*tb){z*=1.0/(tb*tb);aux.DE*=1.0/(tb*tb);} else if(r2<1.0){z/=r2;aux.DE/=r2;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 4: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double fixR=tb*tb; double minR=tc*tc; if(r2<minR){z*=fixR/minR;aux.DE*=fixR/minR;} else if(r2<fixR){z*=fixR/r2;aux.DE*=fixR/r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 5: { z=fabs(z+CVector4(ta,ta,ta,0))-CVector4(ta,ta,ta,0); double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb){z*=tc/tb;aux.DE*=tc/tb;} else if(r2<tc){z*=tc/r2;aux.DE*=tc/r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 6: { if(z.x+z.y<0){double t=z.x;z.x=-z.y;z.y=-t;} if(z.x+z.z<0){double t=z.x;z.x=-z.z;z.z=-t;} if(z.y+z.z<0){double t=z.y;z.y=-z.z;z.z=-t;} z=z*ta-CVector4(tb,tb,tb,0)*(ta-1.0); aux.DE=aux.DE*fabs(ta)+1.0; break; }
						case 7: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb*tb){double s=tc/(tb*tb); z*=s;aux.DE*=s;} else if(r2<tc){double s=tc/r2; z*=s;aux.DE*=s;} z.z=td*z.z; aux.DE*=fabs(td); break; }
						case 8: { double a8=ta*M_PI/180.0; double c8=cos(a8),s8=sin(a8); double nx=z.x*c8-z.y*s8; z.y=z.x*s8+z.y*c8; z.x=nx; z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<0.25){z*=4.0;aux.DE*=4.0;} else if(r2<1.0){z/=r2;aux.DE/=r2;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 9: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); if(r<tb){z*=tc/(tb*tb);aux.DE*=tc/(tb*tb);} else if(r<sqrt(tc)){z*=tc/(r*r);aux.DE*=tc/(r*r);} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 10: { z=fabs(z)-CVector4(ta,ta,ta,0); if(z.x<0)z.x=0; if(z.y<0)z.y=0; if(z.z<0)z.z=0; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb*tb){z*=1.0/(tb*tb);aux.DE*=1.0/(tb*tb);} else if(r2<1.0){z/=r2;aux.DE/=r2;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; z+=CVector4(td,td,td,0); break; }
						case 11: { double rxy=sqrt(z.x*z.x+z.y*z.y); double minR=ta; if(rxy<minR&&rxy>1e-21){double s=minR/rxy; z.x*=s;z.y*=s;aux.DE*=s;} z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 12: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double sph=fmax(tb*tb/fmax(r2,1e-21),1.0); z*=sph*tc; aux.DE=aux.DE*sph*fabs(tc)+1.0; break; }
						case 13: { if(z.x-z.y<0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0){double t=z.y;z.y=z.z;z.z=t;} z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tc*tc){z*=1.0/(tc*tc);aux.DE*=1.0/(tc*tc);} else if(r2<1.0){z/=r2;aux.DE/=r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 14: { z.x=ta*tanh(z.x/fmax(ta,1e-10)); z.y=ta*tanh(z.y/fmax(ta,1e-10)); z.z=ta*tanh(z.z/fmax(ta,1e-10)); double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb*tb){z*=tc/(tb*tb);aux.DE*=tc/(tb*tb);} else if(r2<tc){z*=tc/r2;aux.DE*=tc/r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 15: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=tb*tb; double fixR2=tc*tc; if(r2<minR2){z*=fixR2/minR2;aux.DE*=fixR2/minR2;} else if(r2<fixR2){z*=fixR2/r2;aux.DE*=fixR2/r2;} z*=sf; aux.DE=aux.DE*fabs(sf)+1.0; z+=CVector4(td,td,td,0); break; }
						case 16: { double fold=ta; z.x=fabs(z.x+fold)-fabs(z.x-fold)-z.x; z.y=fabs(z.y+fold)-fabs(z.y-fold)-z.y; z.z=fabs(z.z+fold)-fabs(z.z-fold)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double k=fmax(tb/fmax(r2,1e-21),1.0); z*=k; aux.DE*=k; z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; z.x+=td; break; }
						case 17: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb){z*=tc/tb;aux.DE*=tc/tb;} else if(r2<tc){z*=tc/r2;aux.DE*=tc/r2;} double sc=td+sf*sin((double)i*0.5); z*=sc; aux.DE=aux.DE*fabs(sc)+1.0; break; }
						case 18: { z.x=clamp(z.x,-ta,ta)*2.0-z.x; z.y=clamp(z.y,-ta,ta)*2.0-z.y; z.z=clamp(z.z,-ta,ta)*2.0-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double k=fmax(tb/fmax(r2,1e-21),1.0); z*=k*tc; aux.DE=aux.DE*k*fabs(tc)+1.0; z+=CVector4(td,0,0,0); break; }
						case 19: { z=fabs(z+CVector4(ta,tb,ta,0))-CVector4(ta,tb,ta,0); double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=tc*tc; if(r2<minR2){double s=td/minR2;z*=s;aux.DE*=s;} else if(r2<td){double s=td/r2;z*=s;aux.DE*=s;} z*=sf; aux.DE=aux.DE*fabs(sf)+1.0; break; }
						case 20: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double theta=atan2(z.y,z.x); double r=sqrt(z.x*z.x+z.y*z.y); r=fmax(r,tb); z.x=r*cos(theta); z.y=r*sin(theta); double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tc*tc){z*=1.0/(tc*tc);aux.DE*=1.0/(tc*tc);} else if(r2<1.0){z/=r2;aux.DE/=r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 21: { z=fabs(z)-CVector4(ta,ta,ta,0); z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tc*tc){z*=td/(tc*tc);aux.DE*=td/(tc*tc);} else if(r2<td){z*=td/r2;aux.DE*=td/r2;} z*=sf; aux.DE=aux.DE*fabs(sf)+1.0; break; }
						case 22: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double sph=tb*tb/fmax(r2,tc*tc); z*=sph; aux.DE*=sph; z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 23: { double fold23=ta*(1.0+tb*(double)i/250.0); z.x=fabs(z.x+fold23)-fabs(z.x-fold23)-z.x; z.y=fabs(z.y+fold23)-fabs(z.y-fold23)-z.y; z.z=fabs(z.z+fold23)-fabs(z.z-fold23)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tc*tc){z*=td/(tc*tc);aux.DE*=td/(tc*tc);} else if(r2<td){z*=td/r2;aux.DE*=td/r2;} z*=sf; aux.DE=aux.DE*fabs(sf)+1.0; break; }
						case 24: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double k=tb/fmax(r,tc); z*=k; aux.DE*=k; z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 25: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=tb*tb; double fixR2=tc*tc; double sph=clamp(fixR2/fmax(r2,minR2),1.0,fixR2/minR2); z*=sph; aux.DE*=sph; z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 26: { if(z.x+z.y<0){double t=-z.y;z.y=-z.x;z.x=t;} if(z.x+z.z<0){double t=-z.z;z.z=-z.x;z.x=t;} z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tc*tc){z*=td/(tc*tc);aux.DE*=td/(tc*tc);} else if(r2<td){z*=td/r2;aux.DE*=td/r2;} z*=sf; aux.DE=aux.DE*fabs(sf)+1.0; break; }
						case 27: { z.x=ta-fabs(z.x-ta); z.y=ta-fabs(z.y-ta); z.z=ta-fabs(z.z-ta); double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb*tb){z*=tc/(tb*tb);aux.DE*=tc/(tb*tb);} else if(r2<tc){z*=tc/r2;aux.DE*=tc/r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 28: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double inv=tb/fmax(r2,1e-21); z*=inv; aux.DE*=inv; z.x+=tc; z.y+=tc; z.z+=td; break; }
						case 29: { double s29=sf; z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb*tb){s29*=tc/(tb*tb);} else if(r2<tc){s29*=tc/r2;} z*=s29; aux.DE=aux.DE*fabs(s29)+1.0; z+=CVector4(td,td,td,0); break; }
						case 30: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tc)-fabs(z.z-tc)-z.z; double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); if(r<td&&r>1e-21){double s=td/r;z*=s;aux.DE*=s;} z*=sf; aux.DE=aux.DE*fabs(sf)+1.0; break; }
						case 31: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=tb*tb; if(r2<minR2){z*=1.0/minR2;aux.DE*=1.0/minR2;} else if(r2<1.0){z/=r2;aux.DE/=r2;} z.x=z.x*tc+td; z.y=z.y*tc; z.z=z.z*tc; aux.DE=aux.DE*fabs(tc)+1.0; break; }
						case 32: { double a32=ta*M_PI/180.0; double c32=cos(a32),s32=sin(a32); double ny=z.y*c32-z.z*s32; z.z=z.y*s32+z.z*c32; z.y=ny; z.x=fabs(z.x+tb)-fabs(z.x-tb)-z.x; z.y=fabs(z.y+tb)-fabs(z.y-tb)-z.y; z.z=fabs(z.z+tb)-fabs(z.z-tb)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tc*tc){z*=td/(tc*tc);aux.DE*=td/(tc*tc);} else if(r2<td){z*=td/r2;aux.DE*=td/r2;} z*=sf; aux.DE=aux.DE*fabs(sf)+1.0; break; }
						case 33: { z=fabs(z+CVector4(ta,ta,ta,0))-CVector4(ta,ta,ta,0); z=fabs(z+CVector4(tb,tb,tb,0))-CVector4(tb,tb,tb,0); double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tc*tc){z*=td/(tc*tc);aux.DE*=td/(tc*tc);} else if(r2<td){z*=td/r2;aux.DE*=td/r2;} z*=sf; aux.DE=aux.DE*fabs(sf)+1.0; break; }
						case 34: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double smooth=tb*tb/(fmax(r2,tc*tc)); z*=smooth; aux.DE*=smooth; z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 35: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double len=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double k=tb/fmax(len*len,tc); z*=k;aux.DE*=k; z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 36: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double f36=1.0/(1.0+exp(-tb*(r2-tc))); z*=f36*td; aux.DE=aux.DE*f36*fabs(td)+1.0; break; }
						case 37: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=tb*tb; double maxSph=tc; double sph=clamp(1.0/fmax(r2,minR2),1.0,maxSph); z*=sph; aux.DE*=sph; z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 38: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double sum=fabs(z.x)+fabs(z.y)+fabs(z.z); if(sum>tb){double s=tb/sum;z*=s;aux.DE*=s;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; z+=CVector4(td,0,0,0); break; }
						case 39: { double fold39=ta+tb*sin((double)i*tc); z.x=fabs(z.x+fold39)-fabs(z.x-fold39)-z.x; z.y=fabs(z.y+fold39)-fabs(z.y-fold39)-z.y; z.z=fabs(z.z+fold39)-fabs(z.z-fold39)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<0.25){z*=4.0;aux.DE*=4.0;} else if(r2<1.0){z/=r2;aux.DE/=r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 40: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double p=tb/fmax(r2,1e-21); z.x=z.x*p+tc; z.y=z.y*p+tc; z.z=z.z*p+td; aux.DE=aux.DE*fabs(p)+1.0; break; }
						case 41: { if(z.x>ta)z.x=2.0*ta-z.x; if(z.x<-ta)z.x=-2.0*ta-z.x; if(z.y>ta)z.y=2.0*ta-z.y; if(z.y<-ta)z.y=-2.0*ta-z.y; if(z.z>ta)z.z=2.0*ta-z.z; if(z.z<-ta)z.z=-2.0*ta-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb*tb){z*=tc/(tb*tb);aux.DE*=tc/(tb*tb);} else if(r2<tc){z*=tc/r2;aux.DE*=tc/r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 42: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR=tb*tb; double fixR=tc*tc; double s42=(r2<minR)?fixR/minR:(r2<fixR)?fixR/r2:1.0; z*=s42*td; aux.DE=aux.DE*s42*fabs(td)+1.0; break; }
						case 43: { z.x=ta-fabs(fabs(z.x)-ta); z.y=ta-fabs(fabs(z.y)-ta); z.z=ta-fabs(fabs(z.z)-ta); double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb*tb){z*=tc/(tb*tb);aux.DE*=tc/(tb*tb);} else if(r2<tc){z*=tc/r2;aux.DE*=tc/r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						case 44: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double invR=1.0/fmax(r2,tb*tb); z*=invR*tc; aux.DE=aux.DE*invR*fabs(tc)+1.0; z+=CVector4(td,td,td,0); break; }
						case 45: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r=sqrt(z.x*z.x+z.y*z.y+z.z*z.z); double f45=tb/(1.0+exp(tc*(r-td))); z*=f45; aux.DE*=f45; break; }
						case 46: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double maxC=fmax(fabs(z.x),fmax(fabs(z.y),fabs(z.z))); if(maxC>tb){double s=tb/maxC;z*=s;aux.DE*=s;} z*=tc; aux.DE=aux.DE*fabs(tc)+1.0; z+=CVector4(td,0,0,0); break; }
						case 47: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double k47=fmax(tb*tb/fmax(r2,tc*tc),1.0); z*=k47*td; aux.DE=aux.DE*k47*fabs(td)+1.0; break; }
						case 48: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double minR2=tb*tb; if(r2<minR2){z*=1.0/minR2;aux.DE*=1.0/minR2;} else if(r2<1.0){z/=r2;aux.DE/=r2;} double sc48=tc+td*exp(-(double)i*sf); z*=sc48; aux.DE=aux.DE*fabs(sc48)+1.0; break; }
						case 49: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; double r2=z.x*z.x+z.y*z.y+z.z*z.z; double sph49=fmax(tb/fmax(r2,tc*tc),1.0); z.x=z.x*sph49*td; z.y=z.y*sph49; z.z=z.z*sph49; aux.DE*=sph49*fabs(td); break; }
						case 50: { z.x=fabs(z.x+ta)-fabs(z.x-ta)-z.x; z.y=fabs(z.y+ta)-fabs(z.y-ta)-z.y; z.z=fabs(z.z+ta)-fabs(z.z-ta)-z.z; if(z.x-z.y<0){double t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0){double t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0){double t=z.y;z.y=z.z;z.z=t;} double r2=z.x*z.x+z.y*z.y+z.z*z.z; if(r2<tb*tb){z*=tc/(tb*tb);aux.DE*=tc/(tb*tb);} else if(r2<tc){z*=tc/r2;aux.DE*=tc/r2;} z*=td; aux.DE=aux.DE*fabs(td)+1.0; break; }
						default: break;
					}
					double md_blend = std::min(std::max(mut.mdWeight * mut.masterWeight, 0.0), 1.0);
		...
		if (std::isnan(z.x) || std::isnan(z.y) || std::isnan(z.z) || std::isnan(z.w)
			|| std::isinf(z.x) || std::isinf(z.y) || std::isinf(z.z) || std::isinf(z.w))
		{
			z = preMutZ;
		}
		if (std::isnan(aux.DE) || std::isinf(aux.DE) || aux.DE < 1e-15 || aux.DE > 1e15)
		{
			aux.DE = 1.0;
		}
		...
		if (fractals.IsCheckForBailout(sequence))
		{
			...
			else if (Mode == calcModeCubeOrbitTrap)
			{
				if (i >= in.material->textureFractalizeStartIteration)
				{
					...
					// Negative = inside trap, positive = outside
					double sdf = 1e10;
					...
					switch (in.material->textureFractalizeShape)
					{
						...
						{
							// Box SDF: max of per-axis signed distances
						...
						{
							// Cross: union of 3 infinite slabs, SDF = min distance to any arm
							double dxy = min(fabs(zz.x), fabs(zz.y)) - size;
							double dxz = min(fabs(zz.x), fabs(zz.z)) - size;
							double dyz = min(fabs(zz.y), fabs(zz.z)) - size;
							sdf = min(dxy, min(dxz, dyz));
							break;
						}
						case texture::fractalizeShapeLine:
						{
							// Infinite cylinder SDF (line = Z axis)
							sdf = sqrt(zz.x * zz.x + zz.y * zz.y) - size;
							break;
						}
						case texture::fractalizeShapePlane:
						{
							// Infinite slab SDF (plane = XY, normal = Z)
							sdf = fabs(zz.z) - size;
							break;
						}
						case texture::fractalizeShapeCylinder:
						{
							// Infinite cylinder SDF along Z
							sdf = sqrt(zz.x * zz.x + zz.y * zz.y) - size;
							break;
						}
						case texture::fractalizeShapeTorus:
						{
							// Torus SDF in XY plane
							double majorR = size * 0.7;
							double minorR = size * 0.5;
							double distXY = sqrt(zz.x * zz.x + zz.y * zz.y) - majorR;
							sdf = sqrt(distXY * distXY + zz.z * zz.z) - minorR;
							break;
						}
						case texture::fractalizeShapeSpiral:
						{
							// Archimedean spiral SDF: distance to nearest winding
							double angle = atan2(zz.y, zz.x);
							double r = sqrt(zz.x * zz.x + zz.y * zz.y);
							double spiralR = fmod(r, size);
							double angleOffset = fmod(angle / (2.0 * M_PI) * size, size);
							double spiralDist = fmod(fabs(spiralR - angleOffset), size);
							if (spiralDist > size * 0.5) spiralDist = size - spiralDist;
							// Combine radial distance with Z distance
							sdf = sqrt(spiralDist * spiralDist + zz.z * zz.z) - size * 0.2;
							break;
						}
					}

					// === Cinema4D-style closest-point accumulator ===
					// Weight = exp(-sdf^2 / falloff^2): Gaussian falloff around trap surface
					// Works for both inside (sdf<0) and outside (sdf>0)
					double falloff = size * 0.4; // tune: narrower = sharper texture boundary
					if (falloff < 1e-10) falloff = 1e-10;
					double weight = exp(-(sdf * sdf) / (falloff * falloff));

					// Accumulate weighted z position (Cinema4D: weighted UV coords)
					accumZ += z.GetXYZ() * weight;
					accumWeight += weight;

					// Track closest approach for colorIndex
					if (sdf < closestSdf)
					{
						closestSdf = sdf;
						closestZ = z.GetXYZ();
					...
					// Early exit: deep inside trap and not blending
					if (sdf < -size * 0.5 && !in.material->textureFractalizeIterationBlend)
					{
						break;
					}
				}
	...
	{
		if (accumWeight > 1e-10)
		{
			// Weighted average z = texture-coördinaat die de fractal volgt
			out->z = accumZ / accumWeight;
			// colorIndex = genormaliseerde dichtheid (hoe diep/dicht bij de trap)
			// Normaliseer colorIndex: clamp op 1000 zodat gradient consistent is
			out->colorIndex = fmin(accumWeight * 10.0, 1000.0);
			out->iters = closestIter + 1;
		}
		else if (closestSdf < 1e9)
		{
			// Geen gewogen hits maar wel een dichtstbijzijnde punt
			out->z = closestZ;
			out->colorIndex = 0.0;
			out->iters = closestIter + 1;
		}
		...
		{
			// Nooit in de buurt geweest: gebruik eindpunt z
			out->z = z.GetXYZ();
			out->colorIndex = 0.0;
			out->iters = i;
		}
