		...
		// -------------- Formula Mutation post-processing ---------------
		{
			...
			if (mutationActive)
			{
				...
				if (i >= mut.blkIterStart && i < mut.blkIterStop && mut.blkType != 0)
				{
					...
					double zx = z.x, zy = z.y, zz = z.z;
					double r = sqrt(zx*zx + zy*zy + zz*zz + 1e-21);
					switch (mut.blkType)
					{
						...
						case 11: { double rq=round(r/gx)*gx; double s=rq/r; z.x=zx*s; z.y=zy*s; z.z=zz*s; break; }
						case 12: { double na=fmax(round(fabs(tb)),1.0); double lim=fmax(fabs(td),1e-4); z.x=round(z.x/lim*na)/na*lim; z.y=round(z.y/lim*na)/na*lim; z.z=round(z.z/lim*na)/na*lim; break; }
						case 13: { z.x=trunc(z.x/gx)*gx; z.y=trunc(z.y/gy)*gy; z.z=trunc(z.z/gz)*gz; break; }
						case 14: { double a=fmin(fmax(sf,0.0),1.0); z.x=zx+(round(zx/gx)*gx-zx)*a; z.y=zy+(round(zy/gx)*gx-zy)*a; z.z=zz+(round(zz/gx)*gx-zz)*a; break; }
						case 15: { double sgx=(zx<0.0)?-1.0:1.0; double sgy=(zy<0.0)?-1.0:1.0; double sgz=(zz<0.0)?-1.0:1.0; z.x=sgx*round(fabs(zx)/gx)*gx; z.y=sgy*round(fabs(zy)/gx)*gx; z.z=sgz*round(fabs(zz)/gx)*gx; break; }
						case 16: { z.z=zz+gz*floor(zx/gx); break; }
						case 17: { double ncnt=fmax(round(fabs(ta)),1.0); double ang=atan2(zy,zx); double sct=2.0*M_PI/ncnt; ang=round(ang/sct)*sct; double rxy=sqrt(zx*zx+zy*zy); z.x=rxy*cos(ang); z.y=rxy*sin(ang); break; }
						case 18: { double rxy=sqrt(zx*zx+zy*zy); double rq=round(rxy/gx)*gx; double s=(rxy>1e-9)?rq/rxy:1.0; z.x=zx*s; z.y=zy*s; break; }
						case 19: { double na=fmax(round(fabs(ta)),1.0); z.x=round(z.x*na)/na; z.y=round(z.y*na)/na; z.z=round(z.z*na)/na; break; }
						case 20: { double cx=floor(zx/gx), cy=floor(zy/gy), cz2=floor(zz/gz); double off=(fmod(cx+cy+cz2,2.0)!=0.0)?gx*0.5:0.0; z.x=cx*gx+off; z.y=cy*gy+off; z.z=cz2*gz+off; break; }
						case 21: { z.x=(floor(z.x/gx)+0.5)*gx; z.y=(floor(z.y/gy)+0.5)*gy; z.z=(floor(z.z/gz)+0.5)*gz; break; }
						case 22: { double g=gx*(1.0+(double)i*0.01); z.x=round(z.x/g)*g; z.y=round(z.y/g)*g; z.z=round(z.z/g)*g; break; }
						case 23: { double cx=floor(zx/gx),cy=floor(zy/gy),cz2=floor(zz/gz); double h=sin(cx*12.9898+cy*78.233+cz2*37.719)*43758.5453; h=h-floor(h); z.x=(cx+h*tb)*gx; z.y=(cy+h*tb)*gy; z.z=(cz2+h*tb)*gz; break; }
						case 24: { z.x=round(z.x/gx)*gx; z.y=round(z.y/gy)*gy; double cs=cos(td),sn=sin(td); double nx=z.x*cs-z.y*sn; z.y=z.x*sn+z.y*cs; z.x=nx; break; }
						...
						case 27: { double g=fmax(fabs(ta)+fabs(tb)*fabs(zx),1e-4); z.z=round(zz/g)*g; break; }
						case 28: { double sgx=(zx<0.0)?-1.0:1.0; double lx=(fabs(zx)>1e-6)?exp(round(log(fabs(zx))/gx)*gx):0.0; z.x=sgx*lx; double sgy=(zy<0.0)?-1.0:1.0; double ly=(fabs(zy)>1e-6)?exp(round(log(fabs(zy))/gx)*gx):0.0; z.y=sgy*ly; break; }
						case 29: { double kk=fmax(fabs(tb),0.1); z.x=gx*tanh(zx/gx*kk); z.y=gy*tanh(zy/gy*kk); z.z=gz*tanh(zz/gz*kk); break; }
						case 30: { double c=cos(0.78539816),s=sin(0.78539816); double ux=zx*c-zy*s, uy=zx*s+zy*c; ux=round(ux/gx)*gx; uy=round(uy/gx)*gx; z.x=ux*c+uy*s; z.y=-ux*s+uy*c; break; }
						case 31: { z.x=round(z.x/gx)*gx*sf; z.y=round(z.y/gy)*gy*sf; z.z=round(z.z/gz)*gz*sf; break; }
						case 32: { double bx=round(zx/gx)*gx; double res=zx-bx; res=fmax(fmin(res,gx*0.25),-gx*0.25); z.x=bx+res; double by=round(zy/gy)*gy; double rey=zy-by; rey=fmax(fmin(rey,gy*0.25),-gy*0.25); z.y=by+rey; break; }
						case 33: { double nx=fmax(round(fabs(ta)),1.0),ny=fmax(round(fabs(tb)),1.0),nz=fmax(round(fabs(tc)),1.0); z.x=round(z.x*nx)/nx; z.y=round(z.y*ny)/ny; z.z=round(z.z*nz)/nz; break; }
						case 34: { double s=(r>1e-9)?round(r/gx)*gx/r:1.0; z.x=zx*s; z.y=zy*s; z.z=zz*s; aux.DE*=fmax(fabs(s),0.01); break; }
						...
						case 36: { double ph=sin((double)i*tb); z.x=round((z.x+ph)/gx)*gx-ph; z.y=round((z.y+ph)/gy)*gy-ph; break; }
						case 37: { double cx=floor(zx/gx); double sc=1.0+0.1*sf*fmod(fabs(cx),3.0); z.x=cx*gx; z.y=round(zy/gy)*gy*sc; z.z=round(zz/gz)*gz*sc; break; }
						case 38: { double row=floor(zy/gy); double sh=(fmod(fabs(row),2.0)!=0.0)?gx*0.5:0.0; z.x=round((zx-sh)/gx)*gx+sh; z.y=row*gy; break; }
						case 39: { double rq=round(r/gx)*gx; double s=(r>1e-9)?rq/r:1.0; z.x=zx*s; z.y=zy*s; z.z=zz*s; break; }
						case 40: { double rxy=sqrt(zx*zx+zy*zy); double rq=round(rxy/gx)*gx; double s=(rxy>1e-9)?rq/rxy:1.0; z.x=zx*s; z.y=zy*s; z.z=round(zz/gz)*gz; break; }
						...
						case 43: { double na=pow(2.0,fmax(round(fabs(ta)),1.0)); z.x=floor(z.x*na)/na; z.y=floor(z.y*na)/na; z.z=floor(z.z*na)/na; break; }
						case 44: { double ga=fmax(fabs(ta),1e-4), gb=fmax(fabs(tb),1e-4); double a=fmin(fmax(sf,0.0),1.0); z.x=round(z.x/ga)*ga*(1.0-a)+round(z.x/gb)*gb*a; z.y=round(z.y/ga)*ga*(1.0-a)+round(z.y/gb)*gb*a; break; }
						case 45: { double cx=floor(zx/gx); z.x=cx*gx; double g2=gy*(1.0+0.2*fmod(fabs(cx),2.0)); z.y=round(zy/g2)*g2; break; }
						case 46: { double tol=fmax(fmin(fabs(td),0.5),0.0); double bx=round(zx/gx)*gx; if(fabs(zx-bx)<gx*tol) z.x=bx; double by=round(zy/gy)*gy; if(fabs(zy-by)<gy*tol) z.y=by; break; }
						...
						case 49: { double c=cos(0.61547971),s=sin(0.61547971); double ux=zx, uy=zy*c-zz*s, uz=zy*s+zz*c; ux=round(ux/gx)*gx; uy=round(uy/gx)*gx; uz=round(uz/gx)*gx; z.x=ux; z.y=uy*c+uz*s; z.z=-uy*s+uz*c; break; }
				...
				if (i >= mut.tilIterStart && i < mut.tilIterStop && mut.tilType != 0)
				{
					...
					switch (mut.tilType)
					{
						...
						case 7: { double mx=fmod(fabs(zx),2.0*gx); z.x=((mx<gx)?mx:2.0*gx-mx)-gx*0.5; break; }
						case 8: { double mx=fmod(fabs(zx),2.0*gx); z.x=((mx<gx)?mx:2.0*gx-mx)-gx*0.5; double my=fmod(fabs(zy),2.0*gy); z.y=((my<gy)?my:2.0*gy-my)-gy*0.5; double mz=fmod(fabs(zz),2.0*gz); z.z=((mz<gz)?mz:2.0*gz-mz)-gz*0.5; break; }
						...
						case 11: { double ncnt=fmax(round(fabs(ta)),1.0); double ang=atan2(zy,zx); double sct=2.0*M_PI/ncnt; ang=fmod(ang+10.0*M_PI,sct)-sct*0.5; double rxy=sqrt(zx*zx+zy*zy); z.x=rxy*cos(ang); z.y=rxy*sin(ang); break; }
						case 12: { double rxy=sqrt(zx*zx+zy*zy); double mr=rxy-gx*floor(rxy/gx); double s=(rxy>1e-9)?mr/rxy:0.0; z.x=zx*s; z.y=zy*s; break; }
						case 13: { double row=floor(zy/gy+0.5); double sh=(fmod(fabs(row),2.0)!=0.0)?gx*0.5:0.0; z.x=(zx+sh)-gx*floor((zx+sh)/gx+0.5); z.y=zy-gy*floor(zy/gy+0.5); break; }
						case 14: { double a60=M_PI/3.0; double ux=gx, uy=0.0; double vx=gx*cos(a60), vy=gx*sin(a60); double det=ux*vy-uy*vx; double ix=floor((zx*vy-zy*vx)/det+0.5); double iy=floor((zy*ux-zx*uy)/det+0.5); z.x=zx-ix*ux-iy*vx; z.y=zy-ix*uy-iy*vy; break; }
						case 15: { double ix=floor(zx/gx+0.5), iy=floor(zy/gy+0.5); z.x=zx-ix*gx; z.y=zy-iy*gy; double a=td*(ix+iy); double cs=cos(a),sn=sin(a); double nx=z.x*cs-z.y*sn; z.y=z.x*sn+z.y*cs; z.x=nx; break; }
						case 16: { double ncnt=fmax(round(fabs(ta)),1.0); double ang=atan2(zy,zx); double sct=2.0*M_PI/ncnt; ang=fabs(fmod(ang+10.0*M_PI,sct)-sct*0.5); double rxy=sqrt(zx*zx+zy*zy); rxy=rxy-gy*floor(rxy/gy); z.x=rxy*cos(ang); z.y=rxy*sin(ang); break; }
						case 17: { double ph=sf*sin((double)i*fmax(fabs(tb),0.01)); z.x=(zx+ph)-gx*floor((zx+ph)/gx+0.5); z.y=(zy+ph)-gy*floor((zy+ph)/gy+0.5); break; }
						...
						case 19: { double mz=fmod(fabs(zz),2.0*gz); z.z=((mz<gz)?mz:2.0*gz-mz)-gz*0.5; break; }
						case 20: { double c=cos(0.78539816),s=sin(0.78539816); double ux=zx*c-zy*s, uy=zx*s+zy*c; ux=ux-gx*floor(ux/gx+0.5); uy=uy-gx*floor(uy/gx+0.5); z.x=ux*c+uy*s; z.y=-ux*s+uy*c; break; }
						case 21: { double ix=floor(zx/gx+0.5), iy=floor(zy/gy+0.5); double h=sin(ix*12.9898+iy*78.233)*43758.5453; h=h-floor(h); z.x=zx-ix*gx+(h-0.5)*tb; z.y=zy-iy*gy+(h-0.5)*tb; break; }
						case 22: { double a=0.0; for(int k=0;k<3;k++){ double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){z.x=z.x-2.0*d*cos(a); z.y=z.y-2.0*d*sin(a);} a=a+M_PI/3.0; } z.x=z.x-gx*floor(z.x/gx+0.5); break; }
						case 23: { z.x=z.x-gx*floor(z.x/gx+0.5); z.y=z.y-gy*floor(z.y/gy+0.5); z.z=z.z-gz*floor(z.z/gz+0.5); double q=gx*0.25; z.x=round(z.x/q)*q; z.y=round(z.y/q)*q; break; }
						case 24: { double qx=gx,qy=gx*1.5,qz=gx*2.0; z.x=z.x-qx*floor(z.x/qx+0.5); z.y=z.y-qy*floor(z.y/qy+0.5); z.z=z.z-qz*floor(z.z/qz+0.5); break; }
						case 25: { z.x=fabs(zx-gx*floor(zx/gx+0.5)); break; }
						case 26: { z.y=fabs(zy-gy*floor(zy/gy+0.5)); break; }
						case 27: { z.x=z.x-gx*floor(z.x/gx+0.5); z.y=z.y-gy*floor(z.y/gy+0.5); double mz=fmod(fabs(zz),2.0*gz); z.z=((mz<gz)?mz:2.0*gz-mz)-gz*0.5; break; }
						case 28: { double ix=floor(zx/gx+0.5),iy=floor(zy/gy+0.5),iz=floor(zz/gz+0.5); double a=fmin(fmax(sf,0.0),1.0); z.x=zx-ix*gx*a; z.y=zy-iy*gy*a; z.z=zz-iz*gz*a; break; }
						case 29: { double lim=fmax(fabs(td),1e-4); if(z.x>lim) z.x=2.0*lim-z.x; else if(z.x<-lim) z.x=-2.0*lim-z.x; if(z.y>lim) z.y=2.0*lim-z.y; else if(z.y<-lim) z.y=-2.0*lim-z.y; if(z.z>lim) z.z=2.0*lim-z.z; else if(z.z<-lim) z.z=-2.0*lim-z.z; break; }
						case 30: { z.x=z.x-gx*floor(z.x/gx+0.5); z.y=z.y-gy*floor(z.y/gy+0.5); double rr=sqrt(z.x*z.x+z.y*z.y); if(rr<fabs(td)&&rr>1e-9){double s=fabs(td)/rr; z.x=z.x*s; z.y=z.y*s;} break; }
						case 31: { double iz=floor(zz/gz+0.5); double off=fmod(fabs(iz),2.0)*gx*0.5; z.x=(zx+off)-gx*floor((zx+off)/gx+0.5); z.y=zy-gy*floor(zy/gy+0.5); z.z=zz-gz*iz; break; }
						case 32: { double ncnt=fmax(round(fabs(ta)),1.0); double ang=atan2(zy,zx); double sct=2.0*M_PI/ncnt; ang=fmod(ang+10.0*M_PI,sct)-sct*0.5; double rxy=sqrt(zx*zx+zy*zy); rxy=rxy-gy*floor(rxy/gy); z.x=rxy*cos(ang); z.y=rxy*sin(ang); z.z=zz-gz*floor(zz/gz+0.5); break; }
						case 33: { double ncnt=fmax(round(fabs(ta)),1.0); double ph=atan2(zy,zx); double th=acos(zz/r); double sp=2.0*M_PI/ncnt; ph=fmod(ph+10.0*M_PI,sp)-sp*0.5; double mm=fmax(round(fabs(tb)),1.0); double st=M_PI/mm; th=fmod(th+10.0*M_PI,st); z.x=r*sin(th)*cos(ph); z.y=r*sin(th)*sin(ph); z.z=r*cos(th); break; }
						case 34: { double ix=floor(zx/gx+0.5),iy=floor(zy/gy+0.5),iz=floor(zz/gz+0.5); double sgn=(fmod(ix+iy+iz,2.0)!=0.0)?-1.0:1.0; z.x=sgn*(zx-ix*gx); z.y=sgn*(zy-iy*gy); z.z=sgn*(zz-iz*gz); break; }
						case 35: { double iz=floor(zz/gz+0.5); z.z=zz-iz*gz; z.x=zx-gx*floor(zx/gx+0.5); z.y=zy-gy*floor(zy/gy+0.5); double a=td*iz; double cs=cos(a),sn=sin(a); double nx=z.x*cs-z.y*sn; z.y=z.x*sn+z.y*cs; z.x=nx; break; }
						case 36: { double rxy=sqrt(zx*zx+zy*zy); double mr=rxy-gx*floor(rxy/gx); double s=(rxy>1e-9)?mr/rxy:0.0; z.x=zx*s; z.y=zy*s; break; }
						case 37: { double mx=fmod(fabs(zx),2.0*gx); z.x=((mx<gx)?mx:2.0*gx-mx)-gx*0.5; z.y=zy-gy*floor(zy/gy+0.5); break; }
						case 38: { double iy=floor(zy/gy+0.5); double sh=fmod(fabs(iy)*0.6180339887,1.0)*gx; z.x=(zx+sh)-gx*floor((zx+sh)/gx+0.5); z.y=zy-gy*floor(zy/gy+0.5); break; }
						case 39: { double a=fmin(fmax(sf,0.0),1.0); double tx=zx-gx*floor(zx/gx+0.5); double ty=zy-gy*floor(zy/gy+0.5); z.x=zx+(tx-zx)*a; z.y=zy+(ty-zy)*a; break; }
						case 40: { double tx=zx-gx*floor(zx/gx+0.5); double ty=zy-gy*floor(zy/gy+0.5); double tz=zz-gz*floor(zz/gz+0.5); double rr=tx*tx+ty*ty+tz*tz; double dd=fmax(fabs(td),1e-4); double s=(rr>1e-9)?dd*dd/rr:1.0; z.x=tx*s; z.y=ty*s; z.z=tz*s; break; }
						case 41: { double ix=floor(zx/gx+0.5),iy=floor(zy/gy+0.5); double sc=1.0+sf*0.1*fmod(fabs(ix+iy),3.0); z.x=(zx-ix*gx)*sc; z.y=(zy-iy*gy)*sc; break; }
						case 42: { double a=td; for(int k=0;k<3;k++){ double d=z.x*cos(a)+z.y*sin(a); if(d<0.0){ z.x=z.x-2.0*d*cos(a); z.y=z.y-2.0*d*sin(a);} a=a+2.0*M_PI/3.0; } break; }
						case 43: { z.x=fabs(z.x-gx*floor(z.x/gx+0.5)); z.y=fabs(z.y-gy*floor(z.y/gy+0.5)); z.z=fabs(z.z-gz*floor(z.z/gz+0.5)); break; }
						case 44: { double qy=gx*1.6180339887; z.x=zx-gx*floor(zx/gx+0.5); z.y=zy-qy*floor(zy/qy+0.5); break; }
						case 45: { z.x=zx-gx*floor(zx/gx+0.5); z.y=zy-gy*floor(zy/gy+0.5); double a=td*zz; double cs=cos(a),sn=sin(a); double nx=z.x*cs-z.y*sn; z.y=z.x*sn+z.y*cs; z.x=nx; break; }
						case 46: { z.x=zx-gx*floor(zx/gx+0.5); z.y=zy-gy*floor(zy/gy+0.5); double kk=fmax(fabs(tb),0.01); z.x=z.x+sf*0.1*sin(zy*kk); z.y=z.y+sf*0.1*sin(zx*kk); break; }
						case 47: { double tx=zx-gx*floor(zx/gx+0.5); double ty=zy-gy*floor(zy/gy+0.5); double lim=gx*fmax(fmin(fabs(td),0.5),0.05); z.x=fmax(fmin(tx,lim),-lim); z.y=fmax(fmin(ty,lim),-lim); break; }
						case 48: { for(int k=0;k<3;k++){ double g=gx*pow(2.0,(double)k); z.x=z.x-g*floor(z.x/g+0.5); z.y=z.y-g*floor(z.y/g+0.5); z.z=z.z-g*floor(z.z/g+0.5); } break; }
						case 49: { double mr=r-gx*floor(r/gx); double s=(r>1e-9)?mr/r:0.0; z.x=zx*s; z.y=zy*s; z.z=zz*s; break; }
