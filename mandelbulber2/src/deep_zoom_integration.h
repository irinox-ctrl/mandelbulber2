...
struct sDeepZoomState
{
	...
	struct sGPUData
	{
		std::vector<double> refOrbitFlat; // flattened reference orbit for GPU (fp64)
		double saMatrix[9];               // 3x3 SA matrix (row-major)
		int saSkipIters = 0;
		bool saValid = false;
		int orbitLength = 0;
	} gpuData;
...
struct sDeepZoomState { struct sGPUData { std::vector<double> refOrbitFlat; double saMatrix[9]; int saSkipIters; bool saValid; int orbitLength; }; };
