...
namespace {
void ClampIterationFogParams(sParamRender *params)
{
	if (!params) return;
	params->iterFogOpacityTrim = std::max(params->iterFogOpacityTrim, 0.0f);
	params->iterFogColor1Maxiter =
		std::max(params->iterFogColor1Maxiter, params->iterFogOpacityTrim + 1.0f);
	params->iterFogColor2Maxiter =
		std::max(params->iterFogColor2Maxiter, params->iterFogColor1Maxiter + 1.0f);
	params->iterFogOpacityTrimHigh = std::max(
		params->iterFogOpacityTrimHigh, params->iterFogColor2Maxiter + 1.0f);
	const float maxN = std::max(
		params->iterFogEffectiveN > 0.0f ? params->iterFogEffectiveN : float(params->N), 1.0f);
	params->iterFogOpacityTrimHigh = std::min(params->iterFogOpacityTrimHigh, maxN);
	if (params->iterFogOpacity > 1.0) params->iterFogOpacity = 1.0;
}
} // namespace

void sParamRender::UpdateIterFogEffectiveN(const cNineFractals *fractal)
{
	(void)fractal;
	// The iteration loop is capped by global N; formula_maxiter may be higher in the UI.
	iterFogEffectiveN = std::max(float(N), 1.0f);
	ClampIterationFogParams(this);
}
		...
		: primitives(container, objectData, objectTreeNodes)
{
	...
	cloudsDensityGamma = container->Get<double>("clouds_density_gamma");
	cloudsSoftness = container->Get<double>("clouds_softness");
	cloudsAdaptiveDetail = container->Get<bool>("clouds_adaptive_detail");
	...
	marchingOmega = container->Get<double>("marching_omega");
	progressiveAdaptiveThreshold = container->Get<double>("progressive_adaptive_threshold");
	...
	volumetricMaxSteps = container->Get<int>("volumetric_max_steps");
	if (volumetricMaxSteps <= 0)
	{
		const int quality = container->Get<int>("auto_fog_quality");
		volumetricMaxSteps = (quality == 0) ? 48 : (quality == 2) ? 192 : 96;
	}
	volumetricPreviewFast = container->Get<bool>("volumetric_preview_fast");
	volumetricPreviewStepMul = float(container->Get<double>("volumetric_preview_step_mul"));
	if (volumetricPreviewStepMul < 1.0f) volumetricPreviewStepMul = 1.0f;
	volumetricPreviewActive =
		volumetricPreviewFast && container->Get<int>("image_preview_scale") > 0;
	...
	SSAO_useNormalBuffer = container->Get<bool>("SSAO_use_normal_buffer");
	...
	volFogMode = container->Get<int>("volumetric_fog_mode");
	volFogEnabled = container->Get<bool>("volumetric_fog_enabled");
	voxelFogCacheEnabled = container->Get<bool>("voxel_fog_cache_enabled");
	voxelFogCacheResolution = container->Get<int>("voxel_fog_cache_resolution");
	volumetricLightDEFactor = container->Get<double>("volumetric_light_DE_Factor");
	volumetricBlueNoiseJitter = container->Get<bool>("volumetric_blue_noise_jitter");
	volumetricJitterStrength = container->Get<float>("volumetric_jitter_strength");
	...
	for (int i = 0; i < NUMBER_OF_FRACTALS; i++)
	{
		...
		formulaPositionIterStart[i] = container->Get<CVector3>("formula_position_iter_start", i + 1);
		formulaPositionIterStop[i] = container->Get<CVector3>("formula_position_iter_stop", i + 1);
		formulaRotationIterStart[i] = container->Get<CVector3>("formula_rotation_iter_start", i + 1);
		formulaRotationIterStop[i] = container->Get<CVector3>("formula_rotation_iter_stop", i + 1);
		formulaRepeatIterStart[i] = container->Get<CVector3>("formula_repeat_iter_start", i + 1);
		formulaRepeatIterStop[i] = container->Get<CVector3>("formula_repeat_iter_stop", i + 1);
	...
	common.positionIterStart = container->Get<CVector3>("formula_position_iter_start");
	common.positionIterStop = container->Get<CVector3>("formula_position_iter_stop");
	common.rotationIterStart = container->Get<CVector3>("formula_rotation_iter_start");
	common.rotationIterStop = container->Get<CVector3>("formula_rotation_iter_stop");
	common.repeatIterStart = container->Get<CVector3>("formula_repeat_iter_start");
	common.repeatIterStop = container->Get<CVector3>("formula_repeat_iter_stop");
