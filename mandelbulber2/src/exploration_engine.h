...
// Mutation fitness components for evolutionary parameter optimization
struct sMutationFitness
{
	double visualInterest = 0.0;
	double detailComplexity = 0.0;
	double symmetryBalance = 0.0;
	double boundaryQuality = 0.0;
	double convergenceSpeed = 0.0;
	double numericalStability = 0.0;
	double renderSpeed = 0.0;
	double novelty = 0.0;

	double WeightedTotal(const struct sMutationObjectiveWeights &w) const;

	void FillObjectives(double (&out)[8]) const
	{
		out[0] = visualInterest;
		out[1] = detailComplexity;
		out[2] = symmetryBalance;
		out[3] = boundaryQuality;
		out[4] = convergenceSpeed;
		out[5] = numericalStability;
		out[6] = renderSpeed;
		out[7] = novelty;
	}
};

struct sMutationObjectiveWeights
{
	double visual = 0.30;
	double detail = 0.15;
	double symmetry = 0.10;
...
inline double sMutationFitness::WeightedTotal(const sMutationObjectiveWeights &w) const
{
	return w.visual * visualInterest + w.detail * detailComplexity
		+ w.symmetry * symmetryBalance + w.boundary * boundaryQuality
		+ w.convergence * convergenceSpeed + w.stability * numericalStability
		+ w.speed * renderSpeed + w.novelty * novelty;
}
...
struct sMultiObjectiveResult
{
	...
	static std::vector<int> ComputeParetoFrontier(const std::vector<sMultiObjectiveResult> &candidates)
	{
		...
		for (size_t i = 0; i < n; ++i)
		{
			...
			for (size_t j = 0; j < n && !dominated; ++j)
			{
				...
				bool anyStrictlyBetter = false;
				for (int k = 0; k < 8; ++k)
				{
					if (candidates[j].objectives[k] < candidates[i].objectives[k])
						allBetterOrEqual = false;
					if (candidates[j].objectives[k] > candidates[i].objectives[k])
						anyStrictlyBetter = true;
				}
				if (allBetterOrEqual && anyStrictlyBetter) dominated = true;
			}
			if (!dominated) frontier.push_back(candidates[i].candidateIndex);
		}
		return frontier;
	}
};
