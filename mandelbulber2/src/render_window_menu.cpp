...
{
	CHECK_MAIN_INTERFACE();
	if (gMainInterface->mainImage && gMainInterface->mainImage->IsUsed())
	{
		gMainInterface->StopRender();
		gMainInterface->WaitForMainRenderIdle(60000);
	}
	...
	if (!parSettings.LoadSettingsAsync(fileName, gPar, gParFractal, gAnimFrames, gKeyframes))
	{
		...
		cErrorMessage::showMessage(QObject::tr("Cannot load settings from file!"),
...
void RenderWindow::slotMenuLoadSettingsFromClipboard()
{
	...
	if (parSettings.LoadSettingsFromClipboardAsync(gPar, gParFractal, gAnimFrames, gKeyframes))
	{
		gInterfaceReadyForSynchronization = true;
		gMainInterface->ComboMouseClickUpdate();
...
void RenderWindow::slotResetToDefault()
{
	...
	else
	{
		...
		gMainInterface->SynchronizeAllParameters(gPar, gParFractal, qInterface::write);
		gInterfaceReadyForSynchronization = true;

		gMainInterface->ComboMouseClickUpdate();
		gMainInterface->SyncAutoRefreshHashWithGpar();
		systemData.lastSettingsFile.clear();
		setWindowTitle(QString("Mandelbulber"));
