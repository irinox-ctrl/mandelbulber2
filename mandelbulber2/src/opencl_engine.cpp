...
{
	PERF_SCOPE("OpenCL.LoadSourcesAndCompile");
	if (hardware->getClDevices().size() > 0 && hardware->getEnabledDevices().size() > 0)
	{
					...
					&& useBuildCache))
		{
			kernelCreated = false;
			clKernels.clear();
			commandQueuesValid = false;
			cachedBufferLayoutHash.clear();
...
{
	PERF_SCOPE("OpenCL.CreateKernel4Program");
	if (programsLoaded)
	{
		optimalJob.jobSizeMultiplier = quint64(params->Get<int>("opencl_job_size_multiplier"));

		if (kernelCreated && clKernels.size() == hardware->getEnabledDevices().size())
		{
...
{
	if (kernelCreated && clKernels.size() == hardware->getEnabledDevices().size()) return true;

	PERF_SCOPE("OpenCL.CreateKernels");
	cl_int err;
	clKernels.clear();
...
{
	PERF_SCOPE("OpenCL.CreateCommandQueue");
	if (commandQueuesValid && clQueues.size() == hardware->getEnabledDevices().size() && readyForRendering)
	{
		return true;
	}
...
void cOpenClEngine::InvalidateGpuPipelineCache()
{
	cachedBufferLayoutHash.clear();
	commandQueuesValid = false;
	kernelCreated = false;
...
bool cOpenClEngine::AreCommandQueuesReady() const
{
	return commandQueuesValid && readyForRendering;
}

QByteArray cOpenClEngine::ComputeBufferLayoutHash() const
{
	QCryptographicHash hash(QCryptographicHash::Md5);
	auto addBufferLists = [&](const QList<listOfBuffers> &lists) {
		hash.addData(QByteArray::number(lists.size()));
		for (const auto &deviceList : lists)
		{
			hash.addData(QByteArray::number(deviceList.size()));
			for (const auto &buffer : deviceList)
			{
				hash.addData(QByteArray::number(buffer.itemSize));
				hash.addData(QByteArray::number(buffer.length));
				hash.addData(buffer.name.toUtf8());
			}
		}
	};
	addBufferLists(outputBuffers);
	addBufferLists(inputBuffers);
	addBufferLists(inputAndOutputBuffers);
	return hash.result();
}
...
bool cOpenClEngine::GpuBuffersAllocated() const
{
	auto buffersValid = [](const QList<listOfBuffers> &lists) {
		...
		for (const auto &deviceList : lists)
		{
			...
			for (const auto &buffer : deviceList)
			{
				if (!buffer.clPtr) return false;
			}
		}
		return true;
	};
	return buffersValid(outputBuffers) && buffersValid(inputBuffers)
				 && buffersValid(inputAndOutputBuffers);
}

void cOpenClEngine::ReleaseBufferMemory()
{
	PERF_SCOPE("OpenCL.ReleaseBufferMemory");
...
{
	PERF_SCOPE("OpenCL.PreAllocateBuffers");
	RegisterInputOutputBuffers(params);

	const QByteArray layoutHash = ComputeBufferLayoutHash();
	if (layoutHash == cachedBufferLayoutHash && !cachedBufferLayoutHash.isEmpty() && GpuBuffersAllocated())
	{
		return true;
	}
	...
	ReleaseBufferMemory();
	RegisterInputOutputBuffers(params);
	cachedBufferLayoutHash = layoutHash;
...
{
	PERF_SCOPE("OpenCL.ReleaseMemory");
	InvalidateGpuPipelineCache();
	ReleaseBufferMemory();
}
...
{
	if (deviceIndex < 0 || deviceIndex >= clKernels.size() || !clKernels[deviceIndex])
	{
		emit showErrorMessage(
			QObject::tr("OpenCL kernel is not ready for device %1").arg(deviceIndex),
			cErrorMessage::errorMessage, nullptr);
		return false;
	}
