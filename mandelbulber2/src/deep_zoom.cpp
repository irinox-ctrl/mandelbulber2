...
{
	// For 3D fractals (Mandelbulb power 8), each iteration amplifies errors
	// by ~log2(power). With 250 iterations that's significant precision loss.
	// We use an aggressive multiplier (6x) on the zoom bits plus a 256-bit
	// base to ensure sharp rendering at all zoom depths.
	if (zoomLevel <= 1.0) return kDefaultPrecisionBits;

	double zoomBits = log2(zoomLevel);
	double bitsNeeded = zoomBits * 6.0 + 256.0;
