...
public:
	...
	void SynchronizeAllParameters(std::shared_ptr<cParameterContainer> par,
		std::shared_ptr<cFractalContainer> parFractal, qInterface::enumReadWrite mode);
	void RegenerateListsAfterLoad() const;
	void OnDeferredSettingsLoadFinished();
	void SynchronizeInterfaceOnFormulaChange(int formulaIndex) const;
	
	// Batch UI update control
	void BeginBatchUpdate();
	void EndBatchUpdate();
	bool IsBatchUpdateInProgress() const { return batchUpdateInProgress; }
	bool IsMainRenderBusy() const;
	bool IsNavigationDragging() const;
	bool WaitForMainRenderIdle(int timeoutMs = 60000);
	...
	/**
	 * Zet autoRefreshLastHash op huidige gPar. Nodig na gPar-wijziging vanuit patroonlijnen
	 * of licht-UI, anders triggert PeriodicRefresh (korte interval) extra StartRender = vertraging.
	 */
	void SyncAutoRefreshHashWithGpar();
	void RequestRender(const QString &reason = QString(), int debounceMs = 120, bool noUndo = false,
		bool synchronizeWithUi = true, bool setupMainImagePreviewInInit = true);
	...
	/** Start render immediately, bypassing debounce (e.g. after batch load). */
	...
	QTimer *renderDebounceTimer = nullptr;
	...
	bool batchUpdateInProgress = false;
...
private:
	...
	void requestDebouncedRender(bool noUndo, bool synchronizeWithUi, bool setupMainImagePreviewInInit,
		int debounceMs = 120, const QString &reason = QString());

	struct sPendingRenderRequest
	{
		bool valid = false;
		bool noUndo = false;
		bool synchronizeWithUi = true;
		bool setupMainImagePreviewInInit = true;
	};

	sPendingRenderRequest pendingRenderRequest;

private slots:
	void slotDebouncedStartRender();
