...
public:
	...
	void BuildLightsData(const cLights *lights, const QMap<QString, int> &textureIndexes,
		const CVector3 &positionShift = CVector3(0.0, 0.0, 0.0));
	...
	void BuildDeepZoomData(const double *orbitData, int orbitLength,
		float power, float bailout, float rebaseThreshold,
		double centerX, double centerY, double centerZ,
		const double *saMatrix, int saSkipIters, bool saValid);
	void BuildVoxelFogCacheData(const sParamRender *params, const cLights *lights);
...
private:
	...
	const int voxelFogCacheItemIndex = 6;
