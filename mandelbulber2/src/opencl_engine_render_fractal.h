...
private:
	...
	static sRGBFloat MCMixColor(const cOpenCLWorkerOutputQueue::sClSingleOutput &output,
		const sRGBFloat &pixel, const sRGBFloat &oldPixel, bool temporalAccumulationEnabled,
		bool cameraMoved);
	...
	CVector3 temporalLastCamera;
	CVector3 temporalLastTarget;
	bool temporalHasPreviousFrame;

	std::shared_ptr<const cParameterContainer> lastRenderParams;

	// Color buffer synchronization flags
	bool paletteDirty = false;
	bool gradientDirty = false;
	bool paramsChanged = false;
	bool paletteBufferValid = false;
	bool gradientBufferValid = false;

	QByteArray cachedFractalGpuBufferHash;
	QByteArray lastCompiledAssemblyHash;
