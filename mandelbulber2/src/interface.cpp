...
namespace {
int runModalOpenSync(QDialog *dialog)
{
	if (!dialog) return QDialog::Rejected;
	int result = QDialog::Rejected;
	QEventLoop loop;
	QObject::connect(dialog, &QDialog::finished, dialog, [&](int r) {
		result = r;
		loop.quit();
	});
	dialog->open();
...
void cInterface::ShowUi()
{
	...
	for (cTabFractal *fractalTab : mainWindow->ui->widgetDockFractal->findChildren<cTabFractal *>())
	{
		connect(fractalTab, &cTabFractal::signalFormulaChanged, this,
			&cInterface::SynchronizeInterfaceOnFormulaChange, Qt::UniqueConnection);
	}
...
void cInterface::SynchronizeAllParameters(std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> parFractal, qInterface::enumReadWrite mode)
{
	PERF_SCOPE("SynchronizeInterface.FullWrite");
	BeginBatchUpdate();
	SynchronizeInterface(par, parFractal, mode);
	EndBatchUpdate();
}

void cInterface::RegenerateListsAfterLoad() const
{
...
void cInterface::OnDeferredSettingsLoadFinished()
{
	ComboMouseClickUpdate();
	SyncAutoRefreshHashWithGpar();
	if (gFlightAnimation) gFlightAnimation->RefreshTable();
	if (gKeyframeAnimation) gKeyframeAnimation->RefreshTable();
	if (mainWindow) mainWindow->showDescriptionPopup();
}

void cInterface::SynchronizeInterfaceOnFormulaChange(int /*formulaIndex*/) const
{
#ifdef USE_OPENCL
	if (gOpenCl && gOpenCl->openClEngineRenderFractal)
		gOpenCl->openClEngineRenderFractal->InvalidateColorBuffers();
#endif
}

// Batch UI update control methods
void cInterface::BeginBatchUpdate()
{
	batchUpdateInProgress = true;
	mainWindow->setUpdatesEnabled(false);
}

void cInterface::EndBatchUpdate()
{
...
bool cInterface::IsMainRenderBusy() const
{
	return mainImage && mainImage->IsUsed();
}

bool cInterface::IsNavigationDragging() const
{
	return mainWindow && mainWindow->manipulations && mainWindow->manipulations->isDraggingStarted();
}

bool cInterface::WaitForMainRenderIdle(const int timeoutMs)
{
	if (!mainImage) return true;
	return WaitUntil([this]() { return !mainImage->IsUsed(); }, timeoutMs, 10, false);
}
...
void cInterface::requestDebouncedRender(const bool noUndo, const bool synchronizeWithUi,
	const bool setupMainImagePreviewInInit, const int debounceMs, const QString &reason)
{
	if (batchUpdateInProgress) return;

	pendingRenderRequest.noUndo = noUndo;
	pendingRenderRequest.synchronizeWithUi = synchronizeWithUi;
	pendingRenderRequest.setupMainImagePreviewInInit = setupMainImagePreviewInInit;
	pendingRenderRequest.valid = true;

	if (!reason.isEmpty())
	{
		WriteLog(QString("RequestRender: %1 (debounce %2ms)").arg(reason).arg(debounceMs), 3);
	}

	if (debounceMs <= 0)
	{
		QTimer::singleShot(0, this, &cInterface::slotDebouncedStartRender);
		return;
	}
	...
	if (!renderDebounceTimer)
	{
		...
		renderDebounceTimer->setInterval(120);
		connect(renderDebounceTimer, &QTimer::timeout, this, &cInterface::slotDebouncedStartRender);
	}

	renderDebounceTimer->setInterval(debounceMs);
	renderDebounceTimer->start();
}

void cInterface::slotDebouncedStartRender()
{
	if (!pendingRenderRequest.valid || batchUpdateInProgress) return;

	const sPendingRenderRequest request = pendingRenderRequest;
	pendingRenderRequest.valid = false;
	startRenderImpl(
		request.noUndo, request.synchronizeWithUi, request.setupMainImagePreviewInInit);
}

void cInterface::FlushPendingRender()
{
	if (renderDebounceTimer) renderDebounceTimer->stop();
	if (!pendingRenderRequest.valid) return;
	...
	const bool noUndo, const bool synchronizeWithUi, const bool setupMainImagePreviewInInit)
{
	...
	EnsureWidgetParameterContractsChecked(mainWin, gPar);
	RepairUiParameters(gPar, gParFractal);
	if (mainWin->ui->widgetDockStatistics)
		mainWin->ui->widgetDockStatistics->UpdateValidationStatus();
...
void cInterface::RequestRender(const QString &reason, const int debounceMs, const bool noUndo,
	const bool synchronizeWithUi, const bool setupMainImagePreviewInInit)
{
	requestDebouncedRender(
		noUndo, synchronizeWithUi, setupMainImagePreviewInInit, debounceMs, reason);
}

void cInterface::StartRender(const bool noUndo)
{
	RequestRender(QString(), 120, noUndo, true, true);
}

void cInterface::StartRenderFromCurrentParams(const bool noUndo)
{
	RequestRender(QStringLiteral("current params"), 0, noUndo, false, false);
}
...
bool cInterface::StopRender()
{
	...
	if (mainImage->IsUsed() && timerForAbortWarnings.elapsed() > 1000 * 60 * 10)
	{
		...
		closeResult = runModalOpenSync(messageBox);
	...
	std::shared_ptr<cFractalContainer> _fractalParams) const
{
	...
	try
	{
		...
		paramRender.UpdateIterFogEffectiveN(&fractals);

		const int qualityIndex = std::max(0, std::min(_params->Get<int>("auto_fog_quality"), 2));
		const autoFog::eAutoFogQuality quality =
			static_cast<autoFog::eAutoFogQuality>(qualityIndex);
		...
		{
			const bool wasIterFogEnabled = paramRender.iterFogEnabled;
			const bool wasVolFogEnabled = paramRender.volFogEnabled;
			const bool wasFogEnabled = paramRender.fogEnabled;
			paramRender.iterFogEnabled = true;
			paramRender.volFogEnabled = true;
			paramRender.fogEnabled = true;

			autoFogOk = autoFog.AutoTuneAll(&paramRender, &fractals, &renderData, quality,
				autoFog::eAutoFogTuneMode::All);

			paramRender.iterFogEnabled = wasIterFogEnabled;
			paramRender.volFogEnabled = wasVolFogEnabled;
			paramRender.fogEnabled = wasFogEnabled;

			if (autoFogOk)
				autoFog::WriteAutoFogResults(_params, paramRender, autoFog::eAutoFogTuneMode::All, false);
		}
	}
	catch (...)
	{
...
bool cInterface::QuitApplicationDialog()
{
	...
	switch (closeResult)
	{
		...
		default:
		{
			...
			WaitUntil([]() { return cRenderJob::GetRunningJobCount() <= 0; }, 600000, 50, true);
...
bool cInterface::DataFolderUpgrade() const
{
	...
	messageBox->setDefaultButton(QMessageBox::Ok);
	int dialogResult = runModalOpenSync(messageBox);
...
void cInterface::SaveLocalSettings(const QWidget *widget)
{
	...
	QObject::connect(dialog, &QDialog::accepted, this, [parSettings, dialog]() {
...
void cInterface::LoadLocalSettings(const QWidget *widget)
{
	...
	auto *dialog = new PreviewFileDialog(mainWindow);
	dialog->setAttribute(Qt::WA_DeleteOnClose);
	dialog->setOption(QFileDialog::DontUseNativeDialog);
	...
	QObject::connect(dialog, &QDialog::accepted, this, [this, widget, dialog]() {
		const QStringList filenames = dialog->selectedFiles();
		if (filenames.isEmpty()) return;
		const QString filename = QDir::toNativeSeparators(filenames.first());
...
{
	WaitUntil([]() { return cRenderJob::GetRunningJobCount() <= 0; }, 60000, 10, true);
	...
	for (const QString &parameterName : listOfTextureParameters)
	{
				...
				&& !textureFile.contains("http://") && (!fi.exists(textureFile) || fi.isDir()))
		{
			...
			else if (msgBox.clickedButton() == buttonSelect) // select file
			{
				...
				if (runModalOpenSync(&dialog) == QDialog::Accepted)
				{
					const QStringList filenames = dialog.selectedFiles();
