...
#ifdef ADAPTIVE_AO_SAMPLES
	{
		formulaOut probeOut =
			CalculateDistance(consts, input->point + input->normal * delta, calcParam, renderData);
		float curvature = fabs(probeOut.distance - input->lastDist) / delta;
		if (curvature < 0.05f)
			maxSamples = max(2, maxSamples / 4);
		else if (curvature > 0.3f)
			maxSamples = min(maxSamples * 2, quality * quality * 4);
	}
#endif
