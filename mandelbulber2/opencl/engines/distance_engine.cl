	...
	__constant sClInConstants *consts, float3 point)
{
...
#ifdef DEEP_ZOOM_ENABLED
	{
		...
		__global double *dzCP = (__global double *)&inBuff[deepZoomMainOffset + 16];
		__global double *dzSM = (__global double *)&inBuff[deepZoomMainOffset + 16 + 3 * sizeof(double)];
		int dzSS = GetInteger(deepZoomMainOffset + 16 + 12 * sizeof(double), inBuff);
		int dzSV = GetInteger(deepZoomMainOffset + 16 + 12 * sizeof(double) + sizeof(int), inBuff);
		int dzOO = GetInteger(deepZoomMainOffset + 16 + 12 * sizeof(double) + 2 * sizeof(int), inBuff);
		renderData.deepZoomOrbit = (__global double *)&inBuff[dzOO];
