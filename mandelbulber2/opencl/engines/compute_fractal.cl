		...
		if (mutationActive)
		{
				...
				switch (mut->mbMathType)
			{
				...
				if (i >= mut->skIterStart && i < mut->skIterStop && mut->symKalType != 0)
				{
					...
					switch(mut->symKalType)
					{
						...
						case 32: { { z.x = fabs(z.x); z.y = fabs(z.y); z.z = fabs(z.z); if(z.y > z.x) { float t=z.x; z.x=z.y; z.y=t; } if(z.z > z.x) { float t=z.x; z.x=z.z; z.z=t; } if(z.z > z.y) { float t=z.y; z.y=z.z; z.z=t; } z -= (float4)(sa, sb, sc, 0); } break; }
						...
						case 34: { { z.x = fabs(z.x); z.y = fabs(z.y); float d = z.x*native_cos(sang) + z.y*native_sin(sang); if(d < 0) { z.x -= 2.0*d*native_cos(sang); z.y -= 2.0*d*native_sin(sang); } z -= (float4)(sa, sb, 0, 0); } break; }
						...
						case 37: { { z.x = fabs(z.x); z.y = fabs(z.y); float a = sang; for(int k=0; k<3; k++) { float d = 2.0*fmin(0.0, z.x*native_cos(a) + z.y*native_sin(a)); z.x -= d*native_cos(a); z.y -= d*native_sin(a); a += M_PI_F/3.0; } z -= (float4)(sa, sb, 0, 0); } break; }
						...
						case 41: { { z.x = fabs(z.x); z.y = fabs(z.y); z.z = fabs(z.z); if(z.x - z.y < 0) { float t=z.x; z.x=z.y; z.y=t; } if(z.x - z.z < 0) { float t=z.x; z.x=z.z; z.z=t; } if(z.y - z.z < 0) { float t=z.y; z.y=z.z; z.z=t; } z *= sf; z -= (float4)(sa, sb, sc, 0) * (sf - 1.0); aux.DE = aux.DE * fabs(sf) + 1.0; } break; }
						case 42: { { if(z.x + z.y < 0) { float t=-z.y; z.y=-z.x; z.x=t; } if(z.x + z.z < 0) { float t=-z.z; z.z=-z.x; z.x=t; } if(z.y + z.z < 0) { float t=-z.z; z.z=-z.y; z.y=t; } z *= sf; z -= (float4)(sa, sb, sc, 0) * (sf - 1.0); aux.DE = aux.DE * fabs(sf) + 1.0; } break; }
						...
						case 44: { { z = fabs(z); if(z.x - z.y < 0) { float t=z.x; z.x=z.y; z.y=t; } if(z.x - z.z < 0) { float t=z.x; z.x=z.z; z.z=t; } if(z.y - z.z < 0) { float t=z.y; z.y=z.z; z.z=t; } z.z -= 0.5*sc*(sf-1.0)/sf; z.z = fabs(z.z); z.z += 0.5*sc*(sf-1.0)/sf; z *= sf; z -= (float4)(sa,sb,sc,0)*(sf-1.0); aux.DE = aux.DE*fabs(sf)+1.0; } break; }
						case 45: { { z = fabs(z); float t; if(z.x-z.y<0){t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0){t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0){t=z.y;z.y=z.z;z.z=t;} z *= sf; z -= (float4)(1,1,1,0)*(sf-1.0)*sa; aux.DE=aux.DE*fabs(sf)+1.0; z.x=fabs(z.x); z.y=fabs(z.y); } break; }
						case 46: { { z.x = fabs(z.x); z.y = fabs(z.y); z.z = fabs(z.z); float d1 = z.x - z.y; float d2 = z.y - z.z; if(d1 < 0){float t=z.x;z.x=z.y;z.y=t;} if(d2 < 0){float t=z.y;z.y=z.z;z.z=t;} if(z.x-z.y < 0){float t=z.x;z.x=z.y;z.y=t;} z -= (float4)(sa,sb,sc,0); z *= sf; z += (float4)(sa,sb,sc,0); aux.DE = aux.DE*fabs(sf)+1.0; } break; }
						...
						case 85: { { z.x=fabs(z.x); z.y=fabs(z.y); z.z=fabs(z.z); z *= sf + sam*native_sin(sfq*i); aux.DE *= fabs(sf + sam*native_sin(sfq*i)); z -= (float4)(sa,sb,sc,0)*(sf-1.0); } break; }
						...
						case 88: { { if(z.x+z.y<0){float t=-z.y;z.y=-z.x;z.x=t;} if(z.x+z.z<0){float t=-z.z;z.z=-z.x;z.x=t;} z *= sf; z -= (float4)(sa,sb,sc,0)*(sf-1.0); aux.DE=aux.DE*fabs(sf)+1.0; if(z.y-z.z<0){float t=z.y;z.y=z.z;z.z=t;} } break; }
						...
						case 90: { { float p=fmax(sa,0.01); z.x=z.x-p*round(z.x/p); z.y=z.y-p*round(z.y/p); z.x=fabs(z.x); z.y=fabs(z.y); if(z.x<z.y){float t=z.x;z.x=z.y;z.y=t;} z *= sf; aux.DE *= fabs(sf); z -= (float4)(sb,sc,0,0)*(sf-1.0); } break; }
						case 91: { { z.x=fabs(z.x)-sa; z.y=fabs(z.y)-sb; z.z=fabs(z.z)-sc; if(z.x<z.y){float t=z.x;z.x=z.y;z.y=t;} if(z.x<z.z){float t=z.x;z.x=z.z;z.z=t;} if(z.y<z.z){float t=z.y;z.y=z.z;z.z=t;} z*=sf; aux.DE*=fabs(sf); z+=(float4)(sa,sb,sc,0); } break; }
						...
						case 96: { { z=fabs(z); if(z.x<z.y){float t=z.x;z.x=z.y;z.y=t;} if(z.x<z.z){float t=z.x;z.x=z.z;z.z=t;} if(z.y<z.z){float t=z.y;z.y=z.z;z.z=t;} z-=(float4)(sa,sb,sc,0); float rr=z.x*z.x+z.y*z.y+z.z*z.z; if(rr<sd*sd){float m=sf*sf/fmax(rr,1e-21);z*=m;aux.DE*=m;} z*=sf;aux.DE*=fabs(sf); z+=(float4)(sa,sb,sc,0); } break; }
				...
				if (i >= mut->abIterStart && i < mut->abIterStop && mut->aboxType != 0)
				{
					...
					switch(mut->aboxType)
					{
						...
						case 45: { aux.DE *= fmax(0.01, 1.0 + af * aa * pow(fabs(native_sin(ab*r)), ac)); break; }
						...
						case 167: { { float v=0; for(int o=0;o<4;o++){v+=native_sin(aa*pow(2.0,o)*r+ab*o)/(pow(2.0,o));} aux.DE*=(1.0+af*ac*v); } break; }
						...
						case 188: { { float fr=pow(fabs(1.0-fabs(zz2)/r),5.0); aux.DE*=(1.0+aa*fr); } break; }
						...
						case 219: { { float v=0; for(int o=0;o<4;o++){v+=native_sin(aa*pow(2.0,o)*r)/pow(2.0,o);} aux.DE*=(1.0+ab*fabs(v)); } break; }
						...
						case 256: { { float d=fabs(r-aa); aux.DE*=fmax(0.01,pow(d+1e-21,-ab)); } break; }
			...
			if (i >= mut->orbitIterStart && i < mut->orbitIterStop && mut->orbitTrapType != 0)
			{
				...
				switch(mut->orbitTrapType) {
					...
					case 73: { float r=native_sqrt(z.x*z.x+z.y*z.y+z.z*z.z); float prevR=native_sqrt(point.x*point.x+point.y*point.y+point.z*point.z); float dr=fabs(r-prevR); aux.DE *= (1.0 + of*native_exp(-od*dr)); break; }
			...
			if (i >= mut->torusIterStart && i < mut->torusIterStop && mut->torusType != 0)
			{
				...
				if (i >= mut->asIterStart && i < mut->asIterStop && mut->asType != 0)
				{
					...
					switch(mut->asType) {
						...
						case 49: { float decay = native_exp(-sf*fabs(ta)*(float)i*0.01); aux.DE *= decay; break; }
						...
						case 86: { float decay_t = native_exp(-fabs(ta)*(float)i*0.01); aux.DE *= (1.0 + sf*tb*decay_t*tc); break; }
						case 87: { float baryon = native_sin(z.x*ta)*native_sin(z.y*tb); float rate = native_exp(-fabs(tc)*(float)i*0.01); aux.DE *= (1.0 + sf*baryon*baryon*rate*td); break; }
						...
						case 91: { float ent_max = fabs(ta); float heat_time = fmax(fabs(tb), 0.01); aux.DE *= (1.0 + sf*ent_max*(1.0-native_exp(-(float)i*0.01/heat_time))); break; }
				...
				if (i >= mut->smIterStart && i < mut->smIterStop && mut->smType != 0)
				{
					...
					switch(mut->smType) {
						...
						case 47: { float sc=3.0; z=fabs(z); if(z.x-z.y<0.0){float t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){float t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){float t=z.y;z.y=z.z;z.z=t;} z=z*sc-(float4)(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; float k_angle=ta*M_PI_F/180.0f; float3 nv=(float3)(native_cos(k_angle),native_sin(k_angle),0.0f); float3 zv3=(float3)(z.x,z.y,z.z); float dt=zv3.x*nv.x+zv3.y*nv.y+zv3.z*nv.z; if(dt<0){zv3=zv3-nv*(2.0f*dt); z=(float4)(zv3.x,zv3.y,zv3.z,z.w);} break; }
						...
						case 49: { float sc=3.0+0.1*native_sin((float)i*ta); z=fabs(z); if(z.x-z.y<0.0){float t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){float t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){float t=z.y;z.y=z.z;z.z=t;} z=z*sc-(float4)(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*fabs(sc)+1.0; z=(float4)(0.9*z.x+0.1*ta,0.9*z.y+0.1*tb,0.9*z.z+0.1*tc,z.w); break; }
						case 50: { float h1=native_sin(ta*z.x+tb*z.y+tc*z.z); float h2=native_sin(td*h1+ta*(float)i*0.1); float sc=3.0+sf*h2*0.1; z=fabs(z); if(z.x-z.y<0.0){float t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){float t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){float t=z.y;z.y=z.z;z.z=t;} z=z*sc-(float4)(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*fabs(sc)+1.0; break; }
						case 51: { float sc=3.0; float4 zz=fabs(z); float vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){float t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=(float4)(vals[0],vals[1],vals[2],vals[3])*sc-(float4)(1,1,1,1)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						...
						case 53: { float sc=3.0; z.w=ta; float4 zz=fabs(z); float vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){float t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=(float4)(vals[0],vals[1],vals[2],vals[3])*sc-(float4)(1,1,1,1)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 54: { float angle4d=ta*M_PI_F/180.0*(float)i; float ca=native_cos(angle4d),sa=native_sin(angle4d); float nx=z.x*ca-z.w*sa; z.w=z.x*sa+z.w*ca; z.x=nx; float sc=3.0; float4 zz=fabs(z); float vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){float t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=(float4)(vals[0],vals[1],vals[2],vals[3])*sc-(float4)(1,1,1,1)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 55: { float sc=3.0; float4 zz=fabs(z); float vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){float t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=(float4)(vals[0],vals[1],vals[2],vals[3])*sc-(float4)(1,1,1,1)*(sc-1.0)+(float4)(ta,tb,tc,td); aux.DE=aux.DE*sc+1.0; break; }
						case 56: { float sc=3.0; float qn=native_sqrt(z.x*z.x+z.y*z.y+z.z*z.z+z.w*z.w); z.w=qn*ta; float4 zz=fabs(z); float vals[4]={zz.x,zz.y,zz.z,zz.w}; for(int a=0;a<4;a++)for(int b=a+1;b<4;b++){if(vals[a]<vals[b]){float t=vals[a];vals[a]=vals[b];vals[b]=t;}} z=(float4)(vals[0],vals[1],vals[2],vals[3])*sc-(float4)(1,1,1,1)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						...
						case 58: { float r=native_sqrt(z.x*z.x+z.y*z.y+z.z*z.z+z.w*z.w); float d=tanh(fmin(r*ta,10.0)); float4 unit=z/fmax(r,1e-21); z=unit*d; float sc=3.0; z=fabs(z); if(z.x-z.y<0.0){float t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){float t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){float t=z.y;z.y=z.z;z.z=t;} z=z*sc-(float4)(1,1,1,0)*(sc-1.0); float ch=cosh(fmin(r*ta,20.0)); aux.DE=aux.DE*sc/(ch*ch)+1.0; break; }
						...
						case 63: { float golden=2.39996322972865332; float theta=(float)i*golden*ta; z.x+=native_cos(theta)*tb*0.01; z.y+=native_sin(theta)*tb*0.01; z.z+=native_cos(theta*0.5)*tc*0.01; z.w=native_sin(theta*0.5)*td*0.01; float sc=3.0; z=fabs(z); if(z.x-z.y<0.0){float t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){float t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){float t=z.y;z.y=z.z;z.z=t;} z=z*sc-(float4)(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						...
						case 97: { float angle=ta*M_PI_F/180.0*(float)i; float ca=native_cos(angle),sa=native_sin(angle); float nx=z.x*ca-z.y*sa; z.y=z.x*sa+z.y*ca; z.x=nx; float sc=3.0; z=fabs(z); if(z.x-z.y<0.0){float t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){float t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){float t=z.y;z.y=z.z;z.z=t;} z=z*sc-(float4)(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*sc+1.0; break; }
						case 98: { float sc=3.0*(1.0+sf*0.1*native_sin((float)i*ta)); z=fabs(z); if(z.x-z.y<0.0){float t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){float t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){float t=z.y;z.y=z.z;z.z=t;} z=z*sc-(float4)(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*fabs(sc)+1.0; break; }
						...
						case 100: { float h1=native_sin(ta*z.x+tb*z.y+tc*z.z); float h2=native_sin(td*h1+ta*(float)i*0.1); float sc=3.0+sf*h2*0.05; z=fabs(z); if(z.x-z.y<0.0){float t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0.0){float t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0.0){float t=z.y;z.y=z.z;z.z=t;} z=z*sc-(float4)(1,1,1,0)*(sc-1.0); aux.DE=aux.DE*fabs(sc)+1.0; break; }
					}
				}
