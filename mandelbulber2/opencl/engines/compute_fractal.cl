				...
				switch (mut->mbMathType)
			{
				...
				if (i >= mut->skIterStart && i < mut->skIterStop && mut->symKalType != 0)
				{
					...
					switch(mut->symKalType)
					{
						...
						case 45: { { z = fabs(z); float t; if(z.x-z.y<0){t=z.x;z.x=z.y;z.y=t;} if(z.x-z.z<0){t=z.x;z.x=z.z;z.z=t;} if(z.y-z.z<0){t=z.y;z.y=z.z;z.z=t;} z *= sf; z -= (float4)(1,1,1)*(sf-1.0)*sa; aux.DE=aux.DE*fabs(sf)+1.0; z.x=fabs(z.x); z.y=fabs(z.y); } break; }
