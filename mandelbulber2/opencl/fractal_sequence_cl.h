...
typedef struct
{
	...
	cl_float juliaDecayRate;
	cl_float juliaSpiralAngle;
	cl_float juliaChaosR;
	cl_float juliaThreshold;
	cl_float juliaGradientEnd;
	cl_float juliaLayerC2x, juliaLayerC2y, juliaLayerC2z;
	cl_float juliaBoxFoldLimit;
	cl_float juliaStartEpsilon;
	cl_float juliaDistBandMid;
	cl_float juliaDistBandFar;
	// v7.9 — Per-subsystem iteration ranges for Julia
	cl_int juliaCTransformIterStart, juliaCTransformIterStop;
	cl_int juliaDynamicIterStart, juliaDynamicIterStop;
	cl_int juliaMultiIterStart, juliaMultiIterStop;
	// v7.9 — Fine-tune strengths
	cl_float juliaCTransformStrength;
	cl_float juliaDynamicStrength;
	cl_float juliaMultiStrength;
	cl_float juliaInjectionStrength;
	cl_float juliaDEFactor;
	...
	cl_int clampType;
	cl_float clampFactor;
	cl_float clampParamA, clampParamB, clampParamC, clampParamD;
	cl_int clampIterStart, clampIterStop;
	// v7.14 — T Julia Box system
	cl_int jbType;
	cl_float jbFactor;
	cl_float jbParamA, jbParamB, jbParamC, jbParamD;
	cl_int jbIterStart, jbIterStop;
	// v7.14 — T Mandalay system
	cl_int mdType;
	cl_float mdFactor;
	cl_float mdParamA, mdParamB, mdParamC, mdParamD;
	cl_int mdIterStart, mdIterStop;
	// v7.15 — Blockify system
	cl_int blockifyType;
	cl_float blockifyFactor;
	cl_float blockifyParamA, blockifyParamB, blockifyParamC, blockifyParamD;
	cl_int blockifyIterStart, blockifyIterStop;
	// v7.15 — Tile system
	cl_int tileType;
	cl_float tileFactor;
	cl_float tileParamA, tileParamB, tileParamC, tileParamD;
	cl_int tileIterStart, tileIterStop;
	// Per-subsystem enabled flags (aligned on 4-byte boundary)
	cl_int inversionEnabled;
	cl_int clipEnabled;
	cl_int josLeysEnabled;
	cl_int pkEnabled;
	cl_int mbMathEnabled;
	cl_int warpDistEnabled;
	cl_int symmetryEnabled;
	cl_int aboxEnabled;
	cl_int noiseEnabled;
	cl_int orbitTrapEnabled;
	cl_int torusEnabled;
	cl_int asEnabled;
	cl_int smEnabled;
	cl_int clampEnabled;
	cl_int jbEnabled;
	cl_int mdEnabled;
	cl_int blockifyEnabled;
	cl_int tileEnabled;
	// Section weights (0.0-1.0, multiplied with factor)
	cl_float inversionWeight;
	cl_float clipWeight;
	cl_float josLeysWeight;
	cl_float pkWeight;
	cl_float mbMathWeight;
	cl_float warpDistWeight;
	cl_float symmetryWeight;
	cl_float aboxWeight;
	cl_float noiseWeight;
	cl_float orbitTrapWeight;
	cl_float torusWeight;
	cl_float clampWeight;
	cl_float jbWeight;
	cl_float mdWeight;
	cl_float asWeight;
	cl_float smWeight;
	cl_float blockifyWeight;
	cl_float tileWeight;
...
typedef struct
{
	...
	cl_float4 juliaCIterStart[NUMBER_OF_FRACTALS];
	cl_float4 juliaCIterStop[NUMBER_OF_FRACTALS];
	cl_float4 juliaCWeight[NUMBER_OF_FRACTALS];
	cl_float4 constCIterStart[NUMBER_OF_FRACTALS];
	cl_float4 constCIterStop[NUMBER_OF_FRACTALS];
	cl_float4 constCWeight[NUMBER_OF_FRACTALS];
	cl_float4 formulaPosition[NUMBER_OF_FRACTALS];
	cl_float4 formulaRotation[NUMBER_OF_FRACTALS];
	cl_float4 formulaRepeat[NUMBER_OF_FRACTALS];
	cl_float4 positionIterStart[NUMBER_OF_FRACTALS];
	cl_float4 positionIterStop[NUMBER_OF_FRACTALS];
	cl_float4 rotationIterStart[NUMBER_OF_FRACTALS];
	cl_float4 rotationIterStop[NUMBER_OF_FRACTALS];
	cl_float4 repeatIterStart[NUMBER_OF_FRACTALS];
	cl_float4 repeatIterStop[NUMBER_OF_FRACTALS];
