...
// Deep-zoom ray helpers: always available (macros avoid NVIDIA ptxas unresolved-extern on inline calls)
#ifndef DS_DEEP_ZOOM_HELPERS_DEFINED
#define DS_DEEP_ZOOM_HELPERS_DEFINED

#define ds_deep_zoom_camera(camera, cameraHi, cameraLo, useDeepZoom) \
	((!(useDeepZoom)) ? (camera) : ((float3)((cameraHi) + (cameraLo))))

#define ds_deep_zoom_ray_point(cameraHi, cameraLo, start, direction, scan, step, useDeepZoom) \
	(((useDeepZoom) && ((step) % 64 == 0)) \
		? ((float3)((cameraHi) + (cameraLo)) + (direction) * (scan)) \
		: ((start) + (direction) * (scan)))
