 ...
 *
 * Mandalay Fold: Persistente homologie in Mandalay Fold.
 */

REAL4 MandalayFoldPersistentHomologyIteration(REAL4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	...
	// Persistent Homology fold: barcode persistence
	REAL s = fractal->transformCommon.scale08;
	REAL birth = min(fabs(z.x), min(fabs(z.y), fabs(z.z)));
	REAL death = max(fabs(z.x), max(fabs(z.y), fabs(z.z)));
	REAL persist = death - birth;
	z *= (1.0 + persist * s * 0.1);
