...
REAL4 Threex3MitochondrialNetworkFractalIteration(REAL4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	...
	REAL symmetry = fractal->transformCommon.scale1;
	if (symmetry < 2.0) symmetry = 6.0;
	REAL th = asin(z.z / r) * power;
	REAL ph = atan2(z.y, z.x) * power;
	// Faceting via angular quantization
	REAL phQ = round(ph * symmetry / (2.0 * M_PI_F)) * 2.0 * M_PI_F / symmetry;
	REAL blend = fractal->transformCommon.offset0;
