 ...
 *
 * Wigner-afbeelding.
 * Math: x' = x*cos(alpha) - (p - x²)*sin(alpha), p' = x*sin(alpha) + (p - x²)*cos(alpha)
 */

#include "all_fractal_definitions.h"

cFractalThreex3WignerMap::cFractalThreex3WignerMap() : cAbstractFractal()
{
	nameInComboBox = "3x3 V217 Wigner Map";
	internalName = "threex3_wigner_map";
	internalID = fractal::threex3WignerMap;
...
void cFractalThreex3WignerMap::FormulaCode(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	// Wigner Map: x' = x*cos(alpha) - (p - x²)*sin(alpha), p' = x*sin(alpha) + (p - x²)*cos(alpha)
