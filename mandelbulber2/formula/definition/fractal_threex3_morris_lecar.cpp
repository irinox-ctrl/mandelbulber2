 ...
 *
 * Morris-Lecar.
 * Math: dV/dt = -gCa*m_inf(V)*(V-VCa) - gK*w*(V-VK) - gL*(V-VL) + I, dw/dt = phi*(w_inf(V) - w)/tau_w(V)
 */

#include "all_fractal_definitions.h"

cFractalThreex3MorrisLecar::cFractalThreex3MorrisLecar() : cAbstractFractal()
{
	nameInComboBox = "3x3 V228 Morris-Lecar";
	internalName = "threex3_morris_lecar";
	internalID = fractal::threex3MorrisLecar;
...
void cFractalThreex3MorrisLecar::FormulaCode(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	// Morris-Lecar: dV/dt = -gCa*m_inf(V)*(V-VCa) - gK*w*(V-VK) - gL*(V-VL) + I, dw/dt = phi*(w_inf(V) - w)/tau_w(V)
