 ...
 *
 * Sinaï-afbeelding.
 * Math: x_{n+1} = x_n + y_n mod 1, y_{n+1} = y_n + k*sin(2*pi*x_n) mod 1
 */

#include "all_fractal_definitions.h"

cFractalThreex3SinaiMap::cFractalThreex3SinaiMap() : cAbstractFractal()
{
	nameInComboBox = "3x3 V211 Sinai Map";
	internalName = "threex3_sinai_map";
	internalID = fractal::threex3SinaiMap;
...
void cFractalThreex3SinaiMap::FormulaCode(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	// Sinai Map: x_{n+1} = x_n + y_n mod 1, y_{n+1} = y_n + k*sin(2*pi*x_n) mod 1
