 ...
 *
 * Hoogte-functie Julia.
 * Math: z = z² + c met h(phi^n(P)) ~ d^n * h(P)
 */

#include "all_fractal_definitions.h"

cFractalThreex3HeightsJulia::cFractalThreex3HeightsJulia() : cAbstractFractal()
{
	nameInComboBox = "3x3 V35 Heights Julia";
	internalName = "threex3_heights_julia";
	internalID = fractal::threex3HeightsJulia;
...
void cFractalThreex3HeightsJulia::FormulaCode(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	// Heights Julia: z = z² + c met h(phi^n(P)) ~ d^n * h(P)
	double r = aux.r;
	if (r < 1e-21) r = 1e-21;
