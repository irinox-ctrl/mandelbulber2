 ...
 *
 * Fout-functie Julia.
 * Math: z = erf(z) + c = 2/sqrt(pi)*integral(exp(-t²)) + c
 */

#include "all_fractal_definitions.h"

cFractalThreex3ErrorFunctionJulia::cFractalThreex3ErrorFunctionJulia() : cAbstractFractal()
{
	nameInComboBox = "3x3 V91 Error Function Julia";
	internalName = "threex3_error_function_julia";
	internalID = fractal::threex3ErrorFunctionJulia;
...
void cFractalThreex3ErrorFunctionJulia::FormulaCode(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	// Error Function Julia: z = erf(z) + c = 2/sqrt(pi)*integral(exp(-t²)) + c
	double r = aux.r;
	if (r < 1e-21) r = 1e-21;
