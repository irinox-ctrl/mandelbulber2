 ...
 *
 * Canonieke hoogte Julia.
 * Math: z = z² + c met h_canonical(P) = lim h(phi^n(P))/d^n
 */

#include "all_fractal_definitions.h"

cFractalThreex3CanonicalHeightJulia::cFractalThreex3CanonicalHeightJulia() : cAbstractFractal()
{
	nameInComboBox = "3x3 V36 Canonical Height Julia";
	internalName = "threex3_canonical_height_julia";
	internalID = fractal::threex3CanonicalHeightJulia;
...
void cFractalThreex3CanonicalHeightJulia::FormulaCode(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	// Canonical Height Julia: z = z² + c met h_canonical(P) = lim h(phi^n(P))/d^n
	double r = aux.r;
	if (r < 1e-21) r = 1e-21;
