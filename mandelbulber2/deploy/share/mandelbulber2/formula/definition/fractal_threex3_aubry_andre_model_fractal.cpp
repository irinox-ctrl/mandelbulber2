 ...
 *
 * Aubry-André-model fractaal.
 * Math: H = sum(c_n+1*c_n + h.c.) + V*sum(cos(2*pi*alpha*n)*c_n*c_n)
 */

#include "all_fractal_definitions.h"

cFractalThreex3AubryAndreModelFractal::cFractalThreex3AubryAndreModelFractal() : cAbstractFractal()
{
	nameInComboBox = "3x3 V411 Aubry-Andre Model Fractal";
	internalName = "threex3_aubry_andre_model_fractal";
	internalID = fractal::threex3AubryAndreModelFractal;
...
void cFractalThreex3AubryAndreModelFractal::FormulaCode(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	// Aubry-Andre Model Fractal: H = sum(c_n+1*c_n + h.c.) + V*sum(cos(2*pi*alpha*n)*c_n*c_n)
