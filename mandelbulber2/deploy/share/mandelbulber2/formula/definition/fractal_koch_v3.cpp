...
void cFractalKochV3::FormulaCode(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	...
	{
		double kFoldAngle = fractal->transformCommon.kochFoldAngle;
		double kFoldAngleFreq = fractal->transformCommon.kochFoldAngleFreq;
		double kScaleOsc = fractal->transformCommon.kochScaleOsc;
		double kScaleOscFreq = fractal->transformCommon.kochScaleOscFreq;
		double kOffOscX = fractal->transformCommon.kochOffsetOscX;
		double kOffOscY = fractal->transformCommon.kochOffsetOscY;
		double kOffOscZ = fractal->transformCommon.kochOffsetOscZ;
		double kOffOscFreq = fractal->transformCommon.kochOffsetOscFreq;
		double kPreRot = fractal->transformCommon.kochPreRotAngle;
		double kPostRot = fractal->transformCommon.kochPostRotAngle;
		double kYOffWarp = fractal->transformCommon.kochYOffWarp;
		double kTwistZ = fractal->transformCommon.kochTwistZ;
		double kRadDist = fractal->transformCommon.kochRadialDistort;
		double kTurb = fractal->transformCommon.kochTurbulence;
		double kGradColor = fractal->transformCommon.kochGradientColor;
		double kDETweak = fractal->transformCommon.kochDETweak;
		double kCpixel = fractal->transformCommon.kochCpixelInject;
		double kAniso = fractal->transformCommon.kochAnisotropic;
		double kSphere = fractal->transformCommon.kochSphereFold;
		double kEdgeSoft = fractal->transformCommon.kochEdgeSoftness;

		// 1. Fold angle oscillation
		if (kFoldAngle != 0.0)
		{
			double angle = kFoldAngle * sin(aux.i * kFoldAngleFreq * 0.5);
			double cosA = cos(angle);
			double sinA = sin(angle);
			double nx = z.x * cosA - z.y * sinA;
			double ny = z.x * sinA + z.y * cosA;
			z.x = nx;
			z.y = ny;
		}
		...
		if (kScaleOsc != 0.0)
		{
			double sOsc = 1.0 + kScaleOsc * sin(aux.i * kScaleOscFreq * 0.5);
			z *= sOsc;
			aux.DE *= fabs(sOsc);
		}

		// 4-7. Offset oscillation (per axis)
		if (kOffOscX != 0.0 || kOffOscY != 0.0 || kOffOscZ != 0.0)
		{
			double oPhase = aux.i * kOffOscFreq * 0.5;
			z.x += kOffOscX * sin(oPhase);
			z.y += kOffOscY * sin(oPhase * 1.3);
			z.z += kOffOscZ * sin(oPhase * 0.7);
		}

		// 8. Pre-rotation
		if (kPreRot != 0.0)
		{
			double a = kPreRot * M_PI / 180.0 * aux.i;
		...
		if (kPostRot != 0.0)
		{
			double a = kPostRot * M_PI / 180.0;
			double ca = cos(a); double sa = sin(a);
			double py = z.y * ca - z.z * sa;
			double pzz = z.y * sa + z.z * ca;
			z.y = py; z.z = pzz;
		}
		...
		if (kYOffWarp != 0.0)
		{
			z.y += kYOffWarp * sin(z.x * 3.0 + aux.i);
		}

		// 11. Twist Z
		if (kTwistZ != 0.0)
		{
			double tw = kTwistZ * M_PI / 180.0 * z.z;
		...
		if (kRadDist != 0.0)
		{
			...
			if (r > 1e-15)
			{
				double distort = 1.0 + kRadDist * sin(r * 5.0);
				z *= distort;
				aux.DE *= fabs(distort);
			}
		}
