 ...
 *
 * Mandalay Fold: Minkowski-som in Mandalay Fold.
 */

REAL4 MandalayFoldMinkowskiSumIteration(REAL4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	...
	// Minkowski Sum fold: A+B dilation
	REAL s = fractal->transformCommon.scale08;
	REAL bx = 0.5 * sign(z.x);
	REAL by = 0.5 * sign(z.y);
	REAL bz = 0.5 * sign(z.z);
	z.x += bx * s;
	z.y += by * s;
	z.z += bz * s;
