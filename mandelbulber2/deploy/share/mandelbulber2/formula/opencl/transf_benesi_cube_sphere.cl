...
REAL4 TransfBenesiCubeSphereIteration(REAL4 z, __global const sFractalCl *fractal, sExtendedAuxCl *aux)
{
	Q_UNUSED(fractal);
	// Q_UNUSED(aux);
	REAL4 oldZ = z;
	...
	REAL rCxyz = (z.y + z.z) / z.x;

	if (rCxyz == -1.0f) z.z = 1.0f; //+ 1e-21f
	if (rCyz < 1.0f)
		rCyz = native_sqrt(rCyz + 1.0f);
	else
		rCyz = native_sqrt(1.0f / rCyz + 1.0f);

	if (rCxyz < 1.0f)
		rCxyz = native_sqrt(rCxyz + 1.0f);
	else
		rCxyz = native_sqrt(1.0f / rCxyz + 1.0f);
