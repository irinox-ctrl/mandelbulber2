...
#ifdef QT_DEBUG
			if (!gradientSurface.isEmpty())
			{
				const cColorGradient::sColor &firstColor = gradientSurface.first();
				qDebug() << "OpenCL palette build: surface entries" << paletteSizeSurface << "first RGB"
								 << firstColor.color.R / 256.0 << firstColor.color.G / 256.0
								 << firstColor.color.B / 256.0;
			}
#endif
...
void cOpenClDynamicData::BuildLightsData(
	const cLights *lights, const QMap<QString, int> &textureIndexes, const CVector3 &positionShift)
{
	/* use __attribute__((aligned(16))) in kernel code for array
	 *
	 * header:
	...
	for (int i = 0; i < numberOfLights; i++)
	{
		...
		// world-positioned lights follow the deep zoom coordinate re-centering
		CVector3 lightPosition = light->position;
		CVector3 lightTarget = light->target;
		if (!light->relativePosition)
		{
			lightPosition += positionShift;
			lightTarget += positionShift;
		}
		lightCl.position = toClFloat3(lightPosition);
		lightCl.rotation = toClFloat3(light->rotation);
		lightCl.target = toClFloat3(lightTarget);
...
void cOpenClDynamicData::BuildDeepZoomData(const double *orbitData, int orbitLength,
	float power, float bailout, float rebaseThreshold,
	double centerX, double centerY, double centerZ,
	const double *saMatrix, int saSkipIters, bool saValid)
{
	/* Deep Zoom perturbation data for GPU (fp64, requires cl_khr_fp64)
	 *
	 * header (base aligned to 16):
	 * cl_int orbitLength        offset 0
	 * cl_float power            offset 4
	 * cl_float bailout          offset 8
	 * cl_float rebaseThreshold  offset 12
	 * cl_double centerX, centerY, centerZ   offset 16
	 * cl_double saMatrix[9]     offset 40
	 * cl_int saSkipIters        offset 112
	 * cl_int saValid            offset 116
	 * cl_int orbitDataOffset    offset 120
	 *
	 * array (aligned to 16):
	 *   double[8] orbitPoint0 (Z.x, Z.y, Z.z, DE, r, theta, phi, escaped)
	 *   double[8] orbitPoint1
	 *   ...
	 */
	...
	// orbit data: orbitLength * 8 doubles per point
	int orbitBytes = orbitLength * 8 * sizeof(cl_double);
...
void cOpenClDynamicData::BuildVoxelFogCacheData(const sParamRender *params, const cLights *lights)
{
	totalDataOffset += PutDummyToAlign(totalDataOffset, 16, &data);
	itemOffsets[voxelFogCacheItemIndex].itemOffset = totalDataOffset;

	cl_int voxelRes = 0;
	if (params && lights && params->voxelFogCacheEnabled && params->volFogEnabled)
		voxelRes = qBound(16, params->voxelFogCacheResolution, 64);

	data.append(reinterpret_cast<char *>(&voxelRes), sizeof(voxelRes));
	totalDataOffset += sizeof(voxelRes);

	cl_int arrayOffset = 0;
	const int arrayOffsetAddress = totalDataOffset;
	data.append(reinterpret_cast<char *>(&arrayOffset), sizeof(arrayOffset));
	totalDataOffset += sizeof(arrayOffset);

	if (voxelRes <= 0) return;

	totalDataOffset += PutDummyToAlign(totalDataOffset, 16, &data);
	arrayOffset = totalDataOffset;

	const CVector3 voxelMin = params->voxelFogCacheMin;
	const CVector3 voxelSize = params->voxelFogCacheSize;
	const double delta = params->DEThresh * 0.1;
	const double resolution = params->resolution;
	const double viewDistanceMax = params->viewDistanceMax;
	...
	for (int z = 0; z < voxelRes; z++)
	{
		for (int y = 0; y < voxelRes; y++)
		{
			for (int x = 0; x < voxelRes; x++)
			{
				...
				const CVector3 point = voxelMin
					+ CVector3((x + 0.5) * voxelSize.x, (y + 0.5) * voxelSize.y, (z + 0.5) * voxelSize.z);

				cl_float4 irradianceAndDensity = {0.0f, 0.0f, 0.0f, float(params->volFogDensity)};
				cl_int valid = 1;
				cl_int pad[3] = {0, 0, 0};
				...
				for (int i = 0; i < lights->GetNumberOfLights(); i++)
				{
					...
					const CVector3 lightVector = light->CalculateLightVector(
						point, delta, resolution, viewDistanceMax, distanceLight);

					double intensity = light->intensity;
					if (light->type != cLight::lightDirectional && distanceLight > 0.0)
						intensity /= light->Decay(distanceLight);

					sRGBFloat coneColor;
					light->CalculateCone(point, lightVector, coneColor);

					irradianceAndDensity.s0 += float(light->color.R * intensity * coneColor.R);
					irradianceAndDensity.s1 += float(light->color.G * intensity * coneColor.G);
					irradianceAndDensity.s2 += float(light->color.B * intensity * coneColor.B);
				}

				data.append(reinterpret_cast<char *>(&irradianceAndDensity), sizeof(irradianceAndDensity));
				totalDataOffset += sizeof(irradianceAndDensity);
				data.append(reinterpret_cast<char *>(&valid), sizeof(valid));
				totalDataOffset += sizeof(valid);
				data.append(reinterpret_cast<char *>(pad), sizeof(pad));
				totalDataOffset += sizeof(pad);
			}
		}
	}
