		...
		if (props.allowed)
		{
			...
			if (props.typeName == QString("vect3") || props.typeName == QString("logvect3"))
			{
				...
				else if (mode == qInterface::write)
				{
					...
					const QString newText = QString("%L1").arg(value, 0, 'g', 15);
		...
		if (props.allowed)
		{
			...
			if (props.typeName == QString("spinbox") || props.typeName == QString("spinboxd"))
			{
				...
				else if (mode == qInterface::write)
				{
					...
					if (!qFuzzyCompare(spinbox->value(), value)) spinbox->setValue(value);
					if (props.className == QString("MyDoubleSpinBox"))
					{
						MyDoubleSpinBox *myDoubleSpinBox = static_cast<MyDoubleSpinBox *>(*it);
		...
		if (props.allowed)
		{
			...
			if (props.typeName == QString("spinboxInt"))
			{
				...
				else if (mode == qInterface::write)
				{
					...
					if (spinbox->value() != value) spinbox->setValue(value);
				}
			}
		}
	}
		...
		if (props.allowed)
		{
			...
			if (props.typeName == QString("checkBox"))
			{
				...
				else if (mode == qInterface::write)
				{
					...
					if (checkbox->isChecked() != value) checkbox->setChecked(value);
				}
			}
		}
	}
	...
	for (it = widgets.begin(); it != widgets.end(); ++it)
	{
		...
		if (props.allowed)
		{
			...
			if (props.typeName == QString("groupCheck"))
			{
				...
				{
					bool value = par->Get<bool>(props.paramName);
					QSignalBlocker blocker(groupBbox);
					if (groupBbox->isChecked() != value) groupBbox->setChecked(value);
				}
			}
		}
	}
	...
	QList<FileSelectWidget *> widgets, std::shared_ptr<cParameterContainer> par, enumReadWrite mode)
{
	...
	for (it = widgets.begin(); it != widgets.end(); ++it)
	{
		...
		if (props.allowed)
		{
			...
			{
				const QString path = par->Get<QString>(props.paramName);
				if (fileSelectWidget->GetPath() != path) fileSelectWidget->SetPath(path);
			}
		}
	}
}
	...
	QList<MyColorButton *> widgets, std::shared_ptr<cParameterContainer> par, enumReadWrite mode)
{
	...
	for (it = widgets.begin(); it != widgets.end(); ++it)
	{
		...
		if (props.allowed)
		{
			...
			{
				const sRGB color = par->Get<sRGB>(props.paramName);
		...
		if (props.allowed)
		{
			...
			if (props.typeName == QString("comboBox"))
			{
				...
				else if (mode == qInterface::write)
				{
					...
					if (par->GetVarType(props.paramName) == typeString)
					{
						const QString text = par->Get<QString>(props.paramName);
						for (int i = 0; i < comboBox->count(); i++)
						{
							if (comboBox->itemText(i) == text)
							{
								if (comboBox->currentIndex() != i) comboBox->setCurrentIndex(i);
								break;
							}
						}
					}
					else if (comboBox->currentIndex() != selection)
					{
	...
	for (it = widgets.begin(); it != widgets.end(); ++it)
	{
		...
		if (props.allowed)
		{
			...
			if (props.typeName == QString("text"))
			{
				...
				{
					const QString value = par->Get<QString>(props.paramName);
					QSignalBlocker blocker(textEdit);
					if (textEdit->toPlainText() != value) textEdit->setPlainText(value);
				}
			}
		}
	} // end foreach
	...
	for (it = widgets.begin(); it != widgets.end(); ++it)
	{
		...
		if (props.allowed)
		{
			...
			if (props.typeName == QString("colorpalette"))
			{
				...
				{
					const QString palette = par->Get<QString>(props.paramName);
					if (colorGradientWidget->GetColors() != palette)
	...
	for (it = widgets.begin(); it != widgets.end(); ++it)
	{
		...
		if (props.allowed)
		{
			...
			if (props.typeName == QString("keySequenceEdit"))
			{
				...
				{
					const QString keySequenceString = par->Get<QString>(props.paramName);
					const QKeySequence keySequence(keySequenceString, QKeySequence::PortableText);
					QSignalBlocker blocker(keySequenceEdit);
