	...
	for (int y = startLineInit; y < endLine; y += threadData->noOfThreads)
	{
		...
		for (int x = startX; x < endX; x += step)
		{
			...
			if (z < 1e19)
			{
				...
				bool usedNormalSSAO = false;
				if (params->SSAO_useNormalBuffer && image->GetImageOptional()->optionalNormalWorld)
				{
					const sRGBFloat normalRgb = image->GetPixelNormalWorld(x, y);
					const double nx = normalRgb.R;
					const double ny = normalRgb.G;
					const double nz = normalRgb.B;
					const double nlen = sqrt(nx * nx + ny * ny + nz * nz);
					if (nlen > 0.1)
					{
						const int samples = 16;
						const double radius = scale_factor;
						const double bias = 0.002 * z;
						int occlusion = 0;
						for (int s = 0; s < samples; s++)
						{
							const double angle = double(s) / double(samples) * 2.0 * M_PI;
							const double ca = cos(angle);
							const double sa = sin(angle);
							const double xx = x + ca * radius * (s + 1);
							const double yy = y + sa * radius * (s + 1);
							if (int(xx) == x && int(yy) == y) continue;
							if (xx < startX || xx > endX - 1 || yy < startLine || yy > endLine - 1) continue;
							const double z2 = double(image->GetPixelZBuffer(int(xx), int(yy)));
							if (z2 > 1e19) continue;
							if (z2 - z - bias < 0.0) occlusion++;
						}
						total_ambient = 1.0f - float(occlusion) / float(samples);
						usedNormalSSAO = true;
					}
				}
