	...
	std::shared_ptr<cKeyframes> keyframes)
{
	...
	qint64 startParse = timer.elapsed();
	...
	if (textPrepared)
	{
		if (listOfParametersToProcess.isEmpty() && loadPassMode == loadPassNone)
		{
			// clear settings
		...
		for (int l = 3; l < separatedText.size(); l++)
		{
			...
			else
			{
				...
				{
					if (loadPassMode == loadPassDeferred)
					{
						int firstSpace = line.indexOf(' ');
						QString parameterName = line.left(firstSpace);
						if (IsCriticalSettingsParameter(QString("main_") + parameterName)) continue;
					}
					else if (!listOfParametersToProcess.isEmpty()) // selective loading
					{
				...
				else if (section.contains("fractal"))
				{
					...
					if (loadPassMode == loadPassDeferred)
					{
						int firstSpace = line.indexOf(' ');
						QString parameterName = line.left(firstSpace);
						if (IsCriticalSettingsParameter(QString("fractal%1_").arg(i) + parameterName))
							continue;
					}
					else if (!listOfParametersToProcess.isEmpty()) // selective loading
					{
				...
				{
					if (listOfParametersToProcess.isEmpty() || loadPassMode == loadPassDeferred)
					{
						if (frames)
						{
							if (csvLine == 0)
		...
		if (keyframes && listOfParametersToProcess.isEmpty() && loadPassMode == loadPassNone)
		{
		...
		if (format != formatAppSettings && loadPassMode != loadPassCritical)
		{
		...
		if ((listOfParametersToProcess.isEmpty() || loadPassMode == loadPassDeferred)
				&& loadPassMode != loadPassCritical)
		{
			if (frames) frames->LoadAllAudioFiles(par);
			if (keyframes) keyframes->LoadAllAudioFiles(par);

			// Fix orange fractal bug: mark palette/gradient buffers as dirty after parameter load
#ifdef USE_OPENCL
			if (gOpenCl && gOpenCl->openClEngineRenderFractal)
				gOpenCl->openClEngineRenderFractal->SetPaletteDirty();
#endif
		}

		qint64 endParse = timer.elapsed();
		qDebug() << "Settings decode - Parsing:" << (endParse - startParse) << "ms";
		qDebug() << "Settings decode - TOTAL:" << timer.elapsed() << "ms";
...
namespace
{
QStringList CriticalParameterNames()
{
	QStringList critical;
	critical << "main_camera"
					 << "main_target"
					 << "main_camera_top"
	...
	for (int i = 0; i < NUMBER_OF_FRACTALS; i++)
	{
		critical << QString("fractal%1_formula").arg(i) << QString("fractal%1_maxiter").arg(i)
						 << QString("fractal%1_iterations").arg(i) << QString("fractal%1_formula_id").arg(i);
	}
	return critical;
}
} // namespace

void cSettings::LoadCriticalParameters()
{
	listOfParametersToProcess = CriticalParameterNames();
}

bool cSettings::IsCriticalSettingsParameter(const QString &prefixedName)
{
	return CriticalParameterNames().contains(prefixedName);
}

void cSettings::ResetForSettingsLoad(std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> fractPar, std::shared_ptr<cAnimationFrames> frames,
	std::shared_ptr<cKeyframes> keyframes)
{
	par->ResetAllToDefault();
...
void cSettings::ValidateFormulaParameters(std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> /*fractPar*/) const
{
	if (format != formatAppSettings)
	{
		CheckIfMaterialsAreDefined(par);
...
void cSettings::ComputeDerivedParameters(std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> fractPar, std::shared_ptr<cAnimationFrames> frames,
	std::shared_ptr<cKeyframes> keyframes)
{
	if (format != formatAppSettings)
...
void cSettings::UpdateUI(std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> fractPar) const
{
	if (!gMainInterface) return;

	SyncFormulaMaxiterToN(par);
	gMainInterface->RegenerateListsAfterLoad();
	gMainInterface->SynchronizeInterface(par, fractPar, qInterface::write);
}

void cSettings::UpdatePreviews(std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> fractPar) const
{
	UpdateUI(par, fractPar);
}

bool cSettings::LoadSettings(QString filename, std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> fractPar, std::shared_ptr<cAnimationFrames> frames,
	std::shared_ptr<cKeyframes> keyframes, bool syncUi)
{
	QElapsedTimer totalTimer;
	QElapsedTimer phaseTimer;
	totalTimer.start();

	loadPassMode = loadPassNone;
	listOfParametersToProcess.clear();

	phaseTimer.start();
	if (!LoadFromFile(filename)) return false;
	qDebug() << "LoadSettings - File I/O:" << phaseTimer.elapsed() << "ms";

	phaseTimer.restart();
	if (!Decode(par, fractPar, frames, keyframes)) return false;
	qDebug() << "LoadSettings - Parsing:" << phaseTimer.elapsed() << "ms";

	EnsureDefaultFractalGradientMaterial(par);

	if (syncUi && gMainInterface)
	{
		phaseTimer.restart();
		gMainInterface->BeginBatchUpdate();
		UpdateUI(par, fractPar);
		gMainInterface->EndBatchUpdate();
		qDebug() << "LoadSettings - Parameter sync/UI update:" << phaseTimer.elapsed() << "ms";
	}

	qDebug() << "LoadSettings - TOTAL:" << totalTimer.elapsed() << "ms";
	return true;
}

void cSettings::FinishDeferredLoad(enumFormat fmt, const QString &settingsText, bool isQuiet,
...
{
	cSettings settings(fmt);
	settings.BeQuiet(isQuiet);
	settings.LoadFromString(settingsText);
	settings.loadPassMode = loadPassDeferred;
	settings.listOfParametersToProcess.clear();
	settings.Decode(par, fractPar, frames, keyframes);
	settings.ValidateFormulaParameters(par, fractPar);
	settings.ComputeDerivedParameters(par, fractPar, frames, keyframes);
	if (gMainInterface)
	{
		gMainInterface->BeginBatchUpdate();
		settings.UpdatePreviews(par, fractPar);
		gMainInterface->EndBatchUpdate();
		gMainInterface->OnDeferredSettingsLoadFinished();
	}
}

bool cSettings::StartAsyncSettingsDecode(std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> fractPar, std::shared_ptr<cAnimationFrames> frames,
	std::shared_ptr<cKeyframes> keyframes)
{
	QElapsedTimer phaseTimer;

	ResetForSettingsLoad(par, fractPar, frames, keyframes);
	LoadCriticalParameters();
	loadPassMode = loadPassCritical;

	phaseTimer.start();
	if (!Decode(par, fractPar, nullptr, nullptr))
	{
		loadPassMode = loadPassNone;
		listOfParametersToProcess.clear();
		return false;
	}
	qDebug() << "LoadSettingsAsync - Critical parsing:" << phaseTimer.elapsed() << "ms";
	...
	if (gMainInterface)
	{
		...
		UpdateUI(par, fractPar);
		gMainInterface->EndBatchUpdate();
		qDebug() << "LoadSettingsAsync - UI update:" << phaseTimer.elapsed() << "ms";
	}

	const QString capturedText = settingsText;
	const enumFormat capturedFormat = format;
	const bool capturedQuiet = quiet;

	QTimer::singleShot(0, qApp, [=]() {
		QElapsedTimer deferTimer;
		deferTimer.start();
		FinishDeferredLoad(capturedFormat, capturedText, capturedQuiet, par, fractPar, frames, keyframes);
		qDebug() << "LoadSettingsAsync - Deferred processing:" << deferTimer.elapsed() << "ms";
	});
...
bool cSettings::LoadSettingsAsync(QString filename, std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> fractPar, std::shared_ptr<cAnimationFrames> frames,
	std::shared_ptr<cKeyframes> keyframes)
{
	PERF_SCOPE("LoadSettingsAsync");
	QElapsedTimer totalTimer;
	QElapsedTimer phaseTimer;
	totalTimer.start();
	...
	phaseTimer.start();
	if (!LoadFromFile(filename)) return false;
	qDebug() << "LoadSettingsAsync - File I/O:" << phaseTimer.elapsed() << "ms";

	if (!StartAsyncSettingsDecode(par, fractPar, frames, keyframes)) return false;

	qDebug() << "LoadSettingsAsync - TOTAL (sync portion):" << totalTimer.elapsed() << "ms";
	return true;
}

bool cSettings::LoadSettingsFromClipboardAsync(std::shared_ptr<cParameterContainer> par,
	std::shared_ptr<cFractalContainer> fractPar, std::shared_ptr<cAnimationFrames> frames,
	std::shared_ptr<cKeyframes> keyframes)
{
	if (!LoadFromClipboard()) return false;
	return StartAsyncSettingsDecode(par, fractPar, frames, keyframes);
}
