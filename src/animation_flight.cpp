...
#include "animation_ui_helpers.hpp"
	...
	const sFrameRanges &frameRanges, bool *startRenderKeyframesAgain)
{
	...
	if (!gNetRender->IsClient())
	{
		if (frames->GetNumberOfFrames() > 0 && frameRanges.unrenderedTotalBeforeRender == 0)
		{
			...
			if (!systemData.noGui)
			{
				deletePreviousRender = (AskAnimationQuestion(mainInterface->mainWindow, questionTitle,
																	 questionText, QMessageBox::Yes | QMessageBox::No)
					== QMessageBox::Yes);
			}
			else
			{
