		...
		: cOpenClAbstractDynamicData(_numberOfTextures)
{
	...
	useTriplanarStochasticMapping = false;
	useCylindricalFractalMapping = false;
	useSphericalFractalMapping = false;
	useOctaMapping = false;
	useCameraMapping = false;
}
	...
	QMap<QString, int> *textureIndexes)
{
	...
	for (auto const &materialPair : materials) // for each material from materials
	{
		...
		switch (material.textureMappingType)
		{
			...
			case texture::mappingTriplanarStochastic: useTriplanarStochasticMapping = true; break;
			case texture::mappingCylindricalFractal: useCylindricalFractalMapping = true; break;
			case texture::mappingSphericalFractal: useSphericalFractalMapping = true; break;
			case texture::mappingOcta: useOctaMapping = true; break;
			case texture::mappingCamera: useCameraMapping = true; break;
		}
	}
...
QString cOpenClTexturesData::GetDefinesCollector() const
{
	...
	if (useTriplanarStochasticMapping) definesCollector += " -DUSE_TRIPLANAR_STOCHASTIC_MAPPING";
	if (useCylindricalFractalMapping) definesCollector += " -DUSE_CYLINDRICAL_FRACTAL_MAPPING";
	if (useSphericalFractalMapping) definesCollector += " -DUSE_SPHERICAL_FRACTAL_MAPPING";
	if (useOctaMapping) definesCollector += " -DUSE_OCTA_MAPPING";
	if (useCameraMapping) definesCollector += " -DUSE_CAMERA_MAPPING";
