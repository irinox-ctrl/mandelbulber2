...
void RenderWindow::slotMenuLoadPreset(QString filename)
{
	...
	if (!parSettings.LoadSettingsAsync(filename, gPar, gParFractal, gAnimFrames, gKeyframes))
	{
...
{
	*reply = AskQuestionBlocking(questionTitle, questionText, buttons);
}

QMessageBox::StandardButton RenderWindow::AskQuestionBlocking(const QString &questionTitle,
	const QString &questionText, const QMessageBox::StandardButtons buttons) const
{
	return QMessageBox::question(ui->centralwidget, questionTitle, questionText, buttons);
}
...
void RenderWindow::slotStartRender()
{
	...
	if (gPar) {
		double zoom = gPar->Get<double>("camera_distance_to_target");
		CheckZoomPrecision(zoom);
	}
