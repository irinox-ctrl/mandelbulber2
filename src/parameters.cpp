...
namespace
{
static QHash<QString, sParameterDefinition> parameterDefinitionCache;
static bool cacheInitialized = false;

sParameterDefinition DefinitionFromOneParameter(cOneParameter param)
{
	sParameterDefinition def;
	def.valueType = param.GetValueType();
	def.parType = param.GetParameterType();
	def.morphType = param.GetMorphType();
	def.isGradient = param.IsGradient();
	return def;
}
} // namespace

QHash<QString, sParameterDefinition> GetAllParameterDefinitions()
{
	QHash<QString, sParameterDefinition> definitions;
	if (gPar)
	{
		for (const QString &name : gPar->GetListOfParameters())
		{
			definitions.insert(name, DefinitionFromOneParameter(gPar->GetAsOneParameter(name)));
		}
	}
	if (gParFractal)
	{
		...
		{
			const cParameterContainer &fractPar = *gParFractal->at(i);
			for (const QString &name : fractPar.GetListOfParameters())
			{
				const QString key = QString("fractal%1_%2").arg(i).arg(name);
				definitions.insert(key, DefinitionFromOneParameter(fractPar.GetAsOneParameter(name)));
			}
		}
	}
	return definitions;
...
void InitializeParameterCache()
{
	...
	const QHash<QString, sParameterDefinition> definitions = GetAllParameterDefinitions();
	for (auto it = definitions.constBegin(); it != definitions.constEnd(); ++it)
	{
		parameterDefinitionCache.insert(it.key(), it.value());
	}
	cacheInitialized = true;
}

const sParameterDefinition &GetParameterDefinition(const QString &name)
{
	static sParameterDefinition defaultDef;
	if (!cacheInitialized) InitializeParameterCache();

	const auto it = parameterDefinitionCache.constFind(name);
	if (it != parameterDefinitionCache.constEnd()) return it.value();
	return defaultDef;
}
} // namespace parameterContainer
